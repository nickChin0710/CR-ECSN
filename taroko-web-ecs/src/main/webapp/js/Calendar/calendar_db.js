...
function f_tcalParseDate (s_date) {
  if(s_date.length!=6 && s_date.length!=8)	return alert("日期格式錯誤,yyyymmdd");
	var re_date   = /^\s*(\d{2,4})\/(\d{1,2})\/(\d{1,2})\s*$/;
	var re_date2  = /^\s*(\d{2,4})(\d{1,2})(\d{1,2})\s*$/;
	if ( !re_date.exec(s_date) && !re_date2.exec(s_date) )
...
function f_tcalShow (d_date) {
	...
	this.b_visible = true;
  // change query frame height ,add by Jack Liao
	top.calendarExpand();
}
...
function f_tcalUpdate (d_date) {
	...
	var a_class, s_html = '<table class="ctrl"><tbody><tr>'
		...
		+ d_date.getFullYear()+'/'+this.a_tpl.months[d_date.getMonth()]
