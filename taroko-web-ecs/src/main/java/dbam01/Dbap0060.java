...
public class Dbap0060 extends BaseAction {
	...
	public void queryFunc() throws Exception {
		...
		String lsWhere = " where 1=1 and A.mtch_flag <> 'Y' and A.vdcard_flag = 'D' "
				+ " and uf_nvl(unlock_flag,'N') ='N' and trans_type <> '0420' "
				+ " and reversal_flag <> 'Y' ";
		
		// 2022/03/22
//		String lsWhere = " where 1=1 and A.mtch_flag <> 'Y' and A.vdcard_flag = 'D' "
//				+ " and uf_nvl(unlock_flag,'N') ='N' and iso_resp_code = '00' and trans_type <> '0420' "
//				+ " and reversal_flag <> 'Y' ";
		if ("00".equals(wp.itemStr("ex_iso_resp_code"))) {
		...
		lsWhere +=  sqlBetween("ex_date1", "ex_date2", "A.tx_date")
	...
	public void queryRead() throws Exception {
		...
		wp.selectSQL = "hex(A.rowid) as rowid ," + " A.auth_seqno ," + " A.card_no ," + " A.tx_date ," + " A.tx_time ,"
				...
				+ " A.vd_lock_nt_amt , A.card_acct_idx , A.acno_p_seqno ";
...
//			lsSentData += "IBPBMCRD 0200VDRTATM ";
			...
			// --圈存序號
			if(oldDataDate.compareTo(aaTxDate[ii]) > 0) {
				lsSentData += commString.rpad(aaTxSeq[rr], 10);
			}	else	{
...
//			lsSentData += commString.rpad(aaTxSeq[rr], 10); // --圈存序號
	...
	public int updateCcaAuthTxlog(String lsRowid) {
		...
		sql1 += " chg_date = to_char(sysdate,'yyyymmdd') , chg_time = to_char(sysdate,'hh24miss') , chg_user = ? where 1=1 and rowid = ? ";

		setString(1, wp.modPgm());
		setString(2, wp.loginUser);
		setString(3, wp.loginUser);
		setRowid(4,lsRowid);
		sqlExec(sql1);
	...
	int insertImsLog(int i, String sendData, String respCode, String imsSeqNo) {
		...
		sql1 = "insert into cca_ims_log (" + " tx_date ," + " tx_time ," + " card_no ," + " auth_no ," + " trans_type ,"
				...
				+ " to_char(sysdate,'yyyymmdd') ," + " '' ," + " sysdate ," + " 'Dbap0060' " + " )";
