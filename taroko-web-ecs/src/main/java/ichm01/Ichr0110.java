...
public class Ichr0110 extends BaseAction implements InfacePdf {
  ...
  public void queryFunc() throws Exception {
    ...
    String lsWhere = " where 1=1 " + sqlCol(wp.itemStr2("ex_close_user"), "A.close_user")
        + sqlCol(wp.itemStr2("ex_txn_date1"), "A.txn_date", ">=")
        + sqlCol(wp.itemStr2("ex_txn_date2"), "A.txn_date", "<=")
        + sqlCol(wp.itemStr2("ex_close_date1"), "A.close_date", ">=")
        + sqlCol(wp.itemStr2("ex_close_date2"), "A.close_date", "<=");

    isReportName = "愛金卡問題交易明細表";

    if (wp.itemEq("ex_close_flag", "Y")) {
      isClose = "結案類別: 已結案  理由:" + wp.itemStr2("ex_close_reason");
      lsWhere += " and A.close_date <> '' ";
      if (wp.itemEmpty("ex_close_reason") == false) {
        lsWhere += sqlCol(wp.itemStr2("ex_close_reason"), "A.close_reason");
      }
      if (wp.itemEq("ex_apr_flag", "Y")) {
        lsWhere += " and A.clo_apr_date <> '' ";
        isReportName = "愛金卡問題交易明細表--已覆核";
      } else {
        lsWhere += " and A.clo_apr_date = '' ";
        isReportName = "愛金卡問題交易明細表--未覆核";
      }
    } else {
      isClose = "結案類別: 未結案";
      lsWhere += " and A.close_date = '' ";
  ...
  public void queryRead() throws Exception {
    ...
    wp.selectSQL = "" + " A.ich_card_no ," + " A.txn_date ," + " A.txn_time ," + " A.txn_amt ,"
        + " A.agency_cd ," + " A.store_cd ," + " A.close_reason ," + " A.close_user ,"
        + " A.close_date ," + " A.clo_apr_date ," + " B.card_no ";
  ...
  public void pdfPrint() throws Exception {
    wp.reportId = "ichr0110";
    wp.pageRows = 9999;
    String tmpStr = "";
    tmpStr = "交易日期:" + commString.strToYmd(wp.itemStr2("ex_txn_date1")) + " -- "
        + commString.strToYmd(wp.itemStr2("ex_txn_date2")) + "  結案日期:"
        + commString.strToYmd(wp.itemStr2("ex_close_date1")) + " -- "
        + commString.strToYmd(wp.itemStr2("ex_close_date2")) + "　　" + isClose;
    wp.colSet("cond1", tmpStr);
    queryFunc();
    wp.colSet("report_name", isReportName);
    TarokoPDF pdf = new TarokoPDF();
    wp.fileMode = "Y";
    pdf.excelTemplate = "ichr0110.xlsx";
    pdf.pageCount = 30;
    pdf.sheetNo = 0;
