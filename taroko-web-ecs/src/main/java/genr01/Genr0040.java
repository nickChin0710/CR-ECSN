...
public class Genr0040 extends BaseReport {

  InputStream inExcelFile = null;
  String mProgName = "genr0040";
  ...
  private boolean getWhereStr() throws Exception {
    String std_vouch_cd_f = wp.itemStr("ex_std_vouch_cd_f");
    String std_vouch_cd_u = wp.itemStr("ex_std_vouch_cd_u");
    ...
    if (empty(std_vouch_cd_f) == false) {
      ls_where += " and std_vouch_cd >= :std_vouch_cd_f";
      setString("std_vouch_cd_f", std_vouch_cd_f);
    }

    if (empty(std_vouch_cd_u) == false) {
      ls_where += " and std_vouch_cd <= :std_vouch_cd_u";
      setString("std_vouch_cd_u", std_vouch_cd_u);
    }

    ls_where += sqlCol(curr, "curr", "like%");
  ...
  public void queryRead() throws Exception {
    ...
    wp.daoTable = "gen_sys_vouch v " + "left join ptr_currcode p on v.curr=p.curr_code_gl "
  ...
  void subTitle() {
    ...
    String tmpStr = "";
    // 傳送日期
    if (empty(std_vouch_cd_f) == false || empty(std_vouch_cd_u) == false) {
      tmpStr += "標準分錄代碼 : ";
      if (empty(std_vouch_cd_f) == false) {
        tmpStr += std_vouch_cd_f + " 起 ";
      }
      if (empty(std_vouch_cd_u) == false) {
        tmpStr += " ~ " + std_vouch_cd_u + " 迄 ";
      }
    }
    if (empty(curr) == false) {
      tmpStr += " 幣別 : ";
  ...
  public void dddwSelect() {
    try {
      ...
      this.dddwList("dddw_std_vouch_cd_f", "gen_sys_vouch", "std_vouch_cd", "",
          "where 1=1 group by std_vouch_cd order by std_vouch_cd");

      wp.initOption = "--";
      wp.optionKey = wp.itemStr("ex_std_vouch_cd_u");
      this.dddwList("dddw_std_vouch_cd_u", "gen_sys_vouch", "std_vouch_cd", "",
          "where 1=1 group by std_vouch_cd order by std_vouch_cd");
