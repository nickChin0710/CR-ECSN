...
public class Rskp0015 extends BaseAction {
String kk1 = "", isReferNo = "", isCtrlSeqno = "";
taroko.base.CommDate zzdate = new taroko.base.CommDate();
int procClose = 0;
int iiCurrTerm=0;
rskm01.Rskp0010 rskp0010 = new rskm01.Rskp0010();	

@Override
public void userAction() throws Exception {
	strAction = wp.buttonCode;
	rskp0010.strAction = strAction;
	rskp0010.wp = wp;
	if (eqIgno(wp.requHtml,"rskp0015")) {
		switch (wp.buttonCode) {
			case "S":
				if (wp.respHtml.indexOf("_bill") > 0) {
					rskp0010.querySelect();  //--
		         return;
		      }				
				strAction = "R";
		      //--
		      if (rskp0010.procCheckOpt() != 1) {
		         return;
		      }
		      
		      if (wp.respHtml.indexOf("_rept") > 0)
		      	rskp0010.dataReadRept();
		      else if (wp.respHtml.indexOf("_chgb") > 0)
		      	rskp0010.dataReadChgb(); 
		      break;
			case "Q": //-查詢功能-
				queryFunc(); break;
	...
	if (wp.requHtml.indexOf("_rept") >0) {
      switch (wp.buttonCode) {
         case "R":
         	rskp0010.dataReadRept(); break;
         case "A":
         case "U":
         case "D":
         	rskp0010.dbSaveReceipt(); break;
      }
      return;
   }
   if (wp.requHtml.indexOf("_chgb") >0) {
      switch (wp.buttonCode) {
         case "R":
         	rskp0010.dataReadChgb(); break;
         case "A":
         case "U":
         case "D":
         	rskp0010.dbSaveChgback(); break;
      }
      return;
   }          
...
//   		+ " and ( (A.rsk_type<>'' and A.bill_type<>'NCFC')  or (A.rsk_type='4' and A.bill_type='NCFC') or A.rsk_type ='' ) "
   ...
   String ls_sql="select count(*) as wk_tot_cnt, sum(decode(A.sign_flag,'-',0 - A.dc_dest_amt,A.dc_dest_amt)) as wk_tot_amt"+
         " from bil_bill A left join rsk_problem B on A.reference_no = B.reference_no "+wp.whereStr;
   sqlSelect(ls_sql);
...
public void queryRead() throws Exception {
   ...
   ;
   wp.daoTable = "bil_bill A left join rsk_problem B on A.reference_no = B.reference_no ";
...
public void initButton() {
   ...
   else if (eqIgno(wp.respHtml, "rskp0015")) {
      btnModeAud("XX");
   }
