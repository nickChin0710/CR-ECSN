 ...
 * ---------  --------  ---------- ------------------------------------------ *
 * 106-12-20  V1.00.00  Andy       program initial                            *
 * 107-03-28  V2.00.00  ryan       update                            		 *
 * 107-05-10  V2.00.01  Andy       update report2,report3              		 *
 * 107-12-19  V4.00.02  ryan       update mantis:0002071              		 *
 * 107-12-19  V5.00.03  ryan       update mantis:0002222              		 *
 * 109-01-14  V1.00.05  Andy       update UI                          		 *
 * 109-01-03  V5.00.04  Justin Wu    updated for archit.  change
 * 107-01-21  V1.00.07  ryan       update mantis:0002495              		 *
 * 109-04-21  V1.00.05  YangFang   updated for project coding standard        *
 * 109-07-20  v1.00.09  Amber      Update add wf_report_memo3()               *
 * 109-07-21  v1.00.10  Amber      Update Mantis:0003800		                 *
 * 109-08-07  v1.00.11  Amber      Update Mantis:0003908		                 *
 * 109-09-07  v1.00.12  Andy       Update Mantis:4072     	                 *
 * 109-09-29  v1.00.13  Andy       Update Mantis:4094     	                 *
 * 109-10-06  v1.00.14  Andy       Update add chk flag ,close flag            *
 * 109-10-20  v1.00.15  Andy       Update Mantis:4425                         *
 * 110-01-27  v1.00.16  Andy       Update Mantis:5820                         *
 * 110-04-16  v1.00.17  Andy       Update Week scan                           *
 * 110-05-10  v1.00.18  Andy       Update Mantis6940                          *
 * 110-05-17  v1.00.19  Andy       Update Mantis7020                          *
 * 110-06-16  v1.00.20  Andy       Update Mantis7020                          *
 * 110-06-23  v1.00.21  Andy       Update Mantis7274                          *
 * 110-07-29  v1.00.21  Andy       Update Mantis8019                          *
 * 110-08-06  v1.00.22  Andy       Update add re_serno()                      *
 * 110-08-09  v1.00.23  Andy       Update Mantis8197                          *
 * 110-09-07  v1.00.24  Andy       Update Mantis8616                          *
 * 110-12-08  v1.00.25  Andy       Update Mantis9118                          *
 * 111-04-06  v1.00.26  Lai        Update Mantis9341 display cnt              *
 * 111-11-28  V1.00.27  Zuwei      Sync from mega                             *
 * 111-11-01  V1.00.28  Zuwei      add report template，update 分行/單位key 109 to 3144，列印種類select check                            *
 * 112-02-07  V1.00.29  YANG HAN   ADD NEW filed                         *
 * 112-02-08  V1.00.30  Zuwei Su   queryRead() sql bug
 * 112-11-28  V1.00.31  Zuwei Su   dataProcess 取值ls_vouch_close_flag的sql調整
 * 112-12-05  V1.00.32  Zuwei Su   取消原[會計分錄]格式, 增[傳票]格式
 * 113-01-03  V1.00.33  Zuwei Su   產出PDF摘要按20字換行                                                                                    *    
 ******************************************************************************/
...
public class Genp0110 extends BaseProc {
    String mProgName = "genp0110";
...
//			aa_tx_date[ii] = wp.item_ss("ex_tx_date");
    ...
    public int ofcUpdate() throws Exception {
        ...
        for (int ii = 0; ii < aaBrno.length; ii++) {
            ...
            //
//			if (!empty(aa_rowid[ii])) {
//				sp.ppss("ac_no", aa_ac_no[ii]);
//				sp.ppss("dbcr", aa_dbcr[ii]);
//				sp.ppss("sign_flag", aa_sign_flag[ii]);
//				sp.ppnum("amt", this.to_Num(aa_amt[ii]));
//				sp.ppss("id_no", aa_id_no[ii]);
//				sp.ppss("memo1", aa_memo1[ii]);
//				sp.ppss("memo2", aa_memo2[ii]);
//				sp.ppss("memo3", aa_memo3[ii]);
//				sp.ppss("key_value", aa_key_value[ii]);
//				sp.ppss("post_flag", aa_post_flag[ii]);
//				sp.ppss("ifrs_flag", aa_ifrs_flag[ii]);
//				sp.ppss("curr_code_dc", aa_curr_code_dc[ii]);
//				sp.ppss("curr", aa_curr[ii]);
//				sp.ppss("brno", aa_brno[ii]);
//				sp.ppss("dept", aa_dept[ii]);
//				sp.ppss("depno", aa_depno[ii]);
//				sp.ppnum("voucher_cnt", this.to_Int(aa_voucher_cnt[ii]));
//				sp.ppss("jrn_status", aa_jrn_status[ii]);
//				sp.ppss("apr_user", wp.item_ss("zz_apr_user"));
//				sp.ppss("mod_user", wp.loginUser);
//				sp.ppss("mod_pgm", wp.mod_pgm());
//				sp.addsql(", mod_time =sysdate");
//				sp.addsql(", mod_seqno =nvl(mod_seqno,0)+1");
//				sp.sql2Where("where hex(rowid)=?", aa_rowid[ii]);
//				sqlExec(sp.sql_stmt(), sp.sql_parm());

//				if (sqlRowNum <= 0) {
//					alert_msg(" update gen_vouch err ");
//					wp.colSet(ii, "err_msg", "update gen_vouch err");
//					wp.colSet(ii, "ok_flag", "!");
//					return -1;
//				}
//			}
            ...
            sp.ppstr("refno", aaRefno[ii]);
            sp.ppstr("sys_rem", aaSysRem[ii]);
    ...
    void pdfPrint() throws Exception {
        ...
        switch (exPrintKind) {
            case "1":
                pdf.excelTemplate = "genp0110r02.xlsx";
                wfReport();
                break;
            case "2":
                pdf.excelTemplate = "genp0110r01.xlsx";
                wfReportMemo3();
                break;
            case "3":
                pdf.excelTemplate = "genp0110r03.xlsx";
    ...
    void pdfPrint2() throws Exception {
        wp.reportId = "Genp0110_傳票";
        wp.pageRows = 9999;
        queryReadPdf();
//      wp.setListCount(1);
        TarokoPDF pdf = new TarokoPDF();
//      pdf.fixHeader[1] = "cond_1";
        wp.fileMode = "Y";
        pdf.pageVert = true;
        pdf.excelTemplate = "genp0110_pdf.xlsx";
        pdf.sheetNo = 0;
        pdf.pageCount = 30;
        pdf.procesPDFreport(wp);
        pdf = null;
    ...
    private void queryReadPdf() throws Exception {
        if (getWhereStr() != 1) {
            return;
        }
        wp.pageControl();
        ...
        wp.whereOrder = " order by v.tx_date,v.refno" + ",dbcc desc ,seqno";
        ...
        for (int ii = 0; ii < cnt; ii++) {
            ...
            if (!empty(wp.getValue("acct_no", ii)) && !empty(wp.getValue("acct_name", ii))) {
                wp.colSet("acctNM",
                        wp.getValue("acct_no", ii) + "-" + wp.getValue("acct_name", ii));
            }
            String dbcr = "";
            ...
            for (int i = 0; i < acNo.length; i++) {
                wp.setValue("ac_no_DC"+(i+1), acNo[i], ii);
            }
            ...
            // 經辦
            String crtName = sellectUsrcname(wp.colStr(ii, "crt_user")); // SelectSecUser(txDate, refno);
            ...
            if (wp.getValue("mod_pgm", ii).equals("genp0110")
                    || wp.getValue("mod_pgm", ii).equals("genp0110")) {
