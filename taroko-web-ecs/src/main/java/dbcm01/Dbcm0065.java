...
* ---------  --------  ---------- ------------------------------------------ *
* 112-02-15  V1.00.00  Ryan        program initial                            *
* 112-03-09  V1.00.01  Wilson    updateDbcEmapTmp2 set reject_code = check_code*
******************************************************************************/
...
public class Dbcm0065 extends BaseEdit {
  String mExBatchno = "";
  String mExIdNo = "";
  String mExProcFlag = "";
  String[] aaRowid = null;
...
//      return;
  ...
  public void queryRead() throws Exception {
    ...
    wp.selectSQL = "hex (rowid) AS rowid, " + "'        ' cpm_birthday, " 
        ...
        // wf_id = dbc_emap_tmp.check_code) as db_check_code, "
  ...
  public void saveFunc() throws Exception {
    ...
    // update main table [dbc_emap_tmp] use
  ...
  public int updateCrdEmapTmp(int ii) {
    ...
    usSql = "update dbc_emap_tmp set " + "source_code =:source_code, " + "eng_name =:eng_name, "
        + "birthday =:birthday, " + "stmt_cycle =:stmt_cycle, " 
    	+ "check_code =:ls_check_code, " + "mod_user =:mod_user, "
        + "mod_time = sysdate, " + "mod_pgm = 'dbcm0065', " + "mod_seqno = nvl(mod_seqno,0)+1 where hex(rowid) = :rowid ";
...
//    usSql += sqlCol(aaModSeqno[ii], "mod_seqno");
    ...
    sqlExec(usSql);
    if (sqlRowNum <= 0) {
      wp.colSet(ii, "err_msg", "Update dbc_emap_tmp err");
  ...
  public int crdRejectProc(int ii) {
    ...
    lsSql = "select hex(rowid) as rowid, mod_seqno from dbc_reject where 1=1 ";
    ...
    if (sqlRowNum > 0) {
      ...
      // update dbc_reject
      usSql = "update dbc_reject set "  // 1
          + "emboss_source =:emboss_source, " 
          ...
          + "graduation_elementarty =:graduation_elementarty, "
          ...
          + "mod_user =:mod_user, " + "mod_time = sysdate, " + "mod_pgm ='dbcm0065', "
      ...
      if (sqlRowNum <= 0) {
        wp.colSet(ii, "err_msg", "Update dbc_reject err");
        wp.colSet(ii, "ok_flag", "X");
        return -1;
      }
    } else {
      // insert dbc_reject
      isSql = "insert into dbc_reject ("
          + "batchno,           recno,          apply_id,        birthday, "
          + "emboss_source,  seqno,             card_type, "
          ...
          + "spouse_id_no,      spouse_birthday,  resident_no_expire_date,  passport_date, "
          + "bill_apply_flag,   company_zip,     "
          + "graduation_elementarty,   spouse_name,    company_addr1, "
          + "company_addr2,     company_addr3,    company_addr4,            company_addr5,  "
          + "crt_user,          promote_dept,   promote_emp_no, "
      ...
      sqlExec(isSql);
      if (sqlRowNum <= 0) {
        wp.colSet(ii, "err_msg", "Insert dbc_reject err");
        wp.colSet(ii, "ok_flag", "X");
        return -1;
      }
    }
  ...
  public int updateDbcEmapTmp2(int ii) {
//    String dsSql = "";
//    dsSql = "delete crd_emap_tmp where hex(rowid) =:rowid and mod_seqno =:mod_seqno ";
//    setString("rowid", aaRowid[ii]);
//    setString("mod_seqno", aaModSeqno[ii]);
//    sqlExec(dsSql);
	  String usSql = "update dbc_emap_tmp set reject_code = :check_code ,reject_date = to_char(sysdate,'yyyymmdd'),mod_user = :mod_user ,mod_time = sysdate where 1=1 and hex(rowid) = :rowid and mod_seqno = :mod_seqno ";
		    ...
		    sqlExec(usSql);
    if (sqlRowNum <= 0) {
      wp.colSet(ii, "err_msg", "update dbc_emap_tmp err2");
      wp.colSet(ii, "ok_flag", "X");
      return -1;
    }
    return 1;
