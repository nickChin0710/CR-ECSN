...
public class Ecsq0020 extends BaseEdit
{
  ...
  public void queryRead() throws Exception {
    ...
    wp.selectSQL =
        " " + "hex(a.rowid) as rowid, " + "nvl(a.mod_seqno,0) as mod_seqno, " + "a.obj_code,"
            + "a.obj_name," + "a.obj_type," + "a.unit_code," + "a.owner_code," + "a.owner_name";
  ...
  public void saveFunc() throws Exception {
    ecsq01.Ecsq0020Func func = new ecsq01.Ecsq0020Func(wp);
  ...
  public void dddwSelect() {
    try {
      if ((wp.respHtml.equals("ecsq0020"))) {
        wp.initOption = "--";
        wp.optionKey = "";
  ...
  public void commDeptCode(String columnData1) throws Exception {
    ...
    for (int ii = 0; ii < wp.selectCnt; ii++) {
      if (wp.colStr(ii, "unit_code").length() == 0)
            continue;
      columnData = "";
      sql1 = "select " + " dept_nam as column_dept_nam " + " from ptr_dept_code " + " where 1 = 1 "
          + " and   dept_code = ? ";
      
      sqlSelect(sql1, new Object[] {wp.colStr(ii, "unit_code")});

      if (sqlRowNum > 0) {
        columnData = columnData + sqlStr("column_dept_nam");
