package ccam01;
/* 卡戶交易記錄查詢(auth_txlog) V.2018-1023.jh
 * 109-04-20 V1.00.00  Zhenwu Zhu       updated for project coding standard
 * 109-12-25 V1.00.01   Justin                   parameterize sql
 * 110-01-06 V1.00.02   Justin                   updated for XSS
 */

import java.util.Arrays;

public class Ccaq1030Func extends busi.FuncAction {
  ...
  public int querySelect() {
    ...
    if (wp.itemEmpty("ex_idno") == false) {
      this.daoTid = "wk.";
      strSql = "select uf_idno_pseqno(?) as id_pseqno" + ", uf_vd_idno_pseqno(?) as id_pseqno2"
          + " from " + this.sqlDual;
    ...
    if (wp.itemEmpty("ex_tx_time1") && wp.itemEmpty("ex_tx_time2")) {
      ...
      if (!wp.itemEmpty("ex_tx_date1")) setString("ex_tx_date1");
      if (!wp.itemEmpty("ex_tx_date2")) setString("ex_tx_date2");
    } else {
      lsWhere += commSqlStr.strend(wp.itemStr("ex_tx_date1"), wp.itemStr("ex_tx_date2"), "A.tx_date");
      lsWhere += commSqlStr.strend(wp.itemStr("ex_tx_time1"), wp.itemStr("ex_tx_time2"), "A.tx_time");
      if (!wp.itemEmpty("ex_tx_date1")) setString("ex_tx_date1");
      if (!wp.itemEmpty("ex_tx_date2")) setString("ex_tx_date2");
      if (!wp.itemEmpty("ex_tx_time1")) setString("ex_tx_time1");
      if (!wp.itemEmpty("ex_tx_time2")) setString("ex_tx_time2");
    }
    ...
    dsLog.colList = this.sqlQuery(strSql);
    wp.selectCnt = 0;
  ...
  int checkBankFhc(String lsIdno) {
    ...
    String sql1 = " select " + " count(*) as db_cntBF " + " from ecs_employee "
        + " where id_no = ? " + " and status_id = '1' ";
