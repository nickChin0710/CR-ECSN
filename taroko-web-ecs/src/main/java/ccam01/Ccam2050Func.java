package ccam01;
/** 臨時調整額度維護（依卡戶）V.18-0922.jh
 * 18-0926:		JH		modify
 * 18-0810:		JH		簡訊
 * 109-04-20 V1.00.00  Zhenwu Zhu       updated for project coding standard
 * 111-12-21 V1.00.01  Alex             需求單要求不檢核起日
 * */

import busi.FuncAction;

public class Ccam2050Func extends FuncAction {
...
//		}
	...
	void getHighRiskType() {
		...
		int zz = sqlRowNum;
		for(int rr=0 ; rr<zz;rr++) {
			if (empty(adjRiskType))
				adjRiskType += colStr(rr,"sys_key");
			else
				adjRiskType += "," + colStr(rr,"sys_key");			
		}		
	}
	...
	void updateCcaCardAcct() {
		strSql = "update cca_card_acct set " + " adj_eff_start_date =:adj_eff_start_date ,"
				...
				+ " adj_risk_flag =:adj_risk_flag ," + " adj_sms_flag =:adj_sms_flag , "
				+ " adj_risk_type =:adj_risk_type , notice_flag =:notice_flag ,"
	...
	public int dbDelete() {
		...
		strSql = " update cca_card_acct set " + " adj_eff_start_date = '' , " + " adj_eff_end_date = '' , "
				...
				+ " adj_inst_pct = 0 , " + " adj_quota = 'N' ," + " notice_flag = 'N' , " + " adj_sms_flag ='N' , "
				+ " adj_risk_flag ='N' , " + commSqlStr.setModxxx(modUser, modPgm) + " where card_acct_idx =:card_acct_idx ";
		...
		strSql = " insert into cca_limit_adj_log ( " + " log_date , " + " log_time , " + " aud_code , "
				+ " card_acct_idx , " + " mod_type , " + " rela_flag , " + " tot_amt_month_b , " + " tot_amt_month , "
				+ " adj_inst_pct_b , " + " adj_inst_pct , " + " adj_eff_date1 , " + " adj_eff_date2 , "
				+ " adj_reason , " + " adj_remark , " + " adj_area , " + " ecs_return_code , " + " ecs_adj_rate , "
				+ " adj_user , " + " adj_date , " + " adj_time , " + " debit_flag " + " ) values ( "
				+ " to_char(sysdate,'yyyymmdd') , " + " to_char(sysdate,'hh24miss') , " + " 'D' , "
				+ " :card_acct_idx , " + " '1' , " + " :rela_flag , " + " :tot_amt_month_b , " + " :tot_amt_month , "
				+ " :adj_inst_pct_b , " + " 100 , " + " '' , " + " '' , " + " '' , " + " '[Delete]' , " + " '' , "
				+ " '' , " + " 0 , " + " :adj_user , " + " to_char(sysdate,'yyyymmdd') , "
				+ " to_char(sysdate,'hh24miss') , " + " 'N'" // debit_flag
				+ " )";

		setDouble2("card_acct_idx", commString.strToNum(cardAcctIdx));
		item2ParmStr("rela_flag");
		item2ParmNum("tot_amt_month_b", "org_amt_mon");
		item2ParmNum("tot_amt_month", "line_credit_amt");
		item2ParmNum("adj_inst_pct_b", "ori_inst_pct");
		setString2("adj_user", modUser);
		// item2Parm_ss("debit_flag");
		// item2Parm_ss("id_p_seqno");
		// item2Parm_ss("p_seqno");
	...
	public int insertDetlParm() {
		strSql = "insert into cca_adj_parm (" + " card_acct_idx, " // 1
				...
				+ " crt_date," + " crt_time," + " crt_user," + " mod_user," + " mod_time," + " mod_pgm," + " mod_seqno"
				+ " ) values (" + " :card_acct_idx," + " :risk_type," + " :adj_month_amt," + " :adj_month_cnt,"
				+ " :adj_day_amt," + " :adj_day_cnt," + " :card_note, :risk_level," + " :adj_eff_start_date ,"
		...
		setString2("risk_level", wp.itemStr2("class_code"));
	...
	int insertLimitAdjLog() {
		...
		// ppp("p_seqno", wp.sss("p_seqno"));
		// ppp("debit_flag", "N");
		// ppp("id_p_seqno", wp.sss("id_p_seqno"));
