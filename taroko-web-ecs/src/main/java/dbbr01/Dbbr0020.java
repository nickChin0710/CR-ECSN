...
public class Dbbr0020 extends BaseReport {

  InputStream inExcelFile = null;
  String progName = "dbbr0020";
  ...
  private boolean getWhereStr() throws Exception {
	sqlParm.clear();
    String date1 = wp.itemStr("exDateS");
    String date2 = wp.itemStr("exDateE");
    // 固定條件
  ...
  public void queryRead() throws Exception {
    ...
    if (getWhereStr() == false)
      return;
    wp.selectSQL = "sum(atm_amt_f) as sum_atm_amt_f, " + "sum(atm_cnt_f) as sum_atm_cnt_f, "
        + "sum(atm_amt_f_fee) as sum_atm_amt_f_fee, " + "sum(atm_cnt_f_fee) as sum_atm_cnt_f_fee, "
        + "sum(purchase_amt_f) as sum_purchase_amt_f, "
        + "sum(purchase_cnt_f) as sum_purchase_cnt_f, "
        + "sum(purchase_amt_l) as sum_purchase_amt_l, "
        + "sum(purchase_cnt_l) as sum_purchase_cnt_l, " + "sum(lf_amt) as sum_lf_amt, "
        + "sum(lf_cnt) as sum_lf_cnt, " + "sum(rr_amt) as sum_rr_amt, "
        + "sum(rr_cnt) as sum_rr_cnt, " + "sum(atm_amt_f_m) as sum_atm_amt_f_m, "
        + "sum(atm_cnt_f_m) as sum_atm_cnt_f_m, "
        + "sum(purchase_amt_f_m) as sum_purchase_amt_f_m, "
        + "sum(purchase_cnt_f_m) as sum_purchase_cnt_f_m, "
        + "sum(purchase_amt_l_m) as sum_purchase_amt_l_m, "
        + "sum(purchase_cnt_l_m) as sum_purchase_cnt_l_m, " + "effc_yyyymm," + "'' wk_temp ";
