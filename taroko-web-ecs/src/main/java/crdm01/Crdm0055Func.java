...
* 109-02-11  V1.00.04  Wilson     insert add nccc_type = '1'                 *
* 109-02-15  V1.00.05  Wilson     配偶ID不為空值才檢核               				         *
* 109-03-24  V1.00.06  Zhenwu Zhu 統一證號檢核                                                                                                *
* 109-03-26  V1.00.07  Wilson     統一證號檢核規則調整                                                                                 *
* 109-04-28  V1.00.08  YangFang   updated for project coding standard        *
* 109-06-03  V1.00.08  Shiyuqi    checkId方法調用變更                                                                             *
* 109-06-09  V1.00.09  shiyuqi      修改程式名稱                                                                                           *
* 109-12-03  V1.00.10   Justin       extract common code                     *
* 110-10-08  V1.00.07   Justin    add zip2                                   *
* 112-11-20  V1.00.08   Ryan      移除 身分證、配偶ID檢核                                                                           *
* 112-11-20  V1.00.09   Wilson    還原 身分證、配偶ID檢核                                                                           
* 112-12-25  V1.00.10   Ryan      新增修改時重新取得進件分行                                                                       *
******************************************************************************/
...
public class Crdm0055Func extends FuncEdit {
  ...
  public Crdm0055Func(TarokoCommon wr) {
    wp = wr;
  ...
  public void dataCheck() {
    ...
    if(!this.ibDelete) {
    	...
    	String sql1 = " select a.reg_bank_no from act_acno a join crd_corp b on a.corp_p_seqno = b.corp_p_seqno where a.acno_flag ='2' and b.corp_no = :corp_no "; 
		setString("corp_no",wp.itemStr("corp_no"));
    	sqlSelect(sql1);
    	if(empty(colStr("reg_bank_no"))) {
    		errmsg("進件分行不可為空白，請至cmsm2020的帳戶資料一頁籤維護受理行欄位!");
       	    return;
    	}
    	wp.itemSet("reg_bank_no" , colStr("reg_bank_no"));
    }
    ...
    // 總繳/個繳註記
    String corpActFlag = wp.itemStr("corp_act_flag");
    String corpNo = wp.itemStr("corp_no");
    strSql = "select corp_act_type from crd_corp where corp_no = :corpNo";
    setString("corpNo", corpNo);
    sqlSelect(strSql);
    if ("Y".equals(corpActFlag)) {
      corpActFlag = "1";
    } else if ("N".equals(corpActFlag)) {
      corpActFlag = "2";
    }
    if (!corpActFlag.equals(colStr("corp_act_type"))) {
      errmsg("總繳/個繳註記不一致");
      rc = -1;
    }
    ...
    String stmtCycle = getStmtCycle(wp.itemStr("group_code"), wp.itemStr("card_type"), corpNo);
  ...
  public int dbInsert() {
    ...
    CrdEmap crdEmap = getCrdEmap(wp.itemStr("unit_code"), wp.itemStr("group_code"), wp.itemStr("revolve_int_rate_year"));
    ...
    Object[] param = new Object[] {wp.itemStr("apply_id").toUpperCase(), wp.itemStr("batchno"),
        ...
        wp.itemStr("online_mark").equals("") ? "N" : "Y", wp.itemStr("corp_no"),
        wp.itemStr("corp_act_flag"), wp.itemStr("stmt_cycle"), wp.itemStr("apply_id").toUpperCase(),
   ...
   */
	 ...
	 */
	private CrdEmap getCrdEmap(String unitCode, String groupCode, String revolveIntRateYearStr) {
		CrdEmap crdEmap = new CrdEmap();
