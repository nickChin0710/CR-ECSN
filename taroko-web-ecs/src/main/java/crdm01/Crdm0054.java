...
public class Crdm0054 extends BaseEdit {
  String mExBatchno = "";
  String mExApplyNo = "";
  String mExApplyId = "";
  String mExCorpNo = "";
...
@Override
  public void initPage() {
    String lsSql = "";
    // 批號
    wp.colSet("batchno", getSysDate().substring(2) + "98");
    // 序號
    lsSql = "select nvl(max(recno)+1,'1') as recno " + "from crd_emap_tmp "
        + "where batchno = :ls_sysDate||'98'";
  ...
  public void saveFunc() throws Exception {
    Crdm0054Func func = new Crdm0054Func(wp);
...
//      wp.colSet("btnDelete_disable", "disabled");
  ...
  public void dddwSelect() {
    try {
    	if (eqIgno(wp.respHtml, "crdm0054_detl") && eqIgno(wp.buttonCode, "AJAX") == false) {
    		  // 團體代號
    		  String table = "ptr_group_code a, PTR_PROD_TYPE b, PTR_ACCT_TYPE c";
    		  String where = "where a.GROUP_CODE  = b.GROUP_CODE  AND b.ACCT_TYPE = c.ACCT_TYPE AND c.CARD_INDICATOR = '1' ";
