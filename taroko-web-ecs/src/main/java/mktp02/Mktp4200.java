...
* ---------  --------  ---------- ------------------------------------------ *
* 107-01-08  V1.00.01  yash       program initial                            *
...
public class Mktp4200 extends BaseProc {
  ...
  private boolean getWhereStr() throws Exception {
    ...
    wp.whereStr = " where 1=1  and item_no in ('01','02','03','04','05') ";
    ...
    if (keyType.equals("2")) {
      if (empty(wp.itemStr("ex_card_type")) == false) {
        wp.whereStr += " and  key_data >= :ex_card_type ";
  ...
  public void queryRead() throws Exception {
    ...
    wp.selectSQL = "hex(rowid) as rowid,mod_seqno ," + "item_no, "
        + "(select group_code||'_'||group_name from ptr_group_code where group_code= decode(key_type,'1', key_data,'5',substrb(key_data,1,4), '')) as db_group_code,"
        + "(select card_type||'_'||name from ptr_card_type where card_type= decode(key_type,'2', key_data,'5',substrb(key_data,5,2), '')) as db_card_type,"
        + "input_type, " + "key_data, " + "key_type ," + "cost_month, " + "bonu_cash_flag, "
        + "cost_amt, "
        + "decode(substrb(item_ename_incl,1,1),'Y','BL',' ')||' '||decode(substrb(item_ename_incl,2,1),'Y','IT',' ')||' '||decode(substrb(item_ename_incl,3,1),'Y','CA',' ')||' '||decode(substrb(item_ename_incl,4,1),'Y','AO',' ')||' '||decode(substrb(item_ename_incl,5,1),'Y','OT',' ') as item_enamei, "
        + "decode(substrb(item_ename_excl,1,1),'Y','BL',' ')||' '||decode(substrb(item_ename_excl,2,1),'Y','IT',' ')||' '||decode(substrb(item_ename_excl,3,1),'Y','CA',' ')||' '||decode(substrb(item_ename_excl,4,1),'Y','AO',' ')||' '||decode(substrb(item_ename_excl,5,1),'Y','OT',' ') as item_enamee,"
        + "item_ename_incl," + "item_ename_excl," + "purch_mm, " + "service_amt, " + "cost_month2, "
        + "exist_cost_months, " + "crt_user, " + "crt_date, " + "mod_time, " + "mod_seqno";
  ...
  public void dataProcess() throws Exception {
    ...
    String[] aaBonuCashFlag = wp.itemBuff("bonu_cash_flag");
    String[] aaCostAmt = wp.itemBuff("cost_amt");
    String[] aaItemEnameIncl = wp.itemBuff("item_ename_incl");
    String[] aaItemEnameExcl = wp.itemBuff("item_ename_excl");
    ...
    for (int ii = 0; ii < opt.length; ii++) {
      ...
      String ldsSql2 =
          " delete  mkt_contri_parm " + " where  item_no=:item_no " + "  and   key_data=:key_data"
      ...
      setString("cost_month", aaCostMonth[rr]);
      sqlExec(ldsSql2);



      String insSql = " insert into mkt_contri_parm ( " + "   item_no " + " , key_data"
          + " , key_type" + " , cost_month" + " , cost_month2" + " , bonu_cash_flag" + " , cost_amt"
          + " , item_ename_incl" + " , item_ename_excl" + " , purch_mm" + " , service_amt"
          + " , crt_user" + " , crt_date" + " , mod_pgm" + " , mod_seqno" + " ) values ("
          + "  ?,?,?,?,?,?,?,?,?,?,?,?" + ",to_char(sysdate,'yyyymmdd'),?,1)";
      Object[] param = new Object[] {aaItemNo[rr], aaKeyData[rr], aaKeyType[rr], aaCostMonth[rr],
          aaCostMonth2[rr], aaBonuCashFlag[rr], aaCostAmt[rr], aaItemEnameIncl[rr],
          aaItemEnameExcl[rr], aaPurchMm[rr], aaServiceAmt[rr], wp.loginUser,
          wp.itemStr("mod_pgm")};
      ...
      if (sqlRowNum <= 0) {
        wp.colSet(rr, "ok_flag", "!");
        alertErr("ERROR: delete from mkt_contri_parm_t ");
        sqlCommit(0);
        return;
      } else {
