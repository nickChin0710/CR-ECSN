...
* 110-01-19  V1.00.03  JustinWu    change a variable name
******************************************************************************/
package ofcapp;
/** 個資查詢記錄
 * 2019-0807   JH    modify
...
public class QueryLog extends taroko.base.BaseSQL {
  ...
  public boolean authQuery(String asKey) {
    ...
    try {
      ...
      // -user:不受管制
      sql1 =
          "select count(*) as xx_cnt from col_qry_data_auth where 1=1 and user_id = ?";
      double llCnt = getNumber(wp.getConn(), sql1, wp.loginUser);
      if (llCnt <= 0) {
        return true;
      }
    } catch (Exception ex) {
