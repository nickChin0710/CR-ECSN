...
/**
 * ecsfunc: 公用程式 2019-0903 JH get_busidate(xx) 2019-0610: JH p_seqno >>acno_p_seqno 2018-0831: JH
 * get_Curr_rate(xx) 2018-0815: JH get_user_deptName() 2017-12xx: get_busi_date 2017-11xx:
 * check_CorpNo
 * */

public class EcsComm extends busi.FuncBase {
  ...
  public boolean checkBankNo(String aAcctNo) {
    ...
    int num = 0, liSum = 0, liCk[] = {4, 3, 2, 8, 7, 6, 5, 4, 3, 2};
    ...
    for (int ii = 0; ii < 10; ii++) {
      String colStr = commString.mid(lsAcno, ii, 1);
      liSum += commString.strToInt(colStr) * liCk[ii];
    }
    int liMod = liSum % 11;
  ...
  public String getBusiDate(int aiDay) {
    // --
    String lsWkDate = getBusiDate();
    if (aiDay == 0)
      return lsWkDate;

    taroko.base.CommDate commDate = new CommDate();
    String sql1 = " select count(*) as db_cnt from ptr_holiday where holiday = ? ";
    if (aiDay < 0) {
      // -前N天營業日-
      lsWkDate = commDate.dateAdd(lsWkDate, 0, 0, -1);
      for (int ii = 0; ii < 999; ii++) {
        sqlSelect(sql1, new Object[] {lsWkDate});
        if (sqlRowNum > 0 && colInt("db_cnt") == 0) {
          aiDay++;
        }
        if (aiDay == 0)
          break;
  ...
  public String getPayKey(String aAcctType, String aPSeqno) {
    ...
    for (int ii = 0; ii < 9; ii++) {
      int colNum = this.strToInt(lsPSeqno.substring(ii, ii + 1));
      liTmp = (colNum + prefixn[ii]) % 10;
      if (ii == 4) // char-5
        liInt = liInt - liTmp;
      else
        liInt = liInt + liTmp;
