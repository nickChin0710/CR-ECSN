 ...
 * 110-01-07  V1.00.16  tanwei        修改意義不明確變量                                                                          *
 * 110-09-22  V1.00.17  Justin   use old version itext
 * */
...
public class TarokoPDF {
  ...
  HashMap<String, String> suxHash = new HashMap<String, String>();
  ...
  public void procesPDFreport(TarokoCommon wr) throws Exception {
    this.wp = wr;
    if (wpIndx >= 0 && wp.listCount[wpIndx] <= 0) {
      // wp.ddd("列印筆數為 0");
    ...
    try {
      ...
      if (wp.fileMode.equals("Y")) {
        ...
        // "/work/"+wp.exportFile);
        pdfFile = new FileOutputStream(SecurityUtil.verifyPath(TarokoParm.getInstance().getWorkDir() + wp.exportFile));
...
//      firstFlag = "Y"; // Justin
    ...
    while (rowIterator.hasNext()) {
      ...
      for (int j = 0; j < row.getLastCellNum(); j++) {
        ...
        if (checkSumLine) {
          ...
          int pnt2 = cellData.toUpperCase().indexOf("{");
          if (pnt != -1) {
            sumField[line][j] = cellData.substring(pnt + 1, cellData.length() - 1).trim();
          } else if (pnt2 != -1 && cellData.length() > 4) {
            sumField[line][j] = cellData.substring(1, cellData.length() - 1).trim();
            suxHash.put(sumField[line][j], "Y");
          }
        }
  ...
  private void processExcelSheet() throws Exception {
    ...
    inExcelFile = new FileInputStream(SecurityUtil.verifyPath(TarokoParm.getInstance().getDataRoot()  + "/ExcelTemplate/" + excelTemplate));
    // //2020-01-22 JustinWu: read the template in war
    // String dataPath = wp.rootDir;
    // if(wp.rootDir.substring(1, 2).equals(":")) {
    // dataPath += wp.dataRoot.substring(3);
    // }else {
    // dataPath += wp.dataRoot;
    // }
    // inExcelFile = new FileInputStream(dataPath + "/ExcelTemplate/"+excelTemplate);
    // 2020-02-21 JustinWu
//    inExcelFile =
//        this.getClass().getResourceAsStream(
//            "../../../../EcsWeb/data/ExcelTemplate/" + excelTemplate);
    ...
    wp.setValue("ECS_BANK_NAME", "合作金庫銀行", 0);
  ...
  void spanControl() throws Exception {
    ...
    // 取得 ROW-SPAN ,COLUMN-SPAN 區塊個數
    int regionNumber = sheet.getNumMergedRegions();

    for (int i = 0; i < regionNumber; i++) {

      // 取得 ROW-SPAN ,COLUMN-SPAN 各區塊資訊
      ...
      // 紀錄 COLUMN-SPAN 欄位
      int colNum = spanColLast - spanColFirst;
      if (colNum > 0) {
        colHash.put((spanRowFirst + "#" + spanColFirst), (colNum + 1));
      }

      // 紀錄 ROW-SPAN 欄位
      int rowNum = spanRowLast - spanRowFirst;
      if (rowNum > 0) {
        rowHash.put((spanRowFirst + "#" + spanColFirst), (rowNum + 1));
      }
  ...
  void setCellValue(int pnt, int i, int j) throws Exception {
    ...
    //20230925 新增控制##READ_紅字加粗體  Ryan
    Font fon = new Font(bFont, fontSize[i][j], fontBold[i][j]);
    if(cellData.indexOf("##RED_") >= 0) {
    	fon = new Font(bFont, fontSize[i][j], Font.BOLD);
    	cellData = cellData.replace("##RED_", "");
    	fon.setColor(255,0,0);
    }
    if(cellData.indexOf("##BOLD_") >= 0) {
    	fon = new Font(bFont, fontSize[i][j], Font.BOLD);
    	cellData = cellData.replace("##BOLD_", "");
    }
	...
	if (imageField) {
      ...
      barCode39 = false;
		BaseFont barfont = BaseFont.createFont(dataRoot + "/font/V100016_39code.TTF", BaseFont.IDENTITY_H,
  ...
  // 處理 輸出資料顯示轉換
  String cvtFunction(String cvtField, int int1) throws Exception {
    ...
    String cvtData = wp.getValue(cvtField, int1);
...
//          pagePrevData = compData[i]; // Justin
  ...
  void processSubData(int i) throws Exception {
    ...
    for (int j = 0; j < browseColumn; j++) {
      ...
      String checkSUM = (String) suxHash.get(sumField[i][j]);
      int pnt2 = sumField[i][j].indexOf(".");

      if (checkSUM != null) {
        wp.setValue(sumField[i][j], "" + wp.getValue(sumField[i][j]), i);
      } else if (pnt2 != -1) {
