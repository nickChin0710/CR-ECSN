...
public class IchF019 extends AccessDAO {
    private String progname = "餘額返還通知檔(B09B)媒體處理  109/12/16 V1.01.01";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF019 [notify_date [force_flag]]", "");
            }
...
// 1. xRQx/xRPx,xxxQ/xxxR  ex. BRQA, BRPA      2. FC, FB
    ...
    int updateIchCard() throws Exception
    {
        ...
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_card not found ", hRetdIchCardNo, comcr.hCallBatchSeqno);
        }
...
//      updateSQL += " proc_flag     = 'N',";
        ...
        //setString(6, h_tnlg_file_name);
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_b09b_bal not found!", hRetdIchCardNo
    ...
    class Buf1 {
        String type;
        String ichCardNo;
        String balRsn;/*餘額返還原因*/
        String balance;/*卡片餘額*/
        String rtnAmt;/*愛金卡退款金額*/
        String rtnDate;/*餘額返還通知日*/
        String handlingFee;/*手續費*/
        String procDate;/*愛金卡處理日期*/
        String a3hrAmt;/*掛失後3小時的卡片餘額*/
        String rtnCode;
        String fillerEnd;
    }
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl1.balRsn  = comc.subMS950String(bytes,  17,  2);
        dtl1.balance  = comc.subMS950String(bytes,  19,  8);
        dtl1.rtnAmt  = comc.subMS950String(bytes,  27,  8);
        dtl1.rtnDate  = comc.subMS950String(bytes,  35,  8);
        dtl1.handlingFee  = comc.subMS950String(bytes,  43,  8);
        dtl1.procDate  = comc.subMS950String(bytes,  51,  8);
        dtl1.a3hrAmt  = comc.subMS950String(bytes,  59,  8);
        dtl1.rtnCode   = comc.subMS950String(bytes, 67,  1);
        dtl1.fillerEnd = comc.subMS950String(bytes, 68,  2);
    }
/***********************************************************************/
