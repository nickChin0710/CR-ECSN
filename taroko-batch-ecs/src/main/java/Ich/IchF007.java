...
public class IchF007 extends AccessDAO {
    private String progname = "卡號資料檔(B07B)產生  109/12/15 V1.00.01";
    ...
    String tmpstr1 = "";
    String hTfinFileIden = "B07B";


    String hIcdrIchCardNo = "";
    String hIcdrEffcDate   = "";
    String hIcdrIssueDate  = "";
    String hIcdrIsamNum    = "";
    String hIcdrOnlineAdd  = "";
    String hIcdrOfflineAdd = "";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF007 [notify_date] [force_flag (Y/N)]", "");
            }
    ...
    /***********************************************************************/
    void updateIchB07bCardA() throws Exception {
  ...
  int recCnt = selectTable();*/
    ...
    /***********************************************************************/
    void selectIchB07bCard() throws Exception
    {
        sqlCmd = "select ";
        ...
        sqlCmd += "a.isam_num,";
        sqlCmd += "a.online_add,";
        sqlCmd += "a.offline_add,";
        sqlCmd += "a.rowid as rowid1 ";
        sqlCmd += "from  ich_b07b_card a ";
        sqlCmd += "where 1=1 ";
        ...
        while (fetchTable()) {
            ...
            hIcdrEffcDate   = getValue("effc_date");
            hIcdrIssueDate  = getValue("issue_date");
            hIcdrIsamNum    = getValue("isam_num");
            hIcdrOnlineAdd  = getValue("online_add");
            hIcdrOfflineAdd = getValue("offline_add");
    ...
    void writeRtn() throws Exception
    {
        ...
        tmpstr = String.format("%-8.8s"  , hIcdrEffcDate);
        detailSt.effcDate      = tmpstr;
        tmpstr = String.format("%-8.8s"  , hIcdrIssueDate);
        detailSt.issueDate      = tmpstr;
        tmpstr = String.format("%-14.14s"  , hIcdrIsamNum);
        detailSt.isamNum      = tmpstr;
        tmpstr = String.format("%-1.1s"  , hIcdrOnlineAdd);
        detailSt.onlineAdd      = tmpstr;
        tmpstr = String.format("%-1.1s"  , hIcdrOfflineAdd);
        detailSt.offlineAdd      = tmpstr;
    ...
    class Buf1 {
        ...
        String allText() throws UnsupportedEncodingException {
            ...
            rtn += fixLeft(effcDate, 8);
            rtn += fixLeft(issueDate, 8);
            rtn += fixLeft(isamNum, 14);
            rtn += fixLeft(onlineAdd, 1);
            rtn += fixLeft(offlineAdd, 1);
