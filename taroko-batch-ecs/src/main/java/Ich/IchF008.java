...
public class IchF008 extends AccessDAO {
    private String progname = "代行授權設定檔(B08B)產生  109/12/15 V1.00.01";
    ...
    String tmpstr1 = "";
    String hTfinFileIden = "B08B";


    String hIcdrLimitAdd = "";
    String hIcdrLimitCntAuth   = "";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF008 [notify_date] [force_flag (Y/N)]", "");
            }
            ...
            if(totCnt > 0)
            {
                ...
                tmpstr1 = String.format("H%4.4s%2.2s%4.4s%3.3s%1.1s%-40.40s\r\n",hTfinFileIden
                        ,"01", "0001", comc.ICH_BANK_ID3, "A", hHash);
    ...
    /***********************************************************************/
    void updateIchB08bInstdA() throws Exception {
        daoTable  = "ich_b08b_instd";
    ...
    /***********************************************************************/
    void selectIchB08bInstd() throws Exception
    {
        sqlCmd = "select ";
        sqlCmd += "a.amt01,";
        sqlCmd += "a.cnt01,";
        sqlCmd += "a.rowid as rowid1 ";
        sqlCmd += "from  ich_00_parm a ";
        sqlCmd += "where parm_type = 'ICHM0030' ";
        //sqlCmd += "  and (a.proc_flag = 'N' or decode(a.proc_flag,'', 'N',a.proc_flag) = 'N') ";
        openCursor();
        while (fetchTable()) {
            hIcdrLimitAdd = Integer.toString(getValueInt("amt01"));
            hIcdrLimitCntAuth = getValue("cnt01");
    ...
    void writeRtn() throws Exception
    {
        ...
        tmpstr = String.format("%08d", comc.str2int(hIcdrLimitAdd));
        detailSt.limitAdd = tmpstr;
        tmpstr = String.format("%08d", comc.str2int(hIcdrLimitCntAuth));
        detailSt.limitCntAuth = tmpstr;
        tmpstr = String.format("%-24.24s"  , " ");
    ...
    /***********************************************************************/
    void insertIchB08bInstd() throws Exception
    {

        setValue("limit_add" ,   hIcdrLimitAdd );
        setValue("limit_cnt_auth" ,   hIcdrLimitCntAuth );
    ...
    /***********************************************************************/
    void updateIchB08bInstd() throws Exception
    {
        daoTable   = "ich_b08b_instd";
    ...
    class Buf1 {
        ...
        String allText() throws UnsupportedEncodingException {
            String rtn = "";
            rtn += fixLeft(type, 1);
            rtn += fixLeft(limitAdd, 8);
            rtn += fixLeft(limitCntAuth, 8);
            rtn += fixLeft(space    , 24);
