...
public class IchT013 extends AccessDAO {
    private String progname = "愛金卡檔案(AnnB)接收後前一天的檔案處理(I) 109/12/16 V1.01.01";
    ...
    public int mainProcess(String[] args) {
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchT013 [[notify_date][force_flag]] [force_flag]", "");
            }
            // 固定要做的
    ...
    void fileOpen() throws Exception {
        ...
        while (true) {
            ...
            if (!str600.substring(0, 2).equals("DB"))
                continue;

            hOrgiFileName = str600.substring(1, 23);
            hTnlgFileName = str600.substring(1, 23);
            hTfinFileIden = str600.substring(19, 23).trim();
            if (hTfinFileIden.length() == 0)
                continue;
            totCnt++;
            /* BRPA_017_20190226_B96B */
            if (hTnlgFileName.substring(1, 3).equals("RP")) {
                hTnlgFileName = hTnlgFileName.substring(0, 1) + "RQ" + hTnlgFileName.substring(3, 3 + 6)
                        + hPreNotifyDate + hTnlgFileName.substring(17);
            } else if (hTnlgFileName.substring(0, 2).equals("FB")) {
                /* FB_901719300_20190226 */
                hTnlgFileName = "FC" + hTnlgFileName.substring(2, 2 + 11) + hPreNotifyDate;
            }
            ...
            if (debug == 1)
                showLogMessage("I", "", " Read file=[" + totCnt + "]" + hTfinFileIden + "," + str600);
    ...
    int selectIchNotifyLogC() throws Exception {
        ...
        sqlCmd += "   and decode(ftp_receive_date,'', 'N',ftp_receive_date) = 'N'  ";
        sqlCmd += " fetch first 1 rows only ";
        setString(1, hTnlgFileName);
        int recordCnt = selectTable();
        if (debug == 1)
            showLogMessage("I", "", "  get log=[" + recordCnt + "]" + hTnlgFileName);
        if (notFound.equals("Y")) {
            return (recordCnt);
        }
        if (recordCnt > 0) {
    ...
    /***********************************************************************/
    void copyBackup(String moveFile) throws Exception {
    ...
    void updateIchNotifyLog() throws Exception {
        if (debug == 1)
            showLogMessage("I", "", "  upd log=[" + hTnlgNotifyDate + "]");
