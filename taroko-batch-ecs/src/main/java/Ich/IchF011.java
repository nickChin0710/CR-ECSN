 ...
 *  ---------  --------- ----------- ----------------------------------------  *
 *  107/12/26  V1.01.00  Lai         program initial                           *
...
public class IchF011 extends AccessDAO {
    private String progname = "卡款檔(B01B)媒體處理  109/12/15 V1.00.01";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF011 [notify_date [force_flag]]", "");
            }
            ...
            selectIchNotifyLog();  //取前一天的送檔記錄來處理
...
// 1. xRQx/xRPx,xxxQ/xxxR  ex. BRQA, BRPA      2. FC, FB
    ...
    void updateIchB01bParm() throws Exception {
        ...
        setString(5, hRetdCardCode );
        setString(6, hTnlgFileName);
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_b01b_parm not found!", hRetdCardCode
    ...
    class Buf1 {
        ...
        String allText() throws UnsupportedEncodingException {
            String rtn = "";
            rtn += fixRight(fileDate, 8);
            rtn += fixRight(fileTime, 6);
            rtn += fixRight(fillerEnd, 2);
            return rtn;
        }
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl1.cardCode  = comc.subMS950String(bytes,  1,  9);
        dtl1.txKind    = comc.subMS950String(bytes, 10,  1);
        dtl1.fileDate  = comc.subMS950String(bytes, 11,  8);
        dtl1.fileTime  = comc.subMS950String(bytes, 19,  6);
        dtl1.filler1   = comc.subMS950String(bytes, 25, 36);
