...
public class IchF013 extends AccessDAO {
    private String progname = "金融機構鎖卡名單(B03B)媒體處理  109/12/15 V1.00.01";
    ...
    ArrayList<String> tmpIchCardNo = new ArrayList<String>();
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF013 [notify_date [force_flag]]", "");
            }
...
// 1. xRQx/xRPx,xxxQ/xxxR  ex. BRQA, BRPA      2. FC, FB
        ...
        while (true) {
            ...
            tmpIchCardNo.add(hRetdIchCardNo);
        ...
        updateIchCard1(); /* update ich_card_no not in (當日黑名單ＯＫ回覆檔) */
    ...
    void updateIchCard() throws Exception {
        ...
        updateSQL += " blacklt_s_date = case when blacklt_s_date = '' then to_char(sysdate, 'yyyymmdd') "
                + "                      else (case when blacklt_e_date = '' then blacklt_s_date "
                + "                      else (case when blacklt_e_date < to_char(sysdate, 'yyyymmdd') then to_char(sysdate, 'yyyymmdd') "
    ...
    /***********************************************************************/
    void updateIchCard1() throws Exception {

        daoTable   = "ich_card";
        updateSQL  = " BLACKLT_E_DATE = to_char(sysdate, 'yyyymmdd'), ";
        updateSQL += " mod_pgm = ?, ";
        updateSQL += " mod_time = sysdate ";
        whereStr   = "where BLACKLT_FLAG='Y'  ";
        whereStr  += "  and blacklt_e_date = '' ";
        whereStr  += "  and ich_card_no not in ( ";
        for(int i = 0; i < tmpIchCardNo.size(); i++ ) {
            whereStr  += " ?,";
        }
        whereStr = whereStr.substring(0, whereStr.length() - 1);
        whereStr  += " ) ";
        showLogMessage("I", "", "whereStr = ["+ whereStr + "]" );

        int index = 1;
        setString(index++, javaProgram );
        for(int i = 0; i < tmpIchCardNo.size(); i++ ) {
            setString(index++, tmpIchCardNo.get(i));
        }
    ...
    /***********************************************************************/
    void updateIchB03bLock() throws Exception {
...
//      updateSQL += " proc_flag     = 'N',";
        ...
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_" + daoTable + " not found!", hRetdIchCardNo
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl1.txKind    = comc.subMS950String(bytes, 17,  1);
        dtl1.fileDate  = comc.subMS950String(bytes, 18,  8);
        dtl1.fileTime  = comc.subMS950String(bytes, 26,  6);
        dtl1.filler1   = comc.subMS950String(bytes, 32, 29);
