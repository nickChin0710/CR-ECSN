...
/*ICAH 交易別/會計科目轉換作業*/
public class IchE022 extends AccessDAO {
    private String progname = "ICAH 交易別/會計科目轉換作業 112/08/14 V1.00.04";
    ...
    String prgmId = "IchE022";
    String prgmName = "ICAH 交易別/會計科目轉換作業";
    String rptName = "ICH_E022";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1) {
                comc.errExit("Usage : IchE022 batch_seq", "");
            }
    ...
    void printFirstRtn() throws Exception {
        ...
        szTmp = String.format("%22s", "ICAH 請款媒體入檔合計表");
        ...
        sqlCmd += "UNION select ";
        sqlCmd += "count(*)     p_count,";
        sqlCmd += "sum(balance_amt) p_sum,";
        sqlCmd += "batch_no ";
        sqlCmd += " from ich_a09b_bal ";
        sqlCmd += "where decode(post_flag,'','N',post_flag) <> 'Y' ";
        sqlCmd += "group by batch_no ";
        recordCnt = selectTable();
    ...
    void selectIchA07bAdd() throws Exception {
        ...
        sqlCmd = "select '1' h_sw_type , ";
        sqlCmd += "ich_card_no,";
        sqlCmd += "txn_date,";
        sqlCmd += "txn_time,";
        sqlCmd += "txn_amt,";
        sqlCmd += "batch_no,"; //no column
        sqlCmd += "seq_no,";   //no column
        sqlCmd += "card_no,";
        sqlCmd += "bill_type,";
        sqlCmd += "txn_code,";
        sqlCmd += "mcht_no,";
        ...
        sqlCmd += "bill_desc,";
        sqlCmd += "resp_code_ich,"; /* 剔退碼 */
        sqlCmd += "rowid  as rowid1 ";
        sqlCmd += "from ich_a07b_add ";
        sqlCmd += "where (decode(post_flag,'','N',post_flag) = 'N' or post_flag = '') ";
		sqlCmd += "  and txn_amt <> 0 ";
        sqlCmd += "UNION       ";
        sqlCmd += "select '2' h_sw_type , ";
        sqlCmd += "ich_card_no,";
        sqlCmd += "'00000000'  as txn_date,";
        sqlCmd += "txn_time,";
        sqlCmd += "balance_amt as txn_amt,";
        sqlCmd += "batch_no,";
        sqlCmd += "seq_no,"; 
        sqlCmd += "card_no,";
        sqlCmd += "bill_type,";
        ...
        sqlCmd += "mcht_chi_name,";
        sqlCmd += "bill_desc,";
        sqlCmd += "'000' as resp_code_ich,";
        sqlCmd += "rowid  as rowid1 ";
        sqlCmd += "from ich_a09b_bal ";
        sqlCmd += "where (decode(post_flag,'','N',post_flag) = 'N' or post_flag = '') ";
		sqlCmd += "  and balance_amt <> 0 ";
        sqlCmd += "UNION       ";
        sqlCmd += "select '3' h_sw_type , ";
        sqlCmd += "ich_card_no,";
        sqlCmd += "txn_date,";
        sqlCmd += "txn_time,";
        sqlCmd += "txn_amt ,";
        ...
        sqlCmd += "'000' as resp_code_ich,";
        sqlCmd += "rowid  as rowid1 ";
        sqlCmd += "from ich_a04b_exception ";
        sqlCmd += "where (decode(post_flag,'','N',post_flag) = 'N' or post_flag = '') ";
        sqlCmd += "  and exception_cd = 'E011' ";
		sqlCmd += "  and txn_amt <> 0 ";
		sqlCmd += "  and txn_cd in ('21', '22', '23', '24') ";
        sqlCmd += "UNION       ";
        sqlCmd += "select '4' h_sw_type , ";
        sqlCmd += "ich_card_no,";
        sqlCmd += "'00000000'  as txn_date,";
        sqlCmd += "sys_time    as txn_time,";
        sqlCmd += "adjust_amt  as txn_amt ,";
        sqlCmd += "batch_no,";
        sqlCmd += "seq_no,"; 
        sqlCmd += "card_no,";
        sqlCmd += "bill_type,";
        ...
        sqlCmd += "'000' as resp_code_ich,";
        sqlCmd += "rowid  as rowid1 ";
        sqlCmd += "from ich_a10b_adjust ";
        sqlCmd += "where (decode(post_flag,'','N',post_flag) = 'N' or post_flag = '') ";
		sqlCmd += "  and adjust_amt <> 0 ";
        int recordCnta = selectTable();

        for (int i = 0; i < recordCnta; i++) {
            hSwType            = getValue("h_sw_type", i);
            hIcgeIchCardNo   = getValue("ich_card_no", i);
			...
			hIcgeRespCodeIch   = getValue("resp_code_ich", i);
            hIcgeRowid         = getValue("rowid1", i);
            ...
            hCurpFilmNo = String.format("%-8.8s%-8.8s%-6.6s", hIcgeIchCardNo, hIcgeTxnDate, hIcgeTxnTime);
            hCurpFilmNo = String.format("%-16.16s%03d", hIcgeIchCardNo, hIcgeSeqNo);
            ...
            //若沒有帶入交易日的交易,以營業日寫入交易日
            if ("00000000".equals(hCurpPurchaseDate)) {
            	hCurpPurchaseDate = hBusinessDate;
            }

            chkCrdCard();
            if (selectPtrBintable() == false)
			{
			    showLogMessage("I", "", "  CHK ptr_bintable error =[" +hCurpCardNo + "]");
                break;
			}
			...
			if(hIcgeRespCodeIch.equals("000")) /* 無剔退才入當日帳單暫存檔 */
               insertBilCurpost();
		   
            daoTable   = "ich_a07b_add ";
            if(hSwType.equals("2"))
               daoTable   = "ich_a09b_bal ";
            else if(hSwType.equals("3"))
                    daoTable   = "ich_a04b_exception ";
            else if(hSwType.equals("4"))
            ...
            setRowId(3, hIcgeRowid);
            updateTable();
            if (notFound.equals("Y")) {
                stderr = "update_ich_axxb_add not found!";
                comcr.errRtn(stderr, "", hCallBatchSeqno);
            }
    ...
    void chkCrdCard() throws Exception {
        ...
        sqlCmd += " from ptr_bintable b, crd_idno c, crd_card a ";
        sqlCmd += "where a.card_no     = ? ";
        sqlCmd += "  and b.bin_no      = substr(a.card_no,1,6) ";
    ...
    void chkPtrActcode() throws Exception {
        ...
        if (recordCnt > 0) {
            ...
            stderr = String.format("select prt_actcode not found");
            comcr.errRtn(stderr, "", hCallBatchSeqno);
        }
    ...
    void insertBilCurpost() throws Exception {
        ...
        /* 不用再檢查一次
        if (hIcgeMchtNo.equals("ICAH8003")) {
            chkRskRtn();
        }
        */
        ...
        setValue("this_close_date", hCurpThisCloseDate);  // empty
		...
		setValue("BIN_TYPE", hCurpCardSw); //card_sw
		setValue("ecs_platform_kind","EI"); 
		setValue("ecs_cus_mcht_no","006EI00001");
