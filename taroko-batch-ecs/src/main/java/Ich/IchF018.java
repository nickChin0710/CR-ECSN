...
public class IchF018 extends AccessDAO {
    private String progname = "代行授權設定檔(B08B)媒體處理  109/12/16 V1.01.01";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF018 [notify_date [force_flag]]", "");
            }
...
// 1. xRQx/xRPx,xxxQ/xxxR  ex. BRQA, BRPA      2. FC, FB
        ...
        while (true) {
            ...
            if(hRtnCode.equals("1"))
                errCnt++;
            showLogMessage("I", "", "h_rtn_code = " + hRtnCode + "["+totCnt+"]");
...
//      updateSQL += " proc_flag     = 'N',";
        ...
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_b08b_instd not found!", ""
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl1.limitAdd  = comc.subMS950String(bytes,  1,  8);
        dtl1.limitCntAuth  = comc.subMS950String(bytes,  9,  8);
        dtl1.fileDate  = comc.subMS950String(bytes, 17,  8);
        dtl1.fileTime  = comc.subMS950String(bytes, 25,  6);
        dtl1.filler1   = comc.subMS950String(bytes, 31, 22);
        dtl1.rtnCode   = comc.subMS950String(bytes, 53,  1);
        dtl1.fillerEnd = comc.subMS950String(bytes, 54,  2);
    }
/***********************************************************************/
