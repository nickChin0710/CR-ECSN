 ...
 *  109/07/01  V1.00.02    Wilson    notfound return (1)                       *
 ...
 *  110/02/04  V1.00.07    Wilson    檔名日期改營業日減一日                                                                          *
...
public class IchF033 extends AccessDAO {
  private final String progname = "鎖卡資料拋轉(A03B)處理  111/02/14  V1.00.08";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      // comc.err_exit("Error!! Someone is running this program now!!!", "Please wait a moment to
      // run again!!");
      // }
...
//    hTempNotifyDate = hTempNotifyDate.length() == 0 ? sysDate : hTempNotifyDate;
    hBusiBusinessDate = "";
    hAdd2SystemDate = "";
    sqlCmd = "select to_char(to_date(business_date,'yyyymmdd')-1 days,'yyyymmdd') business_date ";
//    sqlCmd += "to_char(to_date(cast(? as varchar(8)), 'yyyymmdd') + 1 ,'yyyymmdd') h_next_notify_date,";
...
//      hNextNotifyDate = getValue("h_next_notify_date");
...
//      hAdd2SystemDate = getValue("h_add2_system_date");
  ...
  void backupRtn() throws Exception {
    ...
    tmpstr2 = String.format("%s/media/ich/backup/%s/%s", comc.getECSHOME(), hTempNotifyDate,
        hTnlgFileName);
    comc.fileRename2(String.format("%s/%s", root, hTnlgFileName), tmpstr2);

    // tmpstr1 = String.format("ARQB_%3.3s_%8.8s_A03B", comc.ICH_BANK_ID3, h_temp_notify_date);
    // tmpstr2 = String.format("%s/media/ich/backup/%s/%s", comc.GetECSHOME(), h_temp_notify_date,
    // tmpstr1);
