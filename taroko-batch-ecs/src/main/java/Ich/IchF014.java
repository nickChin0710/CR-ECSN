...
public class IchF014 extends AccessDAO {
    private String progname = "遞送特殊處理卡號(B04B)媒體處理  109/12/16 V1.01.01";
    ...
    public int mainProcess(String[] args)
    {
        try
        {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF014 [notify_date [force_flag]]", "");
            }
...
// 1. xRQx/xRPx,xxxQ/xxxR  ex. BRQA, BRPA      2. FC, FB
    ...
    void updateIchCard() throws Exception {
        ...
        setString(1, dtl1.txKind.trim());
        setString(2, javaProgram);
        setString(3, hRetdIchCardNo );
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_card not found!", hRetdIchCardNo
...
//      updateSQL += " proc_flag     = 'N',";
        ...
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_ich_b04b_special not found!", hRetdIchCardNo
                    ,comcr.hCallBatchSeqno);
        }
    ...
    /***********************************************************************/
    void insertIchRefuseLog() throws Exception {

        sqlCmd = "select card_no  from ich_card ";
        sqlCmd += " where ich_card_no   = ?   ";
        setString(1, hRetdIchCardNo );
        selectTable();

        setValue("card_no", getValue("card_no"));
        setValue("ich_card_no", hRetdIchCardNo);
        setValue("rpt_resp_code", hRtnCode);
        setValue("rpt_resp_date", sysDate);
        setValue("rpt_resp_time", sysTime);
        setValue("crt_date", sysDate);
        setValue("crt_time", sysTime);
        setValue("txn_code", dtl1.txKind);

        sqlCmd = "select sys_date, sys_time  from ich_b04b_special ";
        sqlCmd += " where ich_card_no   = ?  and file_name   = ? ";
        setString(1, hRetdIchCardNo );
        setString(2, hTnlgFileName);
        selectTable();
        setValue("send_date", getValue("sys_date"));
        setValue("send_time", getValue("sys_time"));
