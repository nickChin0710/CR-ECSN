...
public class IchF027 extends AccessDAO {
    private String progname = "遞送自動加值結果(A07B)接收處理  109/12/16 V1.00.01";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IchF027 [[notify_date][fo1yy_flag]] [force_flag]", "");
            }
    ...
    int selectIchNotifyLogA() throws Exception {
        ...
        int recordCnt = selectTable();
        if (notFound.equals("Y")) {
            comcr.errRtn(String.format("未有[%s]檔案記錄 , 請通知相關人員處理(error)"
        ...
        if(debug==1)
            showLogMessage("I","","File="+hTnlgFileName+","+hTnlgPerformFlag+","+hTnlgProcFlag);
