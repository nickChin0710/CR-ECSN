...
*  -------- -------------------  ------------------------------------------- *
* 109/12/19 V1.00.14  Allen Ho   cyc_C260                                    *
* 110/11/09 V1.00.15  Lai        M:9017                                      *
* 110/11/19 V1.00.16  Lai        M:9017-1  fund_name                         *
* 111/11/10 V1.00.04  Yang Bo    sync code from mega                         *
******************************************************************************/
package Cyc;
...
public class CycC260 extends AccessDAO
{
 private final String PROGNAME = "現金回饋-更新雙幣基金統計處理程式  111/11/10 V1.00.04";
...
// ************************************************************************
 public int mainProcess(String[] args) {
  try
  {
   ...
   loadPtrPayment();
   showLogMessage("I","","=========================================");
   showLogMessage("I","","處裡 cyc_dc_fund_dtl 資料");
   selectCycDcFundDtl();
   showLogMessage("I","","處理 ["+totalCnt+"] 筆");
   showLogMessage("I","","=========================================");
...
// ************************************************************************
 void selectCycDcFundDtl() throws Exception
 {
  ...
  daoTable  = "cyc_dc_fund_dtl a,cyc_acmm_"+ getValue("wday.stmt_cycle") +" b "; 
  whereStr  = "where a.p_seqno = b.p_seqno "
            + "and  ((a.acct_date > ? "
            + "  and  a.acct_date <= ?)  "
            + " or   (a.acct_date <= ?  "
            + "  and  a.end_tran_amt!=0)) " 
            + "group by a.p_seqno,a.curr_code,substr(a.fund_code,1,4),tran_code,acct_date "
            ;
    ...
    if (getValue("acct_date").compareTo(getValue("wday.last_close_date"))>0)
       {
        if ((getValue("tran_code").equals("1"))||
           ...
           newAddFund = getValueDouble("beg_tran_amt");
        else    
        if ((getValue("tran_code").equals("0"))||
            (getValue("tran_code").equals("3"))||
            (getValue("tran_code").equals("7")))
           adjustFund = getValueDouble("beg_tran_amt");
        else    
        if (getValue("tran_code").equals("4"))
           useFund = getValueDouble("beg_tran_amt");
        else    
        if (getValue("tran_code").equals("6"))
           removeFund = getValueDouble("beg_tran_amt");
       }
    netFund = getValueDouble("end_tran_amt");

if(DEBUG==1)
  showLogMessage("I","","Read FUND=["+getValue("fund_code")+"]"+getValue("tran_code")+","+ netFund +","+getValue("p_seqno"));
...
if(DEBUG==1)
  showLogMessage("I","","   payment=["+cnt1+"]"+getValue("prpt.bill_desc")+","+ fundName);
    ...
    if (updateMktFundHst()!=0)
       {
        ...
        lastMonthFund = 0;
        if (cnt1>0) lastMonthFund = getValueDouble("fund.last_month_fund");
        insertMktFundHst();
       }
   } 
...
// ************************************************************************
 ...
 {
if(DEBUG==1)
  showLogMessage("I","","   insert hst=["+ fundName +"]"+getValue("p_seqno"));
  dateTime();
  setValue("thst.p_seqno"                  , getValue("p_seqno"));
  setValue("thst.acct_month"               , getValue("wday.this_acct_month"));
  setValue("thst.curr_code"                , getValue("curr_code"));
...
//setValue("thst.bill_desc"                , fund_name);
...
// ************************************************************************
 int updateMktFundHst() throws Exception
 {
  ...
  setString(9 , getValue("fund_code"));
