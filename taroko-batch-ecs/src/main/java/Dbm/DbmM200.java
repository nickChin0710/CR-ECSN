...
*  --------  -------------------  ------------------------------------------ *
* 108/09/12  V1.00.09  Allen Ho   dbm_m220                                   *
*  109/11/18  V1.00.10  yanghan       修改了變量名稱和方法名稱                                                                            *
*  109/12/24  V1.00.11  yanghan       修改了變量名稱和方法名稱            *
* 112/03/29  V1.00.12  Zuwei Su  新增[ 一般消費群組 ]參數帳單資料篩選處理                                                                           *
******************************************************************************/
package Dbm;
...
public class DbmM200 extends AccessDAO
{
 private  String progname = "VD紅利-新發卡消費回存扣回處理程式 112/03/29  V1.00.12";
...
// ************************************************************************ 
 void  selectDbbBill() throws Exception
 {
  selectSQL = "a.mcht_no,"
            ...
            + "a.mcht_category,"
            + "a.pos_entry_mode,"
            + "decode(a.group_code,'','0000',group_code),"
            + "a.purchase_date,"
            + "a.card_no,"
            + "a.mcht_no,"
            + "a.ecs_platform_kind,"
  ...
  while( fetchTable() ) 
   { 
	...
	//  showLogMessage("I","","id_p_seqno  : ["+ getValue("id_p_seqno") +"]");
	    ...
	    if (getValueInt("acaj.deduct_amt") <= getValueInt("parm.bp_amt")) continue;
	    ...
	    setValue("bdtl.mod_memo"    , "VD紅利新發卡退貨調整");
