...
public class MktR420 extends com.BaseBatch {
private final String PROGNAME = "聯名機構主機入帳處理-回饋企業員工  2023-0828 V1.00.00";
HH hh=new HH();
//-----------
class HH {
   double rew_amount=0;
...
protected void dataProcess(String[] args) throws Exception {
   ...
   if (args.length > 2) {
      printf("Usage : MktR420 [busi_date]");
      okExit(0);
   }
   ...
   selectMkt_member_reward();

   sqlCommit();
   endProgram();
}
//=======================
void selectMkt_member_reward() throws Exception {
   sqlCmd ="SELECT sum(A.feedback__amt) AS rew_amount " +
            ", B.member_corp_no " +
            " FROM  mkt_member_reward A " +
            " JOIN ptr_group_code B ON A.group_code=B.group_code AND B.member_flag='Y' " +
            " WHERE 1=1 " +
            " AND A.static_month =? " +
            " AND A.feedback_date ='' " +
            " GROUP BY B.member_corp_no"+
            " having sum(A.feedback__amt) >0 "
    ;
   ppp(1, is_rewYYMM);
...
void selectMkt_member() throws Exception {
   ...
   sqlSelect(tiMemb);
   if (sqlNrow <=0) return;
   String ls_acctNo =colSs(CC+"acct_no");
   hh.member_acct_no =ls_acctNo.replaceAll("-","").replaceAll("_","");
}
//================
int tiDrewd=-1;
...
//   ttArewd.aaa("execute_flag"              , hh.execute_flag);
   ...
   if (sqlNrow <=0) {
      sqlerr("insert mkt_in_reward[2] error");
      errExit(1);
   }
