...
*  112/08/07 V1.01.02  Kirin      group_code = 1622,media/mkt                 *
...
*  112/09/02 V1.01.06  Kirin       change introduce_emp_no                    *
...
public class MktRF622 extends AccessDAO {
    final private String PROGNAME = "員工推展「VISA無限金鑽卡」獎勵明細表  112/08/07 V1.01.02";
...
//    String PgmCd = "2023010002";
    ...
    String rptIdR1 = "RCRF622";
    String rptName1 = "員工推展「VISA無限金鑽卡」獎勵明細表";
    ...
    public int mainProcess(String[] args) {
        try {
            ...
            // =====================================
            if (args.length > 3) {
                comc.errExit("Usage : MktRF622 [PROGRAM_CODE] [yyyymmdd] [seq_no] ", "");
            }
            /*
    ...
    void selectCrdEmployee() throws Exception {
        ...
        while (fetchTable()) {
            ...
            if (totCnt % 1000 == 0 || totCnt == 1) {
                showLogMessage("I", "", String.format("MktRF622 Process 1 record=[%d]\n", totCnt));
            }
...
//            100元禮券張數 =  A + B
//            A = 3*全新戶有效卡數 + (全新戶有效卡數/6 取整數)
//            B =  3* (非全新戶有效卡數 /2  取整數)
            allDataInt[brnCnt][3] = 3 * allDataInt[brnCnt][1] + (allDataInt[brnCnt][1] / 6) + 3 * (allDataInt[brnCnt][2] / 2);
            if (DEBUG == 1) {
                showLogMessage("I", "",
                        "   Dtl="
                                + brnCnt
    ...
    void writeHead(int idx) throws Exception {
        ...
        buf = comcr.insertStr(buf, "報表代號: CRF622    科目代號:", 1);
        ...
        buf = "員工ＩＤ   姓  名                全新卡有效卡   非全新卡有效卡   100元禮券張數 簽收人蓋章或簽名";
        writeFile(buf);
...
//        writeFile(buf);
    ...
    void finalTail() throws Exception {
        ...
        buf = " （１） 全新戶有效卡: 推廣1卡全新戶有效卡可獲全聯３００元禮券，累積每增加６卡可再獲全聯";
        writeFile(buf);
        buf = "                      １００元禮券．單卡可獲全聯禮券上限６００元．";
        writeFile(buf);

        buf = " （２） 非全新戶: 每２可獲全聯３００元禮券．";
        writeFile(buf);
