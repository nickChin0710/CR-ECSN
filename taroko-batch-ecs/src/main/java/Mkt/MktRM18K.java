...
/*產生台中第二信用合作社聯名卡企業回饋金明細表*/
public class MktRM18K extends AccessDAO {
    private String progname = "產生台中第二信用合作社聯名卡企業回饋金明細表程式  112/10/19  V1.00.02";
    CommFunction   comm  = new CommFunction();
    CommCrd        comc  = new CommCrd();
    CommCrdRoutine comcr = null;

    String prgmId = "MktRM18K";
    String prgmName = "台中第二信用合作社聯名卡企業回饋金明細表";
    
    String rptNameM18K = "台中第二信用合作社聯名卡企業回饋金明細表";
    String rptIdM18K = "CRM18K";
    int rptSeqM18K = 0;
    int pageCntM18K = 0;
    List<Map<String, Object>> lparM18K = new ArrayList<Map<String, Object>>();
		...
		while (fetchTable(cursorIndex)) {
            ...
            if (keepPromoteDept.equals(getValue("promote_dept")) == false) {
            	if (realCnt == 0) {
            		...
            		printDetailM18K(keepPromoteDept,acctNo);
            	}
    ...
    void printHeaderM18K() throws UnsupportedEncodingException, Exception {
    	pageCntM18K++;
        
        buf = "";
        buf = comcr.insertStr(buf, "分行代號:  3144信用卡部" ,  1);
        buf = comcr.insertStr(buf, ""              + rptNameM18K             , 35);
        buf = comcr.insertStr(buf, "保存年限: 二年"                             ,97);
        lparM18K.add(comcr.putReport(rptIdM18K, rptNameM18K, sysDate, ++rptSeqM18K, "0", buf));
        
        buf = "";
        String strDate = String.format("%3.3s年%2.2s月%2.2s日", hBusDateTwYear,hBusDateMonth, hBusDateDay);
        buf = comcr.insertStr(buf, "報表代號: CRM18K     科目代號:"            ,  1);
        buf = comcr.insertStr(buf, "中華民國 " + strDate                      , 46);
        buf = comcr.insertStr(buf, "頁    次:" + String.format("%4d", pageCntM18K) ,97);
        lparM18K.add(comcr.putReport(rptIdM18K, rptNameM18K, sysDate, ++rptSeqM18K, "0", buf));
        ...
        buf = "";
        buf = comcr.insertStr(buf, "台中二信帳戶             消費總額             0.2% 回饋金       分社代號        分社中文名稱", 1);
        lparM18K.add(comcr.putReport(rptIdM18K, rptNameM18K, sysDate, ++rptSeqM18K, "0", buf));
    ...
    /**
     * @throws Exception *********************************************************************/
    void printFooterM18K() throws Exception {
        ...
        comcr.insertPtrBatchRpt(lparM18K);

    }

    /***********************************************************************/
    void printDetailM18K(String keepPromoteDept,String acctNo) throws Exception {
