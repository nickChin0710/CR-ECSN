...
public class MktFr10 extends AccessDAO {
    private final String PROGNAME = "接收AP4傳送的以房養老名單處理程式 112/07/25 V1.00.00";
...
//    private static final String FILE_NAME = "LAM168.YYYYMMDD.TXT";
    private static final String FILE_NAME = "LAM168.TXT";
    ...
    public int mainProcess(String[] args) {
        try {
            ...
            String fileName = FILE_NAME.replace("YYYYMMDD", commDate.dateAdd(procDate, 0, 0, -1));
    ...
    int openFile(String fileName) {
        ...
        if (rec == -1) {
            showLogMessage("I", "", String.format(" %s 資料內容空檔,連繫ap4業務窗口傳檔", fileName));
            return 7;
        }
    ...
    int readFile(String fileName) throws Exception {
        ...
        while ((lineLength = bufferedReader.readLine()) != null) {
            if (lineLength.length() < 24) {
                continue;
            }
            totCnt++;
            byte[] bytes = lineLength.getBytes("MS950");
            acctNo = comc.subMS950String(bytes, 0, 13).trim();
            idNo = comc.subMS950String(bytes, 14, 10).trim();
            if (totCnt == 1) {
