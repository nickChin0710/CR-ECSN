...
*  --------- --------- ----------- -----------------------------------------  *
*  112/07/10 V1.01.01  Lai         program initial                            *
*  112/07/21 V1.01.02  Lai         add bil_mcht_apply_tmp                     *
*  112/11/27 V1.01.03  Lai         fix return 255                             *
*  112/11/27 V1.01.03  kirin       rename err_rtn  & 移除 h_call_batch_seqno    *
******************************************************************************/
package Mkt;
...
/*產生市區免費停車名單*/
public class MktE080 extends AccessDAO {
  private String progname = "卡友各項權益統計-市區停車  112/07/21 V1.01.02";
 ...
 try 
  {
   ...
   // =====================================
   if (args.length > 2) {
    comc.errExit("Usage : MktE080 [YYYYMM] [batch_seq]", "");
   }
   ...
   if (args.length >  0) {
       ...
       if(args[0].length() == 8) {
          ...
          hBusinessPrevMonth = hBusinessDate.substring(0, 6);
          showLogMessage("I", "", String.format("輸入參數[%s]", hBusinessDate));
         }
   } else {
           commonRtn();
          }

   if (!hBusinessDate.substring(6).equals("01")) {
        comc.errExit(String.format("本程式只在每月1日執行[%s]", hBusinessDate), "");
   }
...
/***************************************************************************/
    ...
    void selectCrdCard() throws Exception {
        ...
        sqlCmd +=                         "  and apr_flag     = 'Y')) ";
        sqlCmd += "  or  (decode(group_code, '','0000', group_code) in ( select data_code  ";
        sqlCmd += "          from bil_dodo_bn_data  ";
        sqlCmd += "         where data_type = '02' ";
        sqlCmd += "           and apr_flag  = 'Y'))))) ";
if(DEBUG==1)
  {
   sqlCmd = sqlCmd + "     and a.card_no in ('4563022001023501','4563022001750194','3565538800109209','4003538500002892','4003538500050909','4003530000046198') ";
   sqlCmd = sqlCmd + " limit 20000 ";
  }
        setInt(1, tempInt);
        setInt(2, tempInt);
        showLogMessage("I", "", "  CARD open 1 =["+tempInt+"]");
        openCursor();
        showLogMessage("I", "", "  CARD open 1 end !");
        while(fetchTable()) {
            ...
            if (debug == 1)
                showLogMessage("I", "", "Read card_no=" + hCardCardNo + ","+totCnt);
            ...
            if (debug == 1)
                showLogMessage("I", "", " 888 chk_stmt =[" + rtn + "]" + hCardStmtCycle);
            if (rtn > 0)
                continue;
    ...
    void selectBilDodoBnData(String tGroupCode, String tCardType) throws Exception {
        ...
        sqlCmd += "   and ((card_type_flag = '0' ";
        sqlCmd += "           or  (card_type_flag = '1' ";
    ...
    int chkBilDodoParm(String tActionCd) throws Exception {
        ...
        for (int idx1 = 0; idx1 < dodoParmCnt; idx1++) {
            ...
            if(debug == 1)
                showLogMessage("I","", String.format("  method=[%s],[%s]", hDopaConsumeMethod1.get(idx1)
                                                       , hDopaConsumePeriod1.get(idx1)));
...
void selectCycAfee() throws Exception {
   ...
   sqlCmd = "select rcv_annual_fee "
          ...
          + "   and rcv_annual_fee > 0 fetch first 1 rows only "
          ;

   setString(1, hCardCardNo);
   setString(2, hBusinessPrevMonth+"01");
   setString(3, hBusinessPrevMonth+"31");
   int recordCnt = selectTable();
   if (recordCnt >0) {
      rcvAnnualFee   = getValueDouble("rcv_annual_fee");
   }
}
/***********************************************************************/
void okRtn(int tHour, String tActionCd, int idx) throws Exception 
{
  ...
  if (hDomsDodoMsg.equals("N") || hDomsDodoMsg.length() == 0) {
      ...
      setValue("proj_code"             , tActionCd);                 //適用專案
      setValueInt("use_cnt"            , UseCnt);                    //INTEGER (4,0)   已使用次數 來自請款檔(實際使用)
//    setValueDouble("free_per_amt     , 0         );                //DECIMAL (12,0)  每次折抵金額
      setValueDouble("curr_month_amt"  , hAmt);                      //DECIMAL (12,0)  消費金額
      setValueDouble("curr_month_cnt"  , hCnt);                      //DECIMAL (12,0)  消費次數
...
//    setValueDouble("cal_seqno        ,             );              //DECIMAL (10,0)  權益流水號
      setValue("acct_type"             , hCardAcctType);             //VARCHAR (2,0)   帳戶類別
      setValue("card_type"             , hCardCardType);             //VARCHAR (2,0)   卡片種類
      setValue("group_code"            , hCardGroupCode);            //VARCHAR (4,0)   團體代號
...
if(DEBUG_F==1) showLogMessage("I", "", " ** insert =[" + hCardCardNo + "]"+hCardIdPSeqno+","+hCardCardNo.length()+", OK="+okCnt+", RTN="+rtn);
      if (dupRecord.equals("Y")) {
  //      comcr.errRtn("insert cms_ticket_card  error!", hCardCardNo, ProjCode);
          showLogMessage("I", "", "    ** insert dupRecord [" + hCardCardNo + "]"+hCardIdPSeqno);
      }
      else {
            insertCmsRightYear(tActionCd);
      }
}
/*******************************************************************/
void insertCmsRightYear(String tActionCd) throws Exception 
{
      setValue("acct_year"             , hBusinessPrevMonth.substring(0,4));
      setValue("id_p_seqno"            , hCardIdPSeqno);
      setValue("card_no"               , hCardCardNo);
      setValue("item_no"               , ItemNo);                    //道路救援
      setValue("proj_code"             , tActionCd);
      setValueInt("use_cnt"            , UseCnt);                    //INTEGER (4,0)   已使用次數 來自請款檔(實際使用)
//    setValueDouble("free_per_amt"    , 0   );                     //DECIMAL (12,0)  每次折抵金額
      setValueDouble("curr_year_consume", hAmt);                     //DECIMAL (12,0)  消費金額
      setValueDouble("curr_year_cnt"   , hCnt);                      //DECIMAL (12,0)  消費次數
      setValueDouble("rcv_annual_fee"  , rcvAnnualFee);              //DECIMAL (10,2)  應收年費
//    setValueDouble("platform_kind_amt,                );
//    setValueDouble("platform_kind_cnt,                );           //DECIMAL (12,0)  排除一般消費次數
//    setValueInt("used_next_cnt       ,                );           //INTEGER (4,0)   預支使用次數
...
//    setValueDouble("cal_seqno        ,         );                  //DECIMAL (10,0)  權益流水號
      ...
      if (dupRecord.equals("Y")) {
          updateCmsRightYear(tActionCd);
      }
}
/*******************************************************************/
void updateCmsRightYear(String tActionCd) throws Exception 
{
    ...
    whereStr   = "WHERE acct_year  = ? "
               ...
               + "  and proj_code  = ? ";
    int idx_f = 1;
    setDouble(idx_f++, hAmt);
    setDouble(idx_f++, hCnt);
    setDouble(idx_f++, rcvAnnualFee);
    setString(idx_f++, hBusinessPrevMonth.substring(0,4) );
    setString(idx_f++, hCardIdPSeqno);
    setString(idx_f++, hCardCardNo  );
    setString(idx_f++, ItemNo       );
    setString(idx_f++, tActionCd    );
    int recCnt = updateTable();

    if (notFound.equals("Y")) {
        //comcr.err_rtn("update cms_right_year error!", hCardCardNo+","+tActionCd, comcr.h_call_batch_seqno);
        comcr.errRtn("update cms_right_year error!", hCardCardNo+","+tActionCd, "");
    }
...
//欄位移除        sqlCmd += "where office_m_code  = 'DODO'  ";
    ...
    void join21(String tActionCd) {
        sData = "select max(a.purchase_date) as purchase_date, sum(decode(a.acct_code,'IT', "
...
if(DEBUG==1)
  {
   sData  = sData + "     and a.card_no in ('5241700000113111','5245167700526180','5468580005755827','4563028600770962','3565537110152438','4003538500059629') ";
  }

        sDataParam.clear();
        sDataParam.add(hTempCardNo);
        sDataParam.add(hBillAcctMonth);
...
void join31(String tActionCd) throws Exception 
{
        sData = "select max(a.purchase_date) as purchase_date, sum(decode(a.acct_code,'IT', "
                ...
                + "  b.contract_no = decode(a.contract_no,'','x',a.contract_no) "
                + "   and b.contract_seq_no = a.contract_seq_no      " + "   where a.id_p_seqno                   = ? "
                + "   and decode(a.acct_month,'','x',a.acct_month)  = ? "
                + "   and decode(a.rsk_type,'','N',a.rsk_type) not in ('1','2','3')  "
...
void selectBilDodoDtlTemp() throws Exception 
{
    ...
    while (fetchTable()) {
      ...
      if(DEBUG == 1) showLogMessage("I", "", "Write card=" + hCardCardNo + ","+hIdNo+","+hCurrentCode+", Cnt="+ totCnt1);
...
private void selectBilMchtApplyTmp() throws Exception
{
  ...
  selectTable();
  if (notFound.equals("Y")) {
//    commCrd.errExit("selectbil_mcht_apply_tmp not found!", "");
  }
  UseCnt  = getValueInt("appl_cnt");
}
  /***********************************************************************/
