...
/*產生花蓮第一信用合作社聯名卡企業回饋金明細表*/
public class MktRM18L extends AccessDAO {
    private String progname = "產生花蓮第一信用合作社聯名卡企業回饋金明細表程式  112/10/19  V1.00.02";
    CommFunction   comm  = new CommFunction();
    CommCrd        comc  = new CommCrd();
    CommCrdRoutine comcr = null;

    String prgmId = "MktRM18L";
    String prgmName = "花蓮第一信用合作社聯名卡企業回饋金明細表";
    
    String rptNameM18L = "花蓮第一信用合作社聯名卡企業回饋金明細表";
    String rptIdM18L = "CRM18L";
    int rptSeqM18L = 0;
    int pageCntM18L = 0;
    List<Map<String, Object>> lparM18L = new ArrayList<Map<String, Object>>();
		...
		while (fetchTable(cursorIndex)) {
            ...
            if (keepPromoteDept.equals(getValue("promote_dept")) == false) {
            	if (realCnt == 0) {
            		...
            		printDetailM18L(keepPromoteDept,acctNo);
            	}
    ...
    void printHeaderM18L() throws UnsupportedEncodingException, Exception {
    	pageCntM18L++;
        
        buf = "";
        buf = comcr.insertStr(buf, "分行代號:  3144信用卡部" ,  1);
        buf = comcr.insertStr(buf, ""              + rptNameM18L             , 35);
        buf = comcr.insertStr(buf, "保存年限: 二年"                             ,97);
        lparM18L.add(comcr.putReport(rptIdM18L, rptNameM18L, sysDate, ++rptSeqM18L, "0", buf));
        
        buf = "";
        String strDate = String.format("%3.3s年%2.2s月%2.2s日", hBusDateTwYear,hBusDateMonth, hBusDateDay);
        buf = comcr.insertStr(buf, "報表代號: CRM18L     科目代號:"            ,  1);
        buf = comcr.insertStr(buf, "中華民國 " + strDate                      , 46);
        buf = comcr.insertStr(buf, "頁    次:" + String.format("%4d", pageCntM18L) ,97);
        lparM18L.add(comcr.putReport(rptIdM18L, rptNameM18L, sysDate, ++rptSeqM18L, "0", buf));
        ...
        buf = "";
        buf = comcr.insertStr(buf, "花蓮一信帳戶             消費總額             0.2% 回饋金       分社代號        分社中文名稱", 1);
        lparM18L.add(comcr.putReport(rptIdM18L, rptNameM18L, sysDate, ++rptSeqM18L, "0", buf));
    ...
    /**
     * @throws Exception *********************************************************************/
    void printFooterM18L() throws Exception {
        ...
        comcr.insertPtrBatchRpt(lparM18L);

    }

    /***********************************************************************/
    void printDetailM18L(String keepPromoteDept,String acctNo) throws Exception {
