...
*  --------- --------- ----------- -----------------------------------------  *
*  112/08/01 V1.01.01  Lai         program initial                            *
*  112/08/30 V1.01.02  Zuwei Su    header控制碼修改                            *
*  112/08/31 V1.01.03  Zuwei Su    註解ftp                            *
*  112/09/02 V1.01.04  Kirin       change introduce_emp_no                    *
*  112/11/24 V1.01.05  Kirin       每月2日執行 專案代碼=2023010001                  *
******************************************************************************/
package Mkt;
...
public class MktRF694 extends AccessDAO {
    private String PROGNAME = "員工推展「卡娜赫拉的小動物悠遊聯名卡」獎勵明細表  112/08/01 V1.01.01";
...
//    String pgmCd     = "2023010002";
    ...
    String rptIdR1   = "RCRF694.1";
    String rptIdR1_1   = "CRF694";
    String rptName1  = "員工推展「卡娜赫拉的小動物悠遊聯名卡」獎勵明細表";
 ...
 try 
   {
    ...
    // =====================================
    if(args.length > 3 ) {
       comc.errExit("Usage : MktRF694 [PROGRAM_CODE] [yyyymmdd] [seq_no] ", "");
      }
/*
...
//   hEndDateBil = applyDateE;   
   ...
   hBegDateBil = comDate.dateAdd(hBusiBusinessDate , 0,-1, 0).substring(0,6) + "31";
...
//sqlCmd += "  where c.clerk_id     = ? ";
  ...
  sqlCmd += "    and c.group_code   in ('1684','1685') ";
...
if(DEBUG_F==1) showLogMessage("I","","   888 bill cnt="+cardIdPSeqno+","+loadBilCnt);
       chkBillMonth();

if(DEBUG_F==1) showLogMessage("I","","  Card="+cardCardNo+" ,"+cardIdPSeqno+" , idx="+k+",C="+chkCnt+",B="+bilCnt);
//  全新戶有效卡:6個月內沒有任一張有效卡       非全新戶有效卡:有任一張有效卡
//  有效卡是半年內有消費
       if(chkCnt < 1 || bilCnt < 1)  allDataInt[brnCnt][1]++;  
       if(chkCnt > 0 && bilCnt > 0)  allDataInt[brnCnt][2]++;
    }
...
if(DEBUG==1) showLogMessage("I","","Read brn="+emplUNitNo+" ID="+emplId+","+emplIdPSeqno+",C="+emplChiName+",Cnt="+totCnt);
     ...
     allDataInt[brnCnt][3] =  (int) (allDataInt[brnCnt][1]/3) * 5 + allDataInt[brnCnt][2];
if(DEBUG==1) showLogMessage("I","","   Dtl="+brnCnt+","+allDataInt[brnCnt][1]+","+allDataInt[brnCnt][2]+","+allDataInt[brnCnt][3]);
    }
...
void writeRptHead(int idx) throws Exception 
{
   ...
   buf = comc.fixLeft(tmpStr1, 10) + comc.fixLeft("RCRF694", 16) 
//       + comc.fixLeft(tmpStr2, 88) + comc.fixLeft("R", 8);
   + comc.fixLeft(tmpStr2, 88) + comc.fixLeft("N", 8);

   writeTextFile(fptr1, buf+"\n");
if(DEBUG==1) showLogMessage("I","","888 Write Rpt Head =======[" +idx+"]"+buf +", Tot="+totCnt);
...
//        lpar1.add(comcr.putReport(rptIdR1, rptName1, sysDate, ++rptSeq1, "0", "##PPP"));
         lpar1.add(comcr.putReport(rptIdR1_1,rptName1, sysDate, ++rptSeq1, "0", "##PPP"));
...
if(DEBUG_F==1) showLogMessage("I","","   Write Head="+idx+",Page="+ pageCnt1+","+tempBrn+","+emplUNitNo);
     ...
     buf = comcr.insertStr(buf, "報表代號: CRF694    科目代號:"   ,  1);
     ...
     buf = "";
     if(idx ==1)
        buf = comcr.insertStr(buf, "薪資單位: " +emplUNitNo+" "+comc.fixLeft(emplBrnChiName,10),  1);
     else
        buf = comcr.insertStr(buf, "薪資單位: " +tempBrn +" "+comc.fixLeft(tempName,10)        ,  1);
     writeFile(buf);

     buf = "員工ＩＤ   姓  名                全新卡有效卡   非全新卡有效卡   兌換券張數 ";
     writeFile(buf);

     buf = "                               （每３卡 500元）（每１卡 100元） 全聯禮券 100元 簽收人蓋章或簽名";
     writeFile(buf);
...
if(DEBUG_F==1) showLogMessage("I","","   Write Dtl="+brnCnt+","+tempBrn+","+emplUNitNo);
     ...
     for (int i = 1; i < brnCnt+1; i++)
       {
if(DEBUG_F==1) showLogMessage("I","","   Write Max="+reportPageLine+","+lineCnt1+","+allDataStr[i][0]);
