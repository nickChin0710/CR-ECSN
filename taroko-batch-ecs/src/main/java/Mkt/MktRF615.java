...
public class MktRF615 extends AccessDAO {
    final private String PROGNAME = "員工推展「VISA御璽房貸卡」獎勵明細表  112/08/07 V1.01.02";
    ...
    int DEBUG = 0;
    int DEBUG_F = 0;
    String prgmId = "MktRF615";
    String hTempUser = "";

    int reportPageLine = 45;
//  String PgmCd = "2023010002";
    String PgmCd = "2023010001";
    String rptIdR1 = "RCRF615";
    String rptName1 = "員工推展「VISA御璽房貸卡」獎勵明細表";
    ...
    public int mainProcess(String[] args) {
        try {
            ...
            // =====================================
            if (args.length > 3) {
                comc.errExit("Usage : MktRF615 [PROGRAM_CODE] [yyyymmdd] [seq_no] ", "");
            }
            /*
...
//      filenameO = String.format("%s/media/crd/%s.1.txt", comc.getECSHOME(), rptIdR1);
...
//      sqlCmd += "  where c.clerk_id     = ? ";        
        ...
        sqlCmd += "    and c.group_code   in ('1615')  ";
        /*
    ...
    void selectCrdEmployee() throws Exception {
        ...
        while (fetchTable()) {
            ...
            if (totCnt % 1000 == 0 || totCnt == 1) {
                showLogMessage("I", "", String.format("MktRF615 Process 1 record=[%d]\n", totCnt));
            }
...
//            A = 2*全新戶有效卡數 
//            B = 非全新戶有效卡數
            allDataInt[brnCnt][3] = 3 * allDataInt[brnCnt][1] + allDataInt[brnCnt][2];
            if (DEBUG == 1) {
                showLogMessage("I", "",
                        "   Dtl="
                                + brnCnt
    ...
    void writeHead(int idx) throws Exception {
        ...
        buf = comcr.insertStr(buf, "報表代號: CRF615    科目代號:", 1);
        ...
        buf = "員工ＩＤ   姓  名                全新戶有效卡     舊戶卡數     禮券張數 簽收人蓋章或簽名";
        writeFile(buf);
...
//        writeFile(buf);
    ...
    void finalTail() throws Exception {
        ...
        buf = " （１） 全新戶有效卡: 每1卡可獲全聯２００元禮券．";
        writeFile(buf);

        buf = " （２） 非全新戶有效卡: 每1卡可獲全聯１００元禮券．";
        writeFile(buf);
