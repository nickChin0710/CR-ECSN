...
/*會計起帳(BIL)處理*/
public class MktT410 extends AccessDAO {

    public final boolean DEBUG_MODE = false;

    private String PROGNAME = "高鐵標準車廂折抵會計分錄處理程式  112/12/18  V1.00.01" ;
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            // =====================================
            if (args.length > 1) {
                comc.errExit("Usage : MktT410, this program need only one parameter  ", "");
            }
            ...
            comcr.rptSeq       = 0;
            pgmName = String.format("MktT410");

            //高鐵升等付款處理
            showLogMessage("I", "", String.format("高鐵升等付款處理......."));
            procMktThsrDisc01();
            
            //高鐵升等收款處理
            showLogMessage("I", "", String.format("高鐵升等收款處理......."));
            procMktThsrDisc02();
	...
	void procMktThsrDisc01() throws Exception {
		...
		/*
		 */
		sqlCmd =  " SELECT sum(DISCOUNT_VALUE) as dest_amt ";
		sqlCmd += " from MKT_THSR_DISC ";
		...
		if (itemCnt <= 0) {
			showLogMessage("E", "", "gen_sys_vouch not defined, stdVouchCode=[F015]");
			return;
		}
		...
		tmpstr = String.format("MktT410_%s.%s_%s","F015", hVouchCdKind, hPcceCurrCodeGl);  //CRD93繳學費分錄
		comcr.hGsvhModPgm = tmpstr;
		comcr.hGsvhModWs = "MKT_T410R1";
	...
	void procMktThsrDisc02() throws Exception {
		...
		/*
		 */
		sqlCmd =  " select sum(DEST_AMT) as dest_amt ";
		sqlCmd += " from MKT_THSR_DISC ";
		sqlCmd += " where ERROR_CODE='00' and trans_date = ? ";
		sqlCmd += " and DEDUCT_TYPE = '2' ";
		...
		if (itemCnt <= 0) {
			showLogMessage("E", "", "gen_sys_vouch not defined, stdVouchCode=[F014]");
			return;
		}
		...
		tmpstr = String.format("MktT410_%s.%s_%s","F014", hVouchCdKind, hPcceCurrCodeGl);  //CRD93繳學費分錄
		comcr.hGsvhModPgm = tmpstr;
		comcr.hGsvhModWs = "MKT_T410R1";
