...
public class MktT250 extends AccessDAO {
    private String progname = "高鐵車廂升等(無限金鑽卡)發簡訊 112/09/11 V1.00.00";
	...
	void selectBilContract() throws Exception{
		...
		while (fetchTable()) {
			totalCnt++;
			SmsMsgDtl250 msgDtl =	getMsgDtlData();
	...
	/**
	 * @throws Exception *****************************************************************************************/
	void getsMsgSeqno(SmsMsgDtl250 msgDtl) throws Exception {
		extendField = "seqno.";
	...
	int selectSmsMsgId() throws Exception {
		extendField = "msg_id.";
		sqlCmd = " select msg_dept,msg_userid,msg_id,msg_desc from SMS_MSG_ID where msg_pgm = 'MKTT250' ";
		int n = selectTable();
		if(n > 0) {
	...
	SmsMsgDtl250 getMsgDtlData() throws Exception {
		SmsMsgDtl250 msgDtl = new SmsMsgDtl250();
		//取得簡訊流水號
		getsMsgSeqno(msgDtl);
		msgDtl.msgDept = msgDept;
		msgDtl.msgUserid = msgUserid;
		msgDtl.msgId = "1622".equals(getValue("group_code"))?"9301":"9302";
		msgDtl.msgPgm = "MKTT250";
	...
	/**
	 * @throws Exception *********************************************************************/
	void insertSmsMsgDtl(SmsMsgDtl250 msgDtl) throws Exception {
		extendField = "msg_dtl.";
