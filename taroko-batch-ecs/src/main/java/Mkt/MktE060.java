...
*  --------- ----------  ----------- ---------------------------------------- *
*  112/07/05 V1.01.01    Lai         Program initial                          *
*  112/07/21 V1.01.02    Lai         add bil_mcht_apply_tmp                   *
*  112/07/26 V1.01.03    Kirin       err_rtn 刪底線                             *
*  112/11/26 V1.00.04    Kirin       移除 comc.errExit--> return code非0       *
******************************************************************************/
package Mkt;
...
public class MktE060 extends BaseBatch {
    private final String PROGNAME = "卡友各項權益統計-影城活動(type:xx)   112/07/21 V1.01.02";
    ...
    String prgmId   = "MktE060";
    String rptName  = "影城活動名單";
    String rptId    = "MktE060R0";
    int    rptSeq   = 0;
...
public int mainProcess(String[] args) 
{
        try {
            ...
            if (args.length >  1) 
                commCrd.errExit(String.format("Usage : MktE060 [BUSINESS_DATE](計算當月)"),"");
            if (args.length == 1) {
                busiDate  = args[0];
                busiMonth = busiDate.substring(0, 6);
                showLogMessage("I", "", String.format("輸入參數[%s]", busiDate));
            } else {
                selectPtrBusinday();
            }
            ...
            if (!busiDate.substring(6).equals("01")) {
                ...
                showLogMessage("I", "", "本程式只在每月1日執行, 本日非執行日!!本日=[" + busiDate + "] process end....");
                return 0;                
            }
            ...
            if (cnt > 0) {
             //   commCrd.errExit("本月("+busiMonth+")己產製名單,不可重覆執行", "");
                showLogMessage("I", "", "本月("+busiMonth +")己產製名單,不可重覆執行");
        		return 0;
            }
...
if(DEBUG==1) showLogMessage("I", "", " Read Parm cnt=[" + ilSelectRow + "]");
  for (int ii = 0; ii < ilSelectRow; ii++) {
      ...
      showLogMessage("I",""," Main PROJ="+cmsRightParm.projCode+"["+cmsRightParm.chooseCond+"]"+cmsRightParm.consumeType+","+cmsRightParm.lastMm+","+cmsRightParm.currCnt);
...
//    getCardNo();
      ...
      showLogMessage("I","", " Credit 筆數  total=["+totCnt+"] ok=["+okCnt+"]"+putBil);
...
if(DEBUG_F==1) showLogMessage("I", "", " ** insert =[" + billInfo.bilCardNo + "]"+billInfo.idNo+","+billInfo.bilCardNo.length()+", OK="+okCnt+", RTN="+rtn);
  ...
  //      comcr.errRtn("insert cms_ticket_card  error!", billInfo.bilCardNo, parm.projCode);
          showLogMessage("I", "", "    ** insert dupRecord [" + billInfo.bilCardNo + "]"+billInfo.idNo);
      }
      else {
            insertCmsRightYear();
...
private void updateCmsRightYear() throws Exception 
{
    ...
    if (notFound.equals("Y")) {
        comcr.errRtn("update cms_right_year error!", CardNo+","+ProjCode , comcr.hCallBatchSeqno);
        //comcr.err_rtn("update cms_right_year error!", CardNo+","+ProjCode , comcr.h_call_batch_seqno);
    }
...
private void selectBilMchtApplyTmp() throws Exception
{
  ...
  selectTable();
  if (notFound.equals("Y")) {
//    commCrd.errExit("select bil_mcht_apply_tmp not found!", "");
  }
