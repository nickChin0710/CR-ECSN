...
/*產生嘉義第三信用合作社聯名卡企業回饋金明細表*/
public class MktRM18N extends AccessDAO {
    private String progname = "產生嘉義第三信用合作社聯名卡企業回饋金明細表程式  112/10/19  V1.00.02";
    CommFunction   comm  = new CommFunction();
    CommCrd        comc  = new CommCrd();
    CommCrdRoutine comcr = null;

    String prgmId = "MktRM18N";
    String prgmName = "嘉義第三信用合作社聯名卡企業回饋金明細表";
    
    String rptNameM18N = "嘉義第三信用合作社聯名卡企業回饋金明細表";
    String rptIdM18N = "CRM18N";
    int rptSeqM18N = 0;
    int pageCntM18N = 0;
    List<Map<String, Object>> lparM18N = new ArrayList<Map<String, Object>>();
		...
		while (fetchTable(cursorIndex)) {
            ...
            if (keepPromoteDept.equals(getValue("promote_dept")) == false) {
            	if (realCnt == 0) {
            		...
            		printDetailM18N(keepPromoteDept,acctNo);
            	}
    ...
    void printHeaderM18N() throws UnsupportedEncodingException, Exception {
    	pageCntM18N++;
        
        buf = "";
        buf = comcr.insertStr(buf, "分行代號:  3144信用卡部" ,  1);
        buf = comcr.insertStr(buf, ""              + rptNameM18N             , 35);
        buf = comcr.insertStr(buf, "保存年限: 二年"                             ,97);
        lparM18N.add(comcr.putReport(rptIdM18N, rptNameM18N, sysDate, ++rptSeqM18N, "0", buf));
        
        buf = "";
        String strDate = String.format("%3.3s年%2.2s月%2.2s日", hBusDateTwYear,hBusDateMonth, hBusDateDay);
        buf = comcr.insertStr(buf, "報表代號: CRM18N     科目代號:"            ,  1);
        buf = comcr.insertStr(buf, "中華民國 " + strDate                      , 46);
        buf = comcr.insertStr(buf, "頁    次:" + String.format("%4d", pageCntM18N) ,97);
        lparM18N.add(comcr.putReport(rptIdM18N, rptNameM18N, sysDate, ++rptSeqM18N, "0", buf));
        ...
        buf = "";
        buf = comcr.insertStr(buf, "嘉義三信帳戶             消費總額             0.2% 回饋金       分社代號        分社中文名稱", 1);
        lparM18N.add(comcr.putReport(rptIdM18N, rptNameM18N, sysDate, ++rptSeqM18N, "0", buf));
    ...
    /**
     * @throws Exception *********************************************************************/
    void printFooterM18N() throws Exception {
        ...
        comcr.insertPtrBatchRpt(lparM18N);

    }

    /***********************************************************************/
    void printDetailM18N(String keepPromoteDept,String acctNo) throws Exception {
