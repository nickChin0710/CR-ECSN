...
*  --------- --------- ----------- -----------------------------------------  *
*  112/08/01 V1.01.01  Wilson       program initial                           *
*  112/08/31 V1.01.02  Wilson       有效卡改為判斷最後消費日期                                                                   *        
*  112/11/08 V1.01.03  Wilson       日期減一天                                                                                                 *
*  112/12/19 V1.01.04  Wilson       調整累計迄日                                                                                             *
******************************************************************************/
package Crd;
...
public class CrdR052C extends AccessDAO {
    private final String PROGNAME = "全部信用卡卡量累計彙總表  112/12/19 V1.01.04";
    ...
    String prgmId    = "CrdR052C";

    String rptIdR1 = "CRD52C";
    String rptName1  = "全部信用卡卡量累計彙總表";
 ...
 try 
   {
    ...
    // =====================================
    if(args.length > 3) {
       comc.errExit("Usage : CrdR052C [yyyymmdd] [seq_no] ", "");
      }
...
public int selectPtrBusinday() throws Exception 
{
   ...
   sqlCmd += "     , to_char(to_date(?,'yyyymmdd'),'yyyy')||'0101' h_beg_date  ";
...
void selectCrdCard() throws Exception 
{
  ...
  // if(DEBUG==1) showLogMessage("I","","Read card="+card_card_no+" G="+card_group_code+" NAME="+card_name+" Cnt="+tot_cnt);
     ...
     if(cardOppostDate.length() > 0 && comc.getSubString(cardOppostDate, 0, 4).equals(comc.getSubString(hBusiBusinessDate, 0, 4))) {
    	 oppostCnt++;
    	 oppostAll++;
     }
     
     if(comc.getSubString(cardOriIssueDate, 0, 4).equals(comc.getSubString(hBusiBusinessDate, 0, 4))) {
    	 issueCnt++;
    	 issueAll++;
     }
...
void headFile() throws Exception 
{
        ...
        buf = comcr.insertStr(buf, "報表代號: CRD52C     科目代號:"            ,  1);
        ...
        buf = "";
        buf = "卡別 BIN-NO        當年發卡數   當年停卡數   流通卡數   有效卡數   卡片名稱 ";
        lpar1.add(comcr.putReport(rptIdR1, rptName1, sysDate, ++rptSeq1, "0", buf));
