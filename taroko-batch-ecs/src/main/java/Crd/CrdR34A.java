...
public class CrdR34A extends AccessDAO {
    private final String PROGNAME = "產生COMBO卡製妥明細表報表檔程式  112/09/10  V1.00.01";
    ...
    private final String DATA_FORM = "RCRD34A";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR34A [business_date]");
...
//        String actNo = getValue("ACT_NO");
        ...
        String str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRD34A", 16) + commCrd.fixLeft(searchDateTw + "COMBO卡製妥明細表", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("COMBO卡製妥明細表", 62) + commCrd.fixLeft("保存年限: 一年", 16);
        ...
        str = "報表代號: " + commCrd.fixLeft("CRD34A", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
