...
public class CrdR88C extends AccessDAO {
    private final String PROGNAME = "產生員工國際信用卡核發備查簿(續卡)報表檔程式  112/09/10  V1.00.01";
    ...
    private final String DATA_FORM = "RCRD88C";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR88C [business_date]");
...
//                    filename.replace(CommTxInf.DAT_EXTENSION, CommTxInf.HDR_EXTENSION);
    ...
    private int generateFile(String fileFolder, String datFileName)
        ...
        try {
            ...
            // 處理各分行當日寫入主檔的員工信用卡(續卡)製卡資料
            showLogMessage("I", "", "開始處理各分行當日寫入主檔的員工信用卡(續卡)製卡資料......");
            selectCrdEmbossData();
            String lastBranch = "";
            int lineCnt = 0;
            while (fetchTable()) {
                ...
                if (cardNo != null && cardNo.length() > 0) {
                	lineCnt++;
                    String rowOfDAT = getRowOfDAT(rowCount);
                    sb.append(rowOfDAT);
                    countInEachBuffer++;
                    totalCnt++;
                }
...
//        String actNo = getValue("ACT_NO");
        ...
        String str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRD88C", 16) + commCrd.fixLeft(searchDateTw + "員工國際信用卡核發備查簿（續卡）", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("員工國際信用卡核發備查簿（續卡）", 62) + commCrd.fixLeft("保存年限: 一年", 16); 
        sb.append(str).append(lineSeparator);
        String strDate = "中華民國 " + searchDateTwYear + " 年 " + searchDateMonth + " 月 " + searchDateDay + " 日";
        temp = String.format("%04d", pageCnt1);
        str = "報表代號: " + commCrd.fixLeft("CRD88C", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
...
//        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
    ...
    private String getRowOfDAT(int rowNum) throws Exception {
        ...
        sb.append(commCrd.fixLeft(getValue("GROUP_CODE"), 9)); // 卡別  (1)讀取到的GROUP_CODE
        sb.append(commCrd.fixLeft(getValue("REG_BANK_NO"), 9)); // 發卡單位    (1)讀取到的REG_BANK_NO
        sb.append(commCrd.fixLeft(getValue("CHI_NAME"), 12)); // 姓名  (1)讀取到的CHI_NAME
        sb.append(commCrd.fixLeft(getValue("ID_NO"), 12)); // 持卡人ID   (1)讀取到的ID_NO
        sb.append(commCrd.fixLeft(getValue("CHI_NAME"), 12)); // 員工姓名    (1)讀取到的CHI_NAME
        sb.append(commCrd.fixLeft(getValue("ACT_NO"), 18)); // 存款帳號    (1)讀取到的ACT_NO
        sb.append(commCrd.fixLeft("", 12)); // 領卡人簽收   空白
        sb.append(commCrd.fixLeft("", 10)); // 核發日期    空白
        sb.append(commCrd.fixLeft("", 12)); // 核發人員    空白
        sb.append(commCrd.fixLeft("", 14)); // 備註  空白
        sb.append(lineSeparator);
