...
*  112/11/14 V1.01.04  Wilson      讀取hce主檔全部資料                                                                                 *
...
public class CrdR052B extends AccessDAO {
    private final String PROGNAME = "全部HCE卡量彙總表  112/12/19 V1.01.05";
    ...
    String prgmId    = "CrdR052B";

    String rptIdR1 = "CRD52B";
    String rptName1  = "全部HCE卡量彙總表";
 ...
 try 
   {
    ...
    // =====================================
    if(args.length > 3) {
       comc.errExit("Usage : CrdR052B [yyyymmdd] [seq_no] ", "");
      }
...
void loadBilEffc() throws Exception {
    ...
    whereStr    = "where batch_date between ? and ? "
                ...
                + "  and card_no in ( select v_card_no from hce_card) ";
...
void selectCrdCard() throws Exception 
{
  ...
  selectSQL = " a.v_card_no           ,b.group_code        , "
            + " a.crt_date            ,a.change_date       , "
            + " a.status_code         ,b.bin_no            , "
            + " c.name                                       ";
  daoTable = " hce_card a,crd_card b ,ptr_group_card c";
...
//  setString(4, hEndDate);
  ...
  while (fetchTable()) {
     ...
     cardVCardNo = getValue("v_card_no");     
   ...
   if(DEBUG==1) showLogMessage("I","","Read card="+cardVCardNo+" G="+cardGroupCode+" NAME="+cardName+" Cnt="+totCnt);
...
void headFile() throws Exception 
{
        ...
        buf = comcr.insertStr(buf, "報表代號: CRD52B      科目代號:"            ,  1);
