...
public class CrdR89C extends AccessDAO {
    private final String PROGNAME = "產生員工機場貴賓(龍騰)卡核發備查簿報表檔程式  112/09/10  V1.00.01";
    ...
    private final String DATA_FORM = "RCRD89C";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR89C [business_date]");
...
//                    filename.replace(CommTxInf.DAT_EXTENSION, CommTxInf.HDR_EXTENSION);
    ...
    private int generateFile(String fileFolder, String datFileName)
        ...
        try {
            ...
            // 處理各分行當日寫入主檔的員工龍騰卡製卡資料
            showLogMessage("I", "", "開始處理各分行當日寫入主檔的員工龍騰卡製卡資料......");
            selectCrdEmbossData();
            String lastBranch = "";
            int lineCnt = 0;
            while (fetchTable()) {
...
//        String actNo = getValue("ACT_NO");
        ...
        String str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRD89C", 16) + commCrd.fixLeft(searchDateTw + "員工機場貴賓（龍騰）卡核發備查簿", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("員工機場貴賓（龍騰）卡核發備查簿", 62) + commCrd.fixLeft("保存年限: 一年", 16); 
        sb.append(str).append(lineSeparator);
        String strDate = "中華民國 " + searchDateTwYear + " 年 " + searchDateMonth + " 月 " + searchDateDay + " 日";
        temp = String.format("%04d", pageCnt1);
        str = "報表代號: " + commCrd.fixLeft("CRD89C", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
