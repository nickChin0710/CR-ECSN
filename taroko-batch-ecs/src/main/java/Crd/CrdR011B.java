...
public class CrdR011B extends AccessDAO {
    private final String PROGNAME = "產生機場貴賓(PP卡)製妥明細表檔程式  112/09/10  V1.00.03";
    ...
    private final String DATA_FORM = "RCRF11";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR011B [business_date]");
...
//                            String.format("將第%d到%d筆資料寫入檔案", rowCount - OUTPUT_BUFF_SIZE, rowCount));
    ...
    private int header(StringBuffer sb) throws Exception {
        ...
        str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRF11", 16) + commCrd.fixLeft(searchDateTw + "機場貴賓（PP）卡製妥明細表", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("機場貴賓（PP）卡製妥明細表", 62) + commCrd.fixLeft("保存年限: 一年", 16); 
        sb.append(str).append(lineSeparator);
        String strDate = "中華民國 " + searchDateTwYear + " 年 " + searchDateMonth + " 月 " + searchDateDay + " 日";
        temp = String.format("%04d", pageCnt1);
        str = "報表代號: " + commCrd.fixLeft("CRF11", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
        ...
        str = " 序號  申請種類   卡別      卡號               身份證字號     姓名        PP卡號末八位         電話(宅)         電話(公)";
        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
        sb.append("====================================================================================================================================").append(lineSeparator);
...
//        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
        if (lineCnt > 0) {
            str = "合計: 首次直接申請PP卡(TYPE 1): " + type1Cnt + "    掛失PP卡(TYPE 2): " + type2Cnt
                   + "    毀損補發PP卡(TYPE 3): " + type3Cnt + "    偽冒PP卡(TYPE 5): " + type5Cnt
                   + "    PP卡續卡(TYPE 9): " + type9Cnt;
            sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
            sb.append(lineSeparator);
            line += 1;
        }
    ...
    private String getRowOfDAT(int seq) throws Exception {
        ...
        String ppCardNo8 = "";
        switch (ppCardNo.substring(0, 1)) {
            case "4":
                ppCardNo8 = ppCardNo.substring(6, 14);
                break;
            case "7":
                ppCardNo8 = ppCardNo.substring(10, 18);
                break;
            case "3":
                ppCardNo8 = ppCardNo.substring(4, 12);
                break;

            default:
                break;
        }
        sb.append(commCrd.fixLeft(ppCardNo8, 20)); // PP卡號末八碼
