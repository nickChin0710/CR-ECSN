...
*  ---------  --------- ----------- ----------------------------------------  *
*  109/04/09  V1.00.00    Wendy      program initial                           *
*
...
*  112/03/04  V1.00.05   Wilson     AP1金融卡製卡檔檔名調整                                                                      *
*  112/03/06  V1.00.06   Wilson     增加update crd_combo                       *
*  112/03/07  V1.00.07   Wilson     新增procFTP                                *
*  112/03/08  V1.00.08   Wilson     AP1金融卡製卡檔檔名調整為combo_debit_mkc_t_yyyymmdd01.txt* 
*  112/03/12  V1.00.09   Wilson     調整procFTP執行順序                                                                          * 
*  112/04/14  V1.00.10   Wilson     讀參數判斷是否由新系統編列票證卡號                                                    *
*  112/06/01  V1.00.11   Wilson     update mail_type、mail_branch              *
*  112/06/13  V1.00.13   Wilson     無檔案不當掉                                                                                             *
*  112/08/26  V1.00.14   Wilson     修正弱掃問題                                                                                             *
*  112/12/06  V1.00.15   Wilson     crd_item_unit不判斷卡種                                                             *
******************************************************************************/
...
public class CrdD064 extends AccessDAO {
    private String progname = "產生COMBO卡續卡製卡檔程式112/12/06  V1.00.15  ";
...
// if (DEBUG     == 1) showLogMessage("D", "", " CMD=[" + sqlCmd + "] ");
    ...
    int openTextFile() throws Exception {
        ...
        hNcccFilename = String.format("cb_%02d_makecard_cg_%s%02d.txt", tmpVendor, hSysDate, hNn);
    ...
    void checkFileCtl() throws Exception {
    	...
    	String likeFilename = "";
    	String hFileName = "";
    	likeFilename = String.format("cb_%02d_makecard_cg_%s.txt", tmpVendor, hSysDate)+"%";
...
if(debugD == 1) showLogMessage("I", "", " 888 NAME=["+file+"]"+ file.length());
                ...
                if (!file.substring(0,18).equals("combo_debit_mkc_t_"))
                    continue;
//                if (!file.substring(18,26).equals(String.valueOf(fileDate)))
//                    continue;
                hTempFilename = file;
...
/***********************************************************************/
    ...
    void createNcccNewFile() throws Exception {
        ...
        ap1ReceiveAddr = ap1ReceiveAddr.trim();
