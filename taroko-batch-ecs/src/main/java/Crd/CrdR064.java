...
*  --------- --------- ----------- -----------------------------------------  *
*  112/02/22 V1.01.01  Lai         program initial (report only one page)     *
...
public class CrdR064 extends AccessDAO {
    private final String PROGNAME = "每月學生卡信用卡流通卡量彙總表  112/11/08 V1.01.06";
    ...
    String prgmId    = "CrdR064";

    String rptIdR1 = "CRM64";
    String rptName1  = "每月學生卡信用卡流通卡量彙總表";
 ...
 try 
   {
    ...
    // =====================================
    if(args.length > 3) {
       comc.errExit("Usage : CrdR064 [yyyymmdd] [seq_no] ", "");
      }
...
void selectCrdCard() throws Exception 
{
  ...
  while (fetchTable()) {
     ...
     if(totCnt % 5000 == 0 || totCnt == 1)
        showLogMessage("I","",String.format("R06R4 rocess 1 record=[%d]\n", totCnt));
...
void headFile() throws Exception 
{
        ...
        buf = comcr.insertStr(buf, "報表代號: CRM64     科目代號:"            ,  1);
        ...
        buf = "";
        buf = "學生信用卡               V-無限 V-御璽 V-白金 V-金卡 V-普卡 M-世界 M-鈦金 M-鈦商 M-白金 M-金卡 M-普卡 J-白金 J-金卡 J-普卡 J-晶緻 總  計 ";
...
void processAi501() throws Exception {
	...
	rptData.put("STU_MAIN_CARD_20UNDER_CURRENT_CNT", allData[0][16]);
	rptData.put("STU_MAIN_CARD_20UP_CURRENT_CNT", allData[1][16]);
	rptData.put("STU_MAIN_CARD_25UP_CURRENT_CNT", allData[2][16]);
	rptData.put("STU_MAIN_CARD_30UP_CURRENT_CNT", allData[3][16]);
	rptData.put("STU_MAIN_CARD_40UP_CURRENT_CNT", allData[4][16]);
	rptData.put("STU_MAIN_CARD_50UP_CURRENT_CNT", allData[5][16]);
	rptData.put("STU_MAIN_CARD_60UP_CURRENT_CNT", allData[6][16]);
	rptData.put("STU_MAIN_CARD_CURRENT_SUM", allData[8][16]);
	
	rptData.put("STU_ADDI_CARD_20UNDER_CURRENT_CNT", allData[10][16]);
	rptData.put("STU_ADDI_CARD_20UP_CURRENT_CNT", allData[11][16]);
	rptData.put("STU_ADDI_CARD_25UP_CURRENT_CNT", allData[12][16]);
	rptData.put("STU_ADDI_CARD_30UP_CURRENT_CNT", allData[13][16]);
	rptData.put("STU_ADDI_CARD_40UP_CURRENT_CNT", allData[14][16]);
	rptData.put("STU_ADDI_CARD_50UP_CURRENT_CNT", allData[15][16]);
	rptData.put("STU_ADDI_CARD_60UP_CURRENT_CNT", allData[16][16]);
	rptData.put("STU_ADDI_CARD_CURRENT_SUM", allData[18][16]);
	
	stuCardCurrentTotal = allData[8][16] + allData[18][16];
...
void insertMisReportData() throws Exception {
    ...
    setValue("DATA_FROM", "CRM64");
    setValue("DATA_DATE", hBusiBusinessDate);
    setValueDouble("SUM_FIELD1", stuCardCurrentTotal);
