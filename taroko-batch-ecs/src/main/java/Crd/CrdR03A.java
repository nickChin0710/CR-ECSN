...
public class CrdR03A extends AccessDAO {
    private final String PROGNAME = "產生法人國際信用卡(新卡)發卡通知書暨核發備查簿報表檔程式  112/09/10  V1.00.01";
    ...
    private final String DATA_FORM = "RCRF03A";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR03A [business_date]");
...
//                    filename.replace(CommTxInf.DAT_EXTENSION, CommTxInf.HDR_EXTENSION);
    ...
    private int generateFile(String fileFolder, String datFileName)
        ...
        try {
            ...
            showLogMessage("I", "", "開始處理各分行當日寫入主檔的法人信用卡(新卡)製卡資料檔......");
            selectCrdEmbossData();
            String lastBranch = "";
            int lineCnt = 0;
            while (fetchTable()) {
...
//        String actNo = getValue("ACT_NO");
        ...
        String str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRF03A", 16) + commCrd.fixLeft(searchDateTw + "法人國際信用卡（新卡）發卡通知書暨核發備查簿", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("法人國際信用卡（新卡）發卡通知書暨核發備查簿", 62) + commCrd.fixLeft("保存期間: 六年", 16);
        ...
        str = "報表代號: " + commCrd.fixLeft("CRF03A", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
