...
* ---------  -------------------  ------------------------------------------ *
* 106/03/29  V1.00.06  Allen Ho   Initial                                    *
* 109/12/21  V1.00.01   shiyuqi       updated for project coding standard   *
* 112/02/02  V1.00.02  Wilson     OR ORI_ISSUE_DATE = ''                     *
* 112/06/16  V1.00.03  Wilson     5000筆commit                                *
* 112/09/05  V1.00.04  Wilson     selectCrdCard排序調整                                                                   *
*****************************************************************************/
package Crd;
...
public class CrdO010 extends AccessDAO {
    private String progname = "補原始卡號資料處理程式 112/09/05  V1.00.04 ";
...
{
    selectSQL = "acno_p_seqno, " + "CARD_NO, " + "OLD_CARD_NO ";
    daoTable = "CRD_CARD";
...
// ************************************************************************
public void selectCrdCard2() throws Exception 
{
    selectSQL = "acno_p_seqno, " + "ORI_CARD_NO, " + "min(ori_apply_no) AS ORI_APPKY_NO, "
    ...
    if (allScanFlag == 1)
        ...
        whereStr = "WHERE acno_p_seqno in (select acno_p_seqno  from crd_card "
                + "                  where end_card_no = '') GROUP BY acno_p_seqno,ori_card_no";
...
// ************************************************************************
public void updateCrdCard() throws Exception 
{
    ...
    if (notFound.equals("Y")) {
        showLogMessage("I", "", "update_crd_card error!");
        showLogMessage("I", "", "card_no=[" + hCardCardNo + "]");
        comcr.errRtn("update_crd_card error!", "", comcr.hCallBatchSeqno);
    }
    return;
}
// ************************************************************************
public void updateCrdCard1() throws Exception 
{
...
// ************************************************************************
public void updateCrdCard2() throws Exception 
{
    updateSQL = "end_card_no    = ? ";
    daoTable = "CRD_CARD";
    whereStr = "WHERE acno_p_seqno     = ? " + "AND   ORI_CARD_NO = ? " + "AND   END_CARD_NO != ? ";
...
// ************************************************************************
public void updateCrdCard3() throws Exception 
{
    updateSQL = "ori_card_no    = card_no, " + "ori_apply_no   = apply_no, " + "end_card_no    = card_no, "
            + "ori_issue_date = issue_date ";
    daoTable = "CRD_CARD";
...
// ************************************************************************
public int selectCrdCard0() throws Exception 
{
	
    selectSQL = "ORI_CARD_NO, " + "ORI_APPLY_NO, " + "ORI_ISSUE_DATE";
    daoTable = "CRD_CARD";
    whereStr = "WHERE CARD_NO = ? " + "AND   acno_p_seqno = ?";
...
// ************************************************************************
public void selectCrdCard1() throws Exception 
{
    extendField = "CRD1.";
    selectSQL = "CARD_NO," + "ISSUE_DATE, " + "OLD_CARD_NO ";
    daoTable = "CRD_CARD";
    whereStr = "WHERE acno_p_seqno = ?  " + "ORDER BY ISSUE_DATE";

    setString(1, hCardPSeqno);
    int recCnt = selectTable();

    if (notFound.equals("Y")) {
        showLogMessage("I", "", "select crd_card_1 error!");
        showLogMessage("I", "", "p_seqno=[" + hCardPSeqno + "]");
        comcr.errRtn("select_crd_card_1 error!", "", comcr.hCallBatchSeqno);
    }
...
// ************************************************************************
public int selectCrdCard3() throws Exception 
{
    extendField = "CRD3.";
    selectSQL = "CARD_NO," + "OLD_CARD_NO, " + "ISSUE_DATE ";
    daoTable = "CRD_CARD";
    whereStr = "WHERE acno_p_seqno = ? " + "AND   ORI_CARD_NO = ? " + "AND   nvl(ISSUE_DATE,'x')  = ? " + "ORDER BY "
    ...
    if (notFound.equals("Y")) {
        showLogMessage("I", "", "select crd_card_3 error!");
        showLogMessage("I", "", "p_seqno=[" + hCardPSeqno + "]");
        comcr.errRtn("select_crd_card_3 error!", "", comcr.hCallBatchSeqno);
    }
