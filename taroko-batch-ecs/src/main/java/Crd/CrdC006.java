...
*  ---------  --------- ----------- ----------------------------------------  *
*  109/02/20  V1.00.00    Rou         program initial                         *
*  109/12/17  V1.00.01    shiyuqi     updated for project coding standard     *
*  112/02/07  V1.00.02    Wilson      檔案產生路徑調整                                                                                 *
*  112/03/07  V1.00.03    Wilson      新增procFTP                              *
*  112/03/07  V1.00.04    Wilson      檔案的結束符號改成0D0A                        *
*  112/03/12  V1.00.05    Wilson      無資料產生空檔                                                                                     *
*  112/06/30  V1.00.06    Wilson      檔案格式增加onlineOpenflag                  *
*  112/07/03  V1.00.07    Wilson      假日不執行                                                                                            *
*  112/08/26  V1.00.08    Wilson      修正弱掃問題                                                                                        *
*  112/10/20  V1.00.09    Wilson      增加卡片組織別、卡片TYP欄位                                                         *
******************************************************************************/
...
import Crd.CrdC004.Buf1;

/*產生*/
public class CrdC006 extends AccessDAO {
    private String progname = "產生COMBO重製清單檔作業  112/10/20  V1.00.09 ";
    ...
    public int mainProcess(String[] args) {
        try {
            ...
            // =====================================
            if (args.length > 1) {
                comc.errExit("Usage : CrdC006 callbatch_seqno", "");
            }
    ...
    void checkOpen() throws Exception {
    	checkFileCtl();
    	filename = String.format("rqst_combo_s_%8s%02d.txt", hSystemDate, hNn);
    ...
    void checkFileCtl() throws Exception {
    	String likeFilename = "";
    	String hFileName = "";
    	likeFilename = String.format("rqst_combo_s_%8s", hSystemDate)+"%";
    ...
    // ************************************************************************
     int writeRtn() throws Exception {
        ...
        switch(hEmbossEmbossReason){
        case "1":
        	comboData.embossReason = "2";
        	break;
        case "2":
        	comboData.embossReason = "3";
        	break;
        case "3":
        	comboData.embossReason = "5";
        	break;
        }
        ...
        comboData.vmjType = "";
        comboData.cardType = "";
        comboData.onlineOpenflag = selectCcaOpposition();
        comboData.filler12          	 = ";;;;;;;;;;;;";
    ...
    String selectCcaOpposition()throws Exception {
    	...
    	if(hEmbossEmbossReason.equals("1")) {
