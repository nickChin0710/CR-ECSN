...
*  ---------  --------- ----------- ---------------------------------------- *
*  112/06/16  V1.00.00   Zuwei      program initial                          *
...
public class CrdR89A extends AccessDAO {
    private final String PROGNAME = "產生員工機場貴賓(PP)卡核發備查簿報表檔程式  112/09/10  V1.00.01";
    ...
    private final String DATA_FORM = "RCRD89A";
...
//            calBalance = new CalBalance(conn, getDBalias());
            ...
            showLogMessage("I","","Usage CrdR89A [business_date]");
...
//                    filename.replace(CommTxInf.DAT_EXTENSION, CommTxInf.HDR_EXTENSION);
    ...
    private int generateFile(String fileFolder, String datFileName)
        ...
        try {
            ...
            // 處理各分行當日寫入主檔的員工PP卡製卡資料
            showLogMessage("I", "", "開始處理各分行當日寫入主檔的員工PP卡製卡資料......");
            selectCrdEmbossData();
            String lastBranch = "";
            int lineCnt = 0;
            while (fetchTable()) {
...
//        String actNo = getValue("ACT_NO");
        ...
        String str = commCrd.fixLeft("3144", 10) + commCrd.fixLeft("CRD89A", 16) + commCrd.fixLeft(searchDateTw + "員工機場貴賓（PP）卡核發備查簿", 88) + commCrd.fixLeft("N", 8); 
        sb.append(str).append(lineSeparator);
        sb.append(commCrd.fixLeft(commCrd.fixRight("合作金庫商業銀行", 70), 132)).append(lineSeparator);
        str = commCrd.fixLeft("分行代號: 3144  信用卡部", 54) + commCrd.fixLeft("員工機場貴賓（PP）卡核發備查簿", 62) + commCrd.fixLeft("保存年限: 一年", 16); 
        sb.append(str).append(lineSeparator);
        String strDate = "中華民國 " + searchDateTwYear + " 年 " + searchDateMonth + " 月 " + searchDateDay + " 日";
        temp = String.format("%04d", pageCnt1);
        str = "報表代號: " + commCrd.fixLeft("CRD89A", 15) + commCrd.fixLeft("科目代號: ", 29) + commCrd.fixLeft(strDate, 56) + commCrd.fixRight("第" + temp + "頁",18);
        ...
        str = " 序號   團代          卡號          PP卡號末八碼     持卡人ID     姓  名        領卡人簽收         核發日期         核發人員  ";
        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
        sb.append("====================================================================================================================================").append(lineSeparator);
...
//        sb.append(commCrd.fixLeft(str, 132)).append(lineSeparator);
    ...
    private String getRowOfDAT(int seq) throws Exception {
        ...
        if (ppCardNo != null && ppCardNo.length()>=8) {
            ppCardNo = ppCardNo.substring(ppCardNo.length() - 8);
        }

        StringBuffer sb = new StringBuffer();
        sb.append(commCrd.fixLeft("" , 2)); // 空白
        sb.append(commCrd.fixLeft("" + seq, 6)); // 序號
//        sb.append(commCrd.fixLeft("", 8)); // 申請種類
        sb.append(commCrd.fixLeft(groupCode, 8)); // 卡別
        sb.append(commCrd.fixLeft(cardNo, 22)); // 卡號
        sb.append(commCrd.fixLeft(ppCardNo, 14)); // PP卡末八碼
        sb.append(commCrd.fixLeft(idNo, 14)); // 持卡人身份證字號
        sb.append(commCrd.fixLeft(chiName, 14)); // 姓名
