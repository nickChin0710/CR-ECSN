 ...
 *  109/09/04  V1.00.07   shiyuqi       bug fix                                *
 *  109/09/05  V1.00.08   Wilson       測試修改                                                                                               *
 *  109/09/17  V1.00.09   Wilson       新增處理毀損重製                                                                                *
 *  109/09/21  V1.00.10   Wilson      insert crd_idno_seqno 新增debit_idno_flag *
 *  109/09/25  V1.00.11   Wilson       insert dba_acno新增class_code = "C"      *
 *  109/09/29  V1.00.12   Wilson       無檔案不秀error、讀檔不綁檔名日期                                            *
 *  109/09/30  V1.00.13   Wilson       營業日才執行                                                                                        *
 *  109/10/12  V1.00.14   Wilson       檔名日期改營業日                                                                                *
 *  109/10/19  V1.00.15   Wilson       錯誤報表FTP                               *
 *  109-10-19  V1.00.16   shiyuqi       updated for project coding standard     *
 *  109/11/27  V1.00.17   Wilson       新增insert關帳期 = 01                      *
 *  109/12/30  V1.00.18   Wilson       新增insert crd_seqno_log                 *
 *  110/03/03  V1.00.19   Wilson       因變更ID程式執行順序調整，營業日減一日                                    *
 *  110/03/10  V1.00.20   Wilson       錯誤原因新增 -> 金融帳號已存在且ID與資料庫不同                     *
 *  110/12/08  V1.00.21   Wilson       錯誤訊息調整                                                                                          *
 *  110/12/16  V1.00.22   Wilson       insert cca_card_base新增card_indicator   *
 *  111/01/21  V1.00.23   Justin      增加MSG_FLAG(固定給 “Y”)、MSG_PURCHASE_AMT(固定給0) *
 *  111/02/14  V1.00.24    Ryan       big5 to MS950                                           *
 *  111/02/15  V1.01.25   Justin       shorten the length of English name       *
 *  111/02/17  V1.01.26   Justin       show no found messages                   *
 *  111/02/18  V1.00.27   Justin      fix the error causing by split phoneNo    *
 *  111/03/01  V1.00.28   Justin      update dba_acno act_no if 金融帳號已存在且ID與資料庫不同 *
 *  111/03/02  V1.00.29   Justin       增加顯示訊息                             *
 *  111/03/03  V1.00.30   Justin       新金融帳號使用舊ID串接                   *
 *  111/05/11  V1.00.31   Justin       update dba_acno增加acct_holder_id        *
 ******************************************************************************/
package Icu;
...
public class IcuD064 extends AccessDAO {
  private final String progname = "設一科悠遊VD卡製卡檔處理程式  111/05/11 V1.00.31";
  ...
  int debug = 0;
  String prgmId = "IcuD064";
  String rptName1 = "";
  List<Map<String, Object>> lpar1 = new ArrayList<Map<String, Object>>();
  BufferedWriter nccc = null;
  // col詳VD製卡檔格式
  protected final String dT1Str =
  ...
  protected final int[] dt1Length = {1, 7, 1, 13, 1, 10, 1, 2, 1, 37, 1, 2, 104, 1, 8, 20, 2, 8, 10,
      8, 3, 3, 1, 8, 8, 60, 16, 16, 16, 16, 16, 16, 16, 16, 24, 24, 24, 24, 24, 24, 24, 24, 16, 1,
      4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2,
      2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2, 1, 4, 8, 2, 2, 2, 2,
      1, 4, 8, 2, 2, 2, 2, 33, 4, 96, 6, 1, 1, 19, 1, 2, 1, 2, 5, 2, 1, 2, 2, 1, 1, 28, 1, 19, 1, 3,
      1, 1, 1, 16, 1, 26, 1, 4, 3, 1, 4, 8, 2, 3, 2, 1, 3, 1, 1, 16, 1, 4, 3, 1, 4, 3, 5, 1, 11, 7,
      1, 1, 4, 3, 3, 3, 23, 1, 4, 1, 1, 3, 3, 1, 41, 3, 5, 10, 1, 10, 1, 16, 33, 26, 5, 18, 18, 9,
      9, 9, 9, 2, 4, 4, 4, 33, 3, 33, 3, 97, 3, 1, 3};
  ...
  int openFile() throws Exception {
    ...
    for (String file : listOfFiles) {
      ...
      if (getFileName.length() != 31)
        continue;
      if (!getFileName.substring(0, 19).equals("VD_RS_TSC_MAKECARD_"))
        continue;
...
//      }
  ...
  private void moveData(Map<String, Object> map) throws Exception {
    ...
    data.cardNo = data.cardNo.substring(0, 4) + data.cardNo.substring(5, 9)
        + data.cardNo.substring(10, 14) + data.cardNo.substring(15, 19);
...
//	  int fileNo = 0;
//		
//	  sqlCmd  = "select max(substr(file_name, 32, 2)) file_no";
//      sqlCmd += " from crd_file_ctl  ";
//      sqlCmd += " where file_name like ?";
//      sqlCmd += "  and crt_date  = to_char(sysdate,'yyyymmdd') ";
//      setString(1, "VD_RS_TSC_MAKECARD_ERR." + "%" + ".TXT");
//      
//      if (selectTable() > 0) 
//      	fileNo = getValueInt("file_no");  
	  
    hNcccFilename = String.format("VD_RS_TSC_MAKECARD_ERR_%s.TXT", fileDate);
...
//      sqlCmd =
//          "select substr(to_char(ecs_card_acct_idx.nextval, '0000000000'), 2,10) as new_card_acct_idx ";
//      sqlCmd += " from dual ";
//      recordCnt = selectTable();
//      if (notFound.equals("Y")) {
...
//        }
  ...
  void insertDbcCard() throws Exception {
    ...
    setValue("new_end_date", sysDate.substring(0, 2) + data.validToYy + data.validToMm + lastdateOfmonth(data.validToYy,data.validToMm) );
