 ...
 * 111/02/14  V1.00.14    Ryan      big5 to MS950                                           *
 ******************************************************************************/
...
public class IcuD066 extends AccessDAO {
	private final String progname = "設一科悠遊VD續卡製卡檔處理程式 111/02/14 V1.00.14";
...
//        System.out.println("tmpstr="+tmpstr);
		...
		for (String file : listOfFiles) {
			...
			if (getFileName.length() != 30)
				continue;
			if (!getFileName.substring(0, 18).equals("VD_T_TSC_MAKECARD_"))
				continue;
...
//	      sqlCmd += "  and crt_date  = to_char(sysdate,'yyyymmdd') ";
//	      setString(1, "VD_T_TSC_MAKECARD_ERR." + "%" + ".TXT");
//	      
//	      if (selectTable() > 0) 
//	      	fileNo = getValueInt("file_no");  

		hNcccFilename = String.format("VD_T_TSC_MAKECARD_ERR_%s.TXT", getFileName.substring(18, 26));
