 ...
 *  ---------  --------- ----------- ----------------------------------------  *
 * 110/02/25  V1.00.00    Wilson      program initial                          *
 * 110/12/08  V1.00.01    Wilson      錯誤訊息調整                                                                                          *
 * 111/02/14  V1.00.02    Ryan      big5 to MS950                                           *
 ******************************************************************************/
...
public class IcuD068 extends AccessDAO {
	private final String progname = "設一科VD數位存款續卡製卡檔處理程式 111/02/14 V1.00.02";
...
//        System.out.println("tmpstr="+tmpstr);
		...
		for (String file : listOfFiles) {
			...
			if (getFileName.length() != 30)
				continue;
			if (!getFileName.substring(0, 18).equals("VD_T_DGT_MAKECARD_"))
				continue;
...
//	      sqlCmd += "  and crt_date  = to_char(sysdate,'yyyymmdd') ";
//	      setString(1, "VD_T_DGT_MAKECARD_ERR." + "%" + ".TXT");
//	      
//	      if (selectTable() > 0) 
//	      	fileNo = getValueInt("file_no");  

		hNcccFilename = String.format("VD_T_DGT_MAKECARD_ERR_%s.TXT", getFileName.substring(18, 26));
