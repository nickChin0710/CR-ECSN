 ...
 *  ---------  --------- ----------- ----------------------------------------  *
 *  109/06/18  V1.00.00    Pino      program initial                           *
...
public class IcuD061 extends AccessDAO {
	private String progname = "設一科VD預製卡(一般)製卡檔處理程式  111/02/14 V1.00.17";
	...
	int openFile() throws Exception {
		...
		for (String file : listOfFiles) {
			...
			if (getFileName.length() != 26)
				continue;
			if (!getFileName.substring(0, 14).equals("VD_P_MAKECARD_"))
				continue;
...
//		sqlCmd = "select max(batchno) as tmpBatchno1";
//		sqlCmd += " from dbc_emboss ";
//		int recordCnt = selectTable();
//		tmpBatchno = getValue("tmpBatchno1");
...
//        setString(1, "VD_P_MAKECARD_ERR." + "%" + ".TXT");
//        
//        if (selectTable() > 0) 
//        	fileNo = getValueInt("file_no");

		hNcccFilename = String.format("VD_P_MAKECARD_ERR_%s.TXT", fileDate);
