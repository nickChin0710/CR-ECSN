 ...
 *  109/08/14  V1.00.06   Wilson       資料夾名稱修改為小寫                                                                       
...
*  109/11/27   V1.00.15   Wilson       新增insert關帳期 = 01                       *
*  109/12/30   V1.00.16   Wilson       新增insert crd_seqno_log                 *
*  110/03/03   V1.00.17   Wilson       因變更ID程式執行順序調整，營業日減一日                                    *
*  110/03/10   V1.00.18   Wilson       錯誤原因新增 -> 金融帳號已存在且ID與資料庫不同                     *
*  110/11/25   V1.00.19   Justin       ACNO.ACNO_P_SEQNO取代CARD_ACCT_IDX       *
*  110/12/08   V1.00.20   Wilson       錯誤訊息調整                                                                                          *
*  110/12/16   V1.00.21   Wilson       insert cca_card_base新增card_indicator   *
*  111/01/21   V1.00.22   Justin       增加MSG_FLAG(固定給 “Y”)、MSG_PURCHASE_AMT(固定給0) *
*  111/02/14   V1.01.23    Ryan         big5 to MS950                                           * 
*  111/02/15   V1.01.24   Justin       shorten the length of English name       *
*  111/02/17   V1.01.25   Justin       show no found messages                   *
*  111/02/18   V1.00.26   Justin       fix the error causing by split phoneNo   *
*  111/03/01   V1.00.27   Justin       update dba_acno act_no if 金融帳號已存在且ID與資料庫不同 *
*  111/03/02   V1.00.28   Justin       增加顯示訊息                             *
*  111/03/03   V1.00.29   Justin       新金融帳號使用舊ID串接                   *
*  111/05/11   V1.00.30   Justin       update dba_acno增加acct_holder_id        *
 ******************************************************************************/
package Icu;
...
public class IcuD063 extends AccessDAO {
	private final String progname = "設一科VD卡製卡檔處理程式  111/05/11   V1.00.30";
	...
	int openFile() throws Exception {
		...
		for (String file : listOfFiles) {
			...
			if (getFileName.length() != 27)
				continue;
			if (!getFileName.substring(0, 15).equals("VD_RS_MAKECARD_"))
				continue;
...
//        setString(1, "VD_RS_MAKECARD_ERR." + "%" + ".TXT");
//        
//        if (selectTable() > 0) 
//        	fileNo = getValueInt("file_no");
		
		hNcccFilename = String.format("VD_RS_MAKECARD_ERR_%s.TXT", fileDate);
...
//				return 1;
	...
	public void updateDbaAcnoAcctNo(String actNo, String pSeqno, String idno, String idNoFromData) throws Exception {
