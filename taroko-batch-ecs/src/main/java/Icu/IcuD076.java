 ...
 *  109/10/23  V1.00.10   Wilson       錯誤報表檔檔名調整                                                                            *
 *  110/12/08  V1.00.11   Wilson       錯誤訊息調整                                                                                      *
 *  111/02/14  V1.00.12    Ryan      big5 to MS950                                           *
 ******************************************************************************/
package Icu;
...
public class IcuD076 extends AccessDAO {
	private final String progname = "CARDLINK一卡通要求列入關閉自動加值功能卡號檔(B2I002)處理程式  111/02/14  V1.00.12";
	...
	int openFile() throws Exception {
		...
		for (String file : listOfFiles) {
			...
			if (getFileName.length() != 25)
				continue;
			if (!getFileName.substring(0, 11).equals("B2I002_0006"))
				continue;
...
//				System.out.println(40 + "rec");
	...
	private void moveData(Map<String, Object> map) throws Exception {
		...
		if (debug == 1)
			System.out.println("卡片外顯卡號=" + data.ipsCardNo);
		selectIpsCard();
		commitDataBase();
		return;
	}
	...
	int outPutTextFile() throws Exception {
		hNcccFilename = String.format("B2I002_ERR_0006%s", fileDate);
	...
	void updateIpsCard() throws Exception {
		...
		updateSQL += " autoload_clo_date  = to_char(sysdate,'yyyymmdd')  , ";
