...
*  109-07-21    V1.00.01   shiyuqi       updated for project coding standard     *  
*  109-07-22    yanghan       修改了字段名称            *
*  109-08-11    V1.00.03    JustinWu   修改讀取方式
...
*  110/02/08   V1.00.09    Wilson    mark check header                         *
 * 111/02/14  V1.00.10    Ryan      big5 to MS950                                           *
 * 111/02/22  V1.00.11     Justin   增加錯誤訊息                               *
 * 111/11/30  V1.00.12     Wilson   新增updateTscCard，將舊悠遊卡號停用                                         *
 * 111/12/14  V1.00.13     Wilson   製卡類別為R(其他補發卡)才做updateTscCard           *
 ******************************************************************************/
package Icu;
...
public class IcuD071 extends AccessDAO {
  private final String progname = "CARDLINK悠遊信用卡製卡回饋檔處理程式 111/12/14  V1.00.13";
  ...
  private void readFile() throws Exception {
    ...
    String readstr = "";
    String errRsn = "";
    final String fileName = String.format("TSC_CRD_MAKECARD_RTN_%s.TXT", queryDate);
    String inFilename = String.format(comc.getECSHOME() + "/media/icu/TSC_CRD_MAKECARD_RTN_%s.TXT", queryDate);
//    boolean br = openBinaryInput(inFilename);    
//    if (br == false) {
//      comcr.errRtn("File not exist", inFilename, "");
//    }
    
    showLogMessage("I", "", String.format("尋找檔案[%s]", inFilename));
    ...
    fileName1 = String.format("TSC_CRD_MAKECARD_RTN_ERR_%s.TXT", queryDate);
    String outFilename = String.format(comc.getECSHOME() + "/media/icu/error/TSC_CRD_MAKECARD_RTN_ERR_%s.TXT", queryDate);
...
//        continue;
      ...
      if (readData.cardStatus.equals("OK") == false) {
        continue;
      }

      if (selectTscCard() > 0) {
        writeCnt++;
        errRsn = "該悠遊卡號已存在悠遊信用卡片資料檔中";
        showLogMessage("E", "", String.format("該悠遊卡號[%s]已存在悠遊信用卡片資料檔中", readData.svcPid));
        writeTextFile(out, String.format("%-16.16s%-16.16s%s%-8.8s\n", readData.svcPid,
            readData.cardNo, comc.fixLeft(errRsn, 200), sysDate));
        continue;
      }
      if (selectCrdCard() == 0) {
        writeCnt++;
        errRsn = "該信用卡號不存在卡片資料檔中";
        showLogMessage("E", "", String.format("該信用卡號[%s]不存在卡片資料檔中", readData.cardNo));
        writeTextFile(out, String.format("%16.16s%16.16s%s%8.8s\n", readData.svcPid, readData.cardNo,
            comc.fixLeft(errRsn, 200), sysDate));
        continue;
      }
      
      if(readData.issueType.equals("R")) {
    	  updateTscCard();
      }     
  ...
  /***********************************************************************/
  private int selectTscCard() throws Exception {
  ...
  /**
	* @ClassName: IcuD071
	* @Description: updateTscCard 將舊悠遊卡號停用
	* @Copyright : Copyright (c) DXC Corp. 2022. All Rights Reserved.
	* @Company: DXC Team.
	* @author Wilson
	* @version V1.00.12, Nov 30, 2022
	*/
  ...
  private void insertTscCard() throws Exception {
    ...
    /* tsc_emboss_rsn */
    if (readData.issueType.equals("N")) {
      tmpTscEmbossRsn = "1";
    } else if (readData.issueType.equals("C")) {
      tmpTscEmbossRsn = "3";
    } else if (readData.issueType.equals("R")) {
      tmpTscEmbossRsn = "5";
    }
    /* autoload_flag */
    if (readData.autoloadDefSts.equals("2")) {
      tmpAutoloadFlag = "N";
    } else if (readData.autoloadDefSts.equals("4")) {
      tmpAutoloadFlag = "Y";
    }
    /* new_end_date */
    tmpNewEndDate = comm.lastdateOfmonth("20" + readData.expirDte);
    ...
    setValue("tsc_amt", readData.isamAmount1);
    setValue("tsc_pledge_amt", readData.isamAmount2);
    setValue("ic_seq_no", readData.chipID);
    setValue("isam_seq_no", readData.isamChipID);
    setValue("isam_batch_no", readData.isamIssueBatchNO);
    setValue("isam_batch_seq", readData.iasmIssueSeqNo);
    setValue("autoload_amt", readData.autoloadAmount);
  ...
  /****************************************************************************/
	...
	/****************************************************************************/	
  class Buf1 {
    String id;/* 001_011 持卡人ID */
    String svcPid;/* 012_027 悠遊卡號 */
    String expirDte;/* 028_031 悠遊卡有效期(信用卡有效期 YYMM) */
    String cardType;/* 032_034 信用卡別 */
    String cardNo;/* 035_050 信用卡號 */
    String acctNo;/* 051_065 金融卡帳號 */
    String issueDate;/* 066_073 製卡日 */
    String chipID;/* 074_093 悠遊卡晶片序號 */
    String isamAmount1;/* 094_098 悠遊卡錢包金額 */
    String isamAmount2;/* 099_103 悠遊卡押金 */
    String isamChipID;/* 104_111 ISAM晶片序號 */
    String isamIssueBatchNO;/* 112_121 ISAM製卡批號 */
    String iasmIssueSeqNo;/* 122_126 ISAM製卡序號 */
    String autoloadAmount;/* 127_130 悠遊卡AUTOLOAD金額 */
    String issueName;/* 131_140 卡廠名稱代碼 */
    String issueStatus;/* 141_141 製卡狀態 */
    String cardStatus;/* 142_151 卡片狀態 */
    String issueType;/* 152_152 製卡類別 */
    String name;/* 153_172 持卡人姓名 */
    String autoloadDefSts;/* 173_173 自動加值功能開啟預設值(2: 預設關閉 disable, 4: 預設開啟 enable) */
  ...
  private void splitBuf1(String str) throws UnsupportedEncodingException {
    ...
    readData.id = comc.subMS950String(bytes, 0, 11).trim();
    readData.svcPid = comc.subMS950String(bytes, 11, 16).trim();
    readData.expirDte = comc.subMS950String(bytes, 27, 4).trim();
    ...
    readData.issueDate = comc.subMS950String(bytes, 65, 8).trim();
    readData.chipID = comc.subMS950String(bytes, 73, 20).trim();
    readData.isamAmount1 = comc.subMS950String(bytes, 93, 5).trim();
    readData.isamAmount2 = comc.subMS950String(bytes, 98, 5).trim();
    readData.isamChipID = comc.subMS950String(bytes, 103, 8).trim();
    readData.isamIssueBatchNO = comc.subMS950String(bytes, 111, 10).trim();
    readData.iasmIssueSeqNo = comc.subMS950String(bytes, 121, 5).trim();
    readData.autoloadAmount = comc.subMS950String(bytes, 126, 4).trim();
    readData.issueName = comc.subMS950String(bytes, 130, 10).trim();
    readData.issueStatus = comc.subMS950String(bytes, 140, 1).trim();
    readData.cardStatus = comc.subMS950String(bytes, 141, 10).trim();
    readData.issueType = comc.subMS950String(bytes, 151, 1).trim();
    readData.name = comc.subMS950String(bytes, 152, 20).trim();
    readData.autoloadDefSts = comc.subMS950String(bytes, 172, 1).trim();
//    readData.filler = comc.subBIG5String(bytes, 173, 27).trim();
  }
