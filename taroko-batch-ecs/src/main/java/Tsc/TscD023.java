...
public class TscD023 extends AccessDAO {

    private final String progname = "悠遊VD卡餘額轉置回應檔(DCBP)媒體接收檢核程式  110/01/15 V1.00.00";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscD023 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
            ...
            hTnlgFileName = String.format("DCBP.%8.8s.%8.8s%2.2s", comc.TSCC_BANK_ID8, hTempNotifyDate, fileSeq);
            ...
            String temstr2 = String.format("%s/reports/TSC_D023R1_%s", comc.getECSHOME(), hTempSystemDate);
    ...
    void printHead() throws Exception {
        buf = "";
        buf = comcr.insertStr(buf, "報表名稱: TSC_D023R1", 1);
        ...
        buf = comcr.insertStr(buf,
                "姓名 信用卡號    狀態  悠遊卡號  回饋日期 餘額轉置金額 餘額轉置服務費 公司電話  手機  異動", 01);
        lpar2.add(comcr.putReport(rptName2, rptName2, sysDate, ++rptSeq2, "0", buf));
