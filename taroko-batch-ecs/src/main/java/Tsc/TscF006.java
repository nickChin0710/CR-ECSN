...
/*悠遊卡簽帳資料檔(STMT)媒體產生程式*/
public class TscF006 extends AccessDAO {
    private final String progname = "悠遊卡簽帳資料檔(STMT)媒體產生程式  112/12/19 V1.00.02";
    ...
    String hStmtMediaCreateDate = "";
    String hStmtMediaCreateTime = "";
    String hStmtTscCardNo = "";
    double hStmtDestinationAmt = 0;
    ...
    public int mainProcess(String[] args) throws Exception {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscF006 [notify_date] [force_flag (Y/N)]", "");
            }
            ...
            tmpstr1 = String.format("STMT.%8.8s.%8.8s01", comc.TSCC_BANK_ID8, hTnlgNotifyDate);
    ...
    void selectPtrBusinday() throws Exception {
        ...
        sqlCmd += "to_char(sysdate,'yyyymmdd') h_stmt_media_create_date,";
        sqlCmd += "to_char(sysdate,'hh24miss') h_stmt_media_create_time ";
        ...
        hStmtMediaCreateDate = getValue("h_stmt_media_create_date");
        hStmtMediaCreateTime = getValue("h_stmt_media_create_time");
    }
    ...
    /***********************************************************************/
    void updateTscStmtLoga() throws Exception {
    ...
    void fileOpen() throws Exception {
        ...
        tmpstr1 = String.format("HSTMT%8.8s%8.8s%6.6s%53.53s", comc.TSCC_BANK_ID8, hStmtMediaCreateDate,
                hStmtMediaCreateTime, " ");
    ...
    /***********************************************************************/
    void selectTscStmtLog() throws Exception {
        ...
        openCursor();
        while(fetchTable()) {
            hStmtTscCardNo = getValue("tsc_card_no");
            hStmtDestinationAmt = getValueDouble("dest_amt");
            hStmtFeedbackAmt = getValueDouble("feedback_amt");
            hStmtRowid = getValue("rowid");

            tmpstr1 = String.format("D01%-20.20s%013.0f%44.44s", hStmtTscCardNo
                                    , hStmtDestinationAmt, " ");
            byte[] tmp = comips.commHashUnpack(tmpstr1.getBytes("big5"));
            tmpstr2 = new String(tmp);
            String buf = String.format("%-80.80s%16.16s\r\n", tmpstr1, tmpstr2.toUpperCase());
            writeTextFile(out, buf);
            updateTscStmtLog();
            totalAmt = totalAmt + hStmtDestinationAmt;
            totalFeedbackAmt = totalFeedbackAmt + hStmtFeedbackAmt;
            totalCnt++;
        }
        closeCursor();
    }
    ...
    void updateTscStmtLog() throws Exception {
        ...
        setString(1, hStmtMediaCreateDate);
        setString(2, hStmtMediaCreateTime);
        setString(3, hTnlgNotifyDate);
        setString(4, hTnlgFileName);
        setString(5, javaProgram);
        setRowId(6, hStmtRowid);
        updateTable();
        if (notFound.equals("Y")) {
