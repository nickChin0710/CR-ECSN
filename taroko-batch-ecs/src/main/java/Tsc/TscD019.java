...
/*悠遊VD卡自動加值開啟回饋檔(DCAE)媒體RPT處理程式*/
public class TscD019 extends AccessDAO {

    private final String progname = "悠遊VD卡自動加值開啟回饋檔(DCAE)媒體RPT處理程式  110/01/12 V1.00.01";
...
//            sprintf(tmpstr1, "%s/media/tsc",GetECSHOME());
    ...
    void fileOpen2() throws Exception {
        ...
        while (true) {
            ...
            hDcaeRptRespCode = comc.rtrim(dtl.rptRespCode);
            hDcaeTscCardNo   = comc.rtrim(dtl.tscCardNo);
            hDcaeIcSeqNo     = comc.rtrim(dtl.icSeqNo);

            showLogMessage("I", "", String.format("Line [%08d][%s] resp_code[%s] error!", totalCnt, hDcaeTscCardNo,
                    hDcaeRptRespCode));
