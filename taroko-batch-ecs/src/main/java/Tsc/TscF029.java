...
/*SVCS代行授權交易至發卡機構(STAD)接收程式*/
public class TscF029 extends AccessDAO {

    private final String progname = "SVCS代行授權交易至發卡機構(STAD)接收程式  109/11/17 V1.00.02";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscF029 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
    ...
    int selectTscNotifyLoga() throws Exception {
        ...
        if (hTnlgProcFlag.toCharArray()[0] >= '2') {
            comcr.errRtn(String.format("[%s]代行授權交易檔已處理過,請通知相關人員處理(error) ", hTnlgFileName), "", hCallBatchSeqno);
        }
        if (!hTnlgCheckCode.equals("0000")) {
            showLogMessage("I", "",
                    String.format("[%s]代行授權交易檔整檔處理失敗  , 錯誤代碼[%s] ", hTnlgFileName, hTnlgCheckCode));
            return (1);
        }
    ...
    /***********************************************************************/
    void updatTscNotifyLoga() throws Exception {
    ...
    void fileOpen() throws Exception {
        ...
        while (true) {
            ...
            if (!comc.subMS950String(str600.getBytes("MS950"), 134, 134+16).equals(tmpstr2)) {
                hOrgdRptRespCode = "0205";
    ...
    int selectTscCard() throws Exception {
        ...
        int recordCnt = selectTable();
        if (recordCnt > 0) {
            hStadTscCardNo = getValue("tsc_card_no");
    ...
    /***********************************************************************/
    void initTscStad() throws Exception {
        hStadCrtDate = "";
        hStadCrtTime = "";
        hStadTscCardNo = "";
        hStadProcessCode = "";
