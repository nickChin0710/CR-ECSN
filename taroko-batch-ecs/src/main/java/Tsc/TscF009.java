...
*  ---------  --------- ----------- ----------------------------------------  *
*  107/06/20  V1.00.00    Brian     program initial                           *
...
/*悠遊卡自動加值開啟回饋檔(AECF)媒體產生程式*/
public class TscF009 extends AccessDAO {
    private final String progname = "悠遊卡自動加值開啟回饋檔(AECF)媒體產生程式   112/12/19 V1.00.02";
    ...
    String hAecfCardNo            = "";
    String hAecfVendorEmbossDate = "";
    String hAecfIcSeqNo          = "";
    String hAecfBirthday           = "";
    String hAecfRowid              = "";
    String tmpstr                    = "";
    String hTempX16                = "";
    String tempX04                  = "";
    ...
    public int mainProcess(String[] args) throws Exception {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscF009 [notify_date] [force_flag (Y/N)]", "");
            }
            ...
            selectPtrBusinday();
            tmpstr1 = String.format("AECF.%8.8s.%8.8s01", comc.TSCC_BANK_ID8, hTnlgNotifyDate);
    ...
    /***********************************************************************/
    void updateTscAecfLoga() throws Exception {
    ...
    void fileOpen() throws Exception {
        ...
        tmpstr1 = String.format("HAECF%8.8s%8.8s%6.6s%61.61s", comc.TSCC_BANK_ID8, hTbigMediaCreateDate, hTbigMediaCreateTime, " ");
    ...
    /***********************************************************************/
    void selectTscAecfLog() throws Exception {
        ...
        sqlCmd += "   FROM tsc_card b,tsc_aecf_log a ";
        sqlCmd += "  WHERE a.tsc_card_no     = b.tsc_card_no ";
        sqlCmd += "    and a.proc_flag       = 'N' ";
        sqlCmd += "    and b.autoload_flag   = 'Y' ";
        int recordCnt = selectTable();
        for (int i = 0; i < recordCnt; i++) {
            hAecfTscCardNo = getValue("tsc_card_no", i);
            hAecfCardNo = getValue("card_no", i);
            hAecfVendorEmbossDate = getValue("vendor_emboss_date", i);
            hAecfIcSeqNo = getValue("ic_seq_no", i);
            hAecfBirthday = getValue("birthday", i);
            hAecfRowid = getValue("rowid", i);
...
//        char    endflag[3]="\x0D\x0A";
        ...
        tmpstr = String.format("%-20.20s", hAecfTscCardNo);
        detailSt.tscCashNo = tmpstr;

        hTempX16 = hAecfTscCardNo;
        tempX04 = "";
        ...
        tmpstr = String.format("%-20.20s", hAecfIcSeqNo);
        ...
        tmpstr = String.format("%-8.8s", hAecfVendorEmbossDate);
        detailSt.embossDate = tmpstr;

        tmpstr = String.format("%-8.8s", hAecfBirthday);
