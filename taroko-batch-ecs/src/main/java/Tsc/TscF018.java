...
/*拒授權名單檔(BKTI)RPT處理*/
public class TscF018 extends AccessDAO {

    private final String progname = "拒授權名單檔(BKTI)RPT處理  110/01/11 V1.00.03";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscF018 [notify_date [force_flag]]", "");
            }
    ...
    void selectTscNotifyLog() throws Exception {
        ...
        sqlCmd += "where proc_flag = decode(?,'Y',proc_flag,'1') ";
        sqlCmd += "  and file_iden = 'BKTI' ";
    ...
    int fileOpen1() throws Exception {
        ...
        while (true) {
            ...
            tmpstr1 = comc.subBIG5String(str600.getBytes("big5"), 0, 80);
    ...
    void fileOpen2() throws Exception {
        ...
        while (true) {
            ...
            hTbigRptRespCode = comc.rtrim(dtl.rptRespCode);
            hTbigTscCardNo = comc.rtrim(dtl.tscCardNo);

            showLogMessage("I", "", String.format("Line [%08d][%s] resp_code[%s] error!", totalCnt, hTbigTscCardNo,
                    hTbigRptRespCode));
    ...
    /***********************************************************************/
    void updateTscRmActauth1() throws Exception {
        daoTable = "tsc_rm_actauth";
        updateSQL = "rpt_resp_code = ?,";
        updateSQL += " mod_time      = sysdate,";
        updateSQL += " mod_pgm       = 'Tsc0312'";
        whereStr = "where retr_ref_no_1 = ? ";
        setString(1, hTbigRptRespCode);
        setString(2, hTbigRetrRefNo);
        updateTable();
        if (notFound.equals("Y")) {
    ...
    /***********************************************************************/
    void updateTscRmActauth2() throws Exception {
        daoTable = "tsc_rm_actauth";
        updateSQL = " rpt_resp_code = ?,";
        updateSQL += " mod_time      = sysdate,";
        updateSQL += " mod_pgm       = 'Tsc0312'";
        whereStr = "where retr_ref_no_2 = ? ";
        setString(1, hTbigRptRespCode);
        setString(2, hTbigRetrRefNo);
        updateTable();
        if (notFound.equals("Y")) {
    ...
    /***********************************************************************/
    void updateTscBktiHst() throws Exception {
        daoTable = "tsc_bkti_hst";
        updateSQL = "rpt_resp_code   = ?,";
        updateSQL += " rpt_resp_date  = to_char(sysdate, 'yyyymmdd'),";
        updateSQL += " rpt_resp_time  = to_char(sysdate, 'hh24miss'),";
        updateSQL += " mod_time       = sysdate,";
        updateSQL += " mod_pgm        = ?";
        whereStr = "where retr_ref_no = ? ";
        setString(1, hTbigRptRespCode);
        setString(2, javaProgram);
        setString(3, hTbigRetrRefNo);
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_tsc_bkti_hst not found!", "", hCallBatchSeqno);
        }
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl.tscCardNo = comc.subMS950String(bytes, 11, 20);
        dtl.effcYymm = comc.subMS950String(bytes, 31, 4);
        dtl.crtDate = comc.subMS950String(bytes, 35, 8);
        dtl.crtTime = comc.subMS950String(bytes, 43, 6);
        dtl.riskClass = comc.subMS950String(bytes, 49, 2);
        dtl.filler1i = comc.subMS950String(bytes, 51, 36);
        dtl.hashValue = comc.subMS950String(bytes, 87, 16);
        dtl.fillerEnd = comc.subMS950String(bytes, 103, 2);
    }
