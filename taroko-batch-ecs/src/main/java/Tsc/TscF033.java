...
*  110/01/13  V1.00.04    Wilson    mark ecs_crd_card                         *
*  112/05/09  V1.00.05    Wilson    mark update_tsc_notify_log                *
******************************************************************************/
...
/*悠遊卡餘額轉置回應檔(BTRP)接收處理*/
public class TscF033 extends AccessDAO {
    private final String progname = "悠遊卡餘額轉置回應檔(BTRP)接收處理  112/05/09 V1.00.05";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscF033 [[notify_date][force_flag]] [force_flag][seq]", "");
            }
            ...
            tmpstr1 = String.format("BTRP.%8.8s.%8.8s%2.2s", comc.TSCC_BANK_ID8, hTempNotifyDate, fileSeq);
      ...
      //    lp_rtn("TSC_F023R1");
            ...
            String temstr3 = String.format("%s/reports/TSC_F033R1_%s", comc.getECSHOME(), hTempSystemDate);
            if (lpar2.size() > 0) {
            	;
...
//        }
    ...
    void fileOpen() throws Exception {
        ...
        /*
        String temstr2 = String.format("%s/reports/TSC_F033R1_%s", comc.getECSHOME(), hTempSystemDate);
...
//            if (selectCrdCard() != 0 && (selectEcsCrdCard() != 0)) {
            ...
            /*************************************************************************/
            hCgecPurchaseDate = hBusiBusinessDate;  //放營業日
...
//                    }
    ...
    void printHead() throws Exception {
        buf = "";
        buf = comcr.insertStr(buf, "報表名稱: TSC_F033R1", 1);
        ...
        buf = "";
        buf = comcr.insertStr(buf, "\"姓      名 信用卡號         狀態 悠遊卡號         通知日期 餘額轉置金額 餘額轉置服務費 公司電話         手機", 01);
        lpar2.add(comcr.putReport(rptName2, rptName2, sysDate, ++rptSeq2, "0", buf));
...
//    }
    ...
    void insertTscCgecAll() throws Exception {
        ...
        insertTable();
        if (dupRecord.equals("Y")) {
            comcr.errRtn("insert_tsc_cgec_all duplicate!", hCgecBatchNo, hCallBatchSeqno);
        }
    ...
    /***********************************************************************/
    void insertBilSysexp1() throws Exception {
        if (hTardBalanceRtnFee > 0) {
            hCardAcctType = "";
            hCardPSeqno = "";
            sqlCmd = "select c.acct_type ";
            sqlCmd += ", a.acct_key ";
            sqlCmd += " from crd_card c, act_acno a ";
...
//                }
            ...
            setValue("dest_curr"    , "901");
            setValueInt("src_amt"   , hTardBalanceRtnFee);
            setValue("CURR_CODE"    , "901");
            setValueInt("DC_DEST_AMT", hTardBalanceRtnFee);
            setValue("mod_time", sysDate + sysTime);
            setValue("mod_pgm", javaProgram);
            daoTable = "bil_sysexp";
            insertTable();
