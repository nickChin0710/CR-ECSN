...
*  112/08/13  V1.00.04    Wilson    收不到檔不當掉(營業日才會有檔)                                                             *
******************************************************************************/
...
/*發卡撥款檔(FNTI)媒體接收處理程式*/
public class TscF027 extends AccessDAO {

    private final String progname = "發卡撥款檔(FNTI)媒體接收處理程式  112/08/13 V1.00.04";
    ...
    String rptName2 = "發卡撥款檔明細表";
    String rptId2 = "TSC_F027R1";
    int rptSeq2 = 0;
    String buf = "";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscF027 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
            ...
            tmpstr1 = String.format("FNTI.%8.8s.%8.8s%2.2s", comc.TSCC_BANK_ID8, hTempNotifyDate, fileSeq);
            ...
            String ftpName = String.format("%s.%s_%s", "FNTI", sysDate, hBusiBusinessDate);
            String filename = String.format("%s/reports/%s.%s_%s", comc.getECSHOME(), "FNTI", sysDate, hBusiBusinessDate);
    ...
    void printHead() throws Exception {
        buf = "";
        buf = comcr.insertStr(buf, "報表名稱: TSC_F027R1", 1);
        String szTmp = String.format("%22s", "發卡撥款檔明細表");
