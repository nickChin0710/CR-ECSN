...
public class TscD03b extends AccessDAO {
    private final String progname = "悠遊VD卡掛失贖回餘額交易資料檔(DCDE)媒體接收處理程式  110/01/22 V1.00.00";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscD03b [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
    ...
    int selectTscCard() throws Exception {
        ...
        sqlCmd += "a.balance_date, ";
        sqlCmd += "b.id_p_seqno ";
        sqlCmd += "from dbc_card b ,tsc_vd_card a ";
        sqlCmd += "where tsc_card_no = ? ";
        sqlCmd += "and b.card_no = a.vd_card_no ";
