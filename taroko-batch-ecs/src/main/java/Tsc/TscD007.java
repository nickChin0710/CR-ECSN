...
*  112/05/03  V1.00.01    Wilson      DCPF change to DCPS                     *
*  112/09/01  V1.00.02    Wilson      修正hash值有誤問題                                                                         *
...
public class TscD007 extends AccessDAO {
    private final String progname = "悠遊VD卡卡人資料檔(DCPS)媒體產生程式   112/12/19 V1.00.03";
    ...
    String hDcpfMediaCreateDate = "";
    String hDcpfMediaCreateTime = "";
    ...
    public int mainProcess(String[] args) throws Exception {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscD007 [notify_date] [force_flag (Y/N)]", "");
            }
            ...
            selectPtrBusinday();
            tmpstr1 = String.format("DCPS.%8.8s.%8.8s01", comc.TSCC_BANK_ID8, hTnlgNotifyDate);
    ...
    void selectPtrBusinday() throws Exception {
        ...
        sqlCmd += "to_char(sysdate,'yyyymmdd') h_dcpf_media_create_date,";
        sqlCmd += "to_char(sysdate,'hh24miss') h_dcpf_media_create_time ";
        ...
        hDcpfMediaCreateDate = getValue("h_dcpf_media_create_date");
        hDcpfMediaCreateTime = getValue("h_dcpf_media_create_time");
    }
    ...
    /***********************************************************************/
    void updateTscDcpfLoga() throws Exception {
    ...
    void fileOpen() throws Exception {
        ...
        tmpstr1 = String.format("HDCPS%8.8s%8.8s%6.6s%445.445s", comc.TSCC_BANK_ID8, hDcpfMediaCreateDate, hDcpfMediaCreateTime, " ");
    ...
    /***********************************************************************/
    void selectTscDcpfLog() throws Exception {
        ...
        sqlCmd += "from dba_acno d, dbc_idno c, dbc_card b,tsc_dcpf_log a,tsc_vd_card e ";
        sqlCmd += "where b.card_no = a.card_no ";
        sqlCmd += "and d.p_seqno = b.p_seqno ";
        sqlCmd += "and c.id_p_seqno = b.id_p_seqno ";
        sqlCmd += "and a.tsc_card_no = e.tsc_card_no ";
        sqlCmd += "and a.proc_flag = 'N' ";
        openCursor();
        while (fetchTable()) {
            hDcpfTscCardNo = getValue("tsc_card_no");
            hDcpfCardNo = getValue("card_no");
            hDcpfTxType = getValue("tx_type");
            hDcpfTxRsn = getValue("tx_rsn");
            hDcpfId = getValue("id_no");
    ...
    void writeRtn() throws Exception {
        ...
        tmpstr = String.format("%-1.1s", hDcpfTxType);
        detailSt.txType = tmpstr;

        tmpstr = String.format("%-1.1s", hDcpfTxRsn);
        detailSt.txRsn = tmpstr;

        tmpstr = String.format("%-20.20s", hDcpfTscCardNo);
        detailSt.tscCashNo = tmpstr;

        hTempX16 = hDcpfTscCardNo;
        tempX04 = "";
        tempX01 = "";
        ...
        tmpstr = String.format("%-20.20s", hDcpfId);
        detailSt.id = tmpstr;
    ...
    void updateTscDcpfLog() throws Exception {
        ...
        setString(1, hDcpfMediaCreateDate);
        setString(2, hDcpfMediaCreateTime);
        setString(3, hTnlgNotifyDate);
        setString(4, hTnlgFileName);
        setString(5, javaProgram);
        setRowId(6, hDcpfRowid);
        updateTable();
        if (notFound.equals("Y")) {
            comcr.errRtn("update_tsc_dcpf_log not found!", "", hCallBatchSeqno);
        }
