...
public class TscF032 extends AccessDAO {
  private boolean debug = false;

  private final String progname = "悠遊卡退卡資料檔(RTEC)媒體接收處理程式 111/02/14 V1.00.08";
...
//        comc.errExit("Usage : TscF032 [[notify_date][force_flag]] [force_flag][seq]", "");
//      }
      ...
      /*
       * if(force_flag==0) { if(select_tsc_notify_log_a()!=0) { update_tsc_notify_log_a();
       * backup_rtn(); db_commit(); return(0); } }
       */
  ...
  void fileOpen() throws Exception {
    ...
    while (true) {
      ...
      if (hOrgdRptRespCode.equals("0000")) {
        ...
        /*** 若為開啟自動加值的非記名卡 ***/
        // 第二階段上線須解除mark
        // if (hTard_autoload_flag.equals("Y") &&
        // hTard_tsc_sign_flag.equals("N")) {
        // /*** 查詢已回饋基金金額 ***/
        // select_tsc_nsign_loan_hst();
        // /*** 若餘轉與基金相等 且 基金回饋大於0 ***/
        // if (h_sign.equals("-") == false &&
        // hTnlh_remain_amt == hCgec_destination_amt &&
        // hTnlh_prebalance_amt > 0) {
        // /*** 加檔 寫入正向費用 ***/
        // insert_bil_sysexp();
        // /*** 已餘額轉置 已線上加檔 ***/
        // hTnlh_process_status = "3";
        // update_tsc_nsign_loan_hst();
        // } else if(hTnlh_prebalance_amt > 0) {
        // if (hTnlh_prebalance_amt > hCgec_destination_amt) {
        // hTnlh_prebalance_amt = hCgec_destination_amt;
        //
        // //起 P-61 分錄(reference bil_a008) 金額為 (hTnlh_prebalance_amt - hCgec_destination_amt)
        // vouch_rtn();
        // }
        //
        // if (hTnlh_prebalance_amt > 0) insert_bil_sysexp();
        //
        // hTnlh_process_status = "5";
        // update_tsc_nsign_loan_hst();
        // }
        // }
      }
  ...
  void selectTscNsignLoanHst() throws Exception {
    ...
    sqlCmd +=
        " (select chi_name from crd_idno where id_p_seqno = t.major_id_p_seqno) major_chi_name,";
  ...
  void insertBilSysexp() throws Exception {
    ...
    setValue("bill_desc",
        "沖正"
            + comc.getSubString(hBusiBusinessDate, 4, 4 + 2)
            + "/"
            + comc.getSubString(hBusiBusinessDate, 6, 6 + 2)
            + "悠遊卡退費");
  ...
  void selectBilPostcntl() throws Exception {
    hTempBatchSeq = "";
    sqlCmd =
        "select substr( to_char( decode( max(batch_seq), 0, 0, max(batch_seq))+1, '0000'), 2, 4) hTemp_batch_seq ";
