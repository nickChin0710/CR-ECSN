...
/*餘額轉置通知檔(BTRQ)媒體RPT處理程式*/
public class TscF015 extends AccessDAO {

    private final String progname = "餘額轉置通知檔(BTRQ)媒體RPT處理程式  110/01/11 V1.00.03";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscF015 [notify_date [force_flag]]", "");
            }
    ...
    void fileOpen2() throws Exception {
        ...
        while (true) {
            ...
            hBtrqRptRespCode = comc.rtrim(dtl.rptRespCode);
            hBtrqTscCardNo = comc.rtrim(dtl.tscCardNo);

            showLogMessage("I", "", String.format("Line [%08d][%s] resp_code[%s] error!", totalCnt, hBtrqTscCardNo,
                    hBtrqRptRespCode));
    ...
    void splitBuf1(String str) throws UnsupportedEncodingException {
        ...
        dtl.tscCardNo = comc.subMS950String(bytes, 10, 20);
        dtl.amt = comc.subMS950String(bytes, 30, 13);
        dtl.amtBal = comc.subMS950String(bytes, 43, 13);
        dtl.filler2 = comc.subMS950String(bytes, 56, 31);
        dtl.hashValue = comc.subMS950String(bytes, 87, 16);
        dtl.fillerEnd = comc.subMS950String(bytes, 103, 200);
    }
