...
/*悠遊卡卡號資料檔(CDPS)媒體RPT處理程式*/
public class TscF017 extends AccessDAO {

    private final String progname = "悠遊卡卡號資料檔(CDPS)媒體RPT處理程式  110/01/11 V1.00.03";
    ...
    void fileOpen2() throws Exception {
        ...
        while (true) {
            ...
            hCdpfRptRespCode = comc.rtrim(dtl.rptRespCode);
            hCdpfTscCardNo = comc.rtrim(dtl.tscCashNo);

            showLogMessage("I", "", String.format("Line [%08d][%s] resp_code[%s] error!", totalCnt, hCdpfTscCardNo,
                    hCdpfRptRespCode));
