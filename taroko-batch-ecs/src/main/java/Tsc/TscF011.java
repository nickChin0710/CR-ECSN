...
*  110/01/11  V1.00.02    Wilson    修正bug                                    *
*                                                                             *
******************************************************************************/
...
/*悠遊卡製卡回饋檔(CDRP)媒體RPT處理程式*/
public class TscF011 extends AccessDAO {
    private final String progname = "悠遊卡製卡回饋檔(CDRP)媒體RPT處理程式   110/01/11 V1.00.02";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : TscF011 [notify_date] [force_flag (Y/N)]", "");
            }
    ...
    int fileOpen1() throws Exception {
        ...
        while (true) {
            ...
            if (str600.substring(0, 1).equals("H")) {
                ...
                headTag = 1;
                if (!hTnlgFileIden.substring(0, 4).equals(comc.subBIG5String(str600.getBytes("big5"), 1, 1+4))) {
                    hTnlgCheckCode = "0114";/* HEADER_FILE_NAME_OVER */
                    updateTscNotifyLog();
                    return (1);
                }
                if (!comc.TSCC_BANK_ID8.equals(comc.subBIG5String(str600.getBytes("big5"), 5, 5+8))) {
                    hTnlgCheckCode = "0117";
                    updateTscNotifyLog();
                    return (1);
                }
                tmpstr1 = String.format("%8.8s", comc.subBIG5String(str600.getBytes("big5"), 13, 13+8));
                ...
                tmpstr1 = String.format("%6.6s", comc.subBIG5String(str600.getBytes("big5"), 21, 21+6));
                ...
                if (comc.subBIG5String(str600.getBytes("big5"), 27, 27+4).equals("0000") == false) {
                    hTnlgCheckCode = "B001";/* 與通知檔錯誤代碼不一致 */
                    updateTscNotifyLog();
                    return (1);
                }

                tmpstr1 = String.format("%8.8s", comc.subBIG5String(str600.getBytes("big5"), 31, 31+8));
                if (comcr.str2long(tmpstr1) != hTnlgRecordSucc) {
                    hTnlgCheckCode = "B002";
                    updateTscNotifyLog();
                    return (1);
                }
                tmpstr1 = String.format("%8.8s", comc.subBIG5String(str600.getBytes("big5"), 39, 39+8));
    ...
    void fileOpen2() throws Exception {
        ...
        while (true) {
            ...
            hCdrpRptRespCode = comc.rtrim(dtl.rptRespCode);
            hCdrpTscCardNo = comc.rtrim(dtl.tscCardNo);
            hCdrpIcSeqNo = comc.rtrim(dtl.icSeqNo);

            showLogMessage("I", "", String.format("Line [%08d][%s] resp_code[%s] error!", totalCnt, hCdrpTscCardNo,
                    hCdrpRptRespCode));
