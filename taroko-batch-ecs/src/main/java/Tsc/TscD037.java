...
/*VD悠遊卡現金加值交易檔(DCCH)媒體接收處理程式*/
public class TscD037 extends AccessDAO {

    private final String progname = "VD悠遊卡現金加值交易檔(DCCH)媒體接收處理程式  110/01/08 V1.00.03";
    ...
    String rptName2 = "VD悠遊卡現金加值明細表";
    String rptId2 = "TSC_D037R1";
    int rptSeq2 = 0;
    String buf = "";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : TscD037 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
            ...
            tmpstr1 = String.format("DCCH.%8.8s.%8.8s%2.2s", comc.TSCC_BANK_ID8, hTempNotifyDate, fileSeq);
            ...
            String ftpName = String.format("%s.%s_%s", "DCCH", sysDate, hBusiBusinessDate);
            String filename = String.format("%s/reports/%s.%s_%s", comc.getECSHOME(), "DCCH", sysDate, hBusiBusinessDate);
    ...
    void printHead() throws Exception {
        buf = "";
        buf = comcr.insertStr(buf, "報表名稱: TSC_D037R1", 1);
        String szTmp = String.format("%22s", "VD悠遊卡現金加值明細表");
