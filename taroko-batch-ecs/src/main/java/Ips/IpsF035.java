...
*  112/07/10  V1.00.02    JeffKung  Daily post the return amount              *
******************************************************************************/
...
/*每日卡片退費紀錄檔(I2B005)處理*/
public class IpsF035 extends AccessDAO {
    private String progname = "每日卡片退費紀錄檔(I2B005)處理  112/07/10 V1.00.02";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2 && args.length != 3) {
                comc.errExit("Usage : IpsF035 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
            }
            ...
            tmpstr1 = String.format("I2B005_%4.4s%8.8s%2.2s.dat", comc.IPS_BANK_ID4, hTempNotifyDate, fileSeq);
...
//            tmpstr1 = String.format("%010.0f", h_orgd_tscc_data_seqno);
    ...
    /***********************************************************************/
    void insertIpsI2b005Log() throws Exception {
        ...
        setValue("mod_time", sysDate + sysTime);
        setValue("mod_pgm", javaProgram);
        daoTable = "ips_i2b005_log";
        insertTable();
        if (dupRecord.equals("Y")) {
            comcr.errRtn("insert_ips_i2b005_log duplicate!", "", hCallBatchSeqno);
        }
