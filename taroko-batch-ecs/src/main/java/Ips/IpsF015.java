 ...
 *  ---------  --------- ----------- ----------------------------------------  *
* 107/04/12  V1.01.01  詹曜維     RECS-s1070103-001 新增個人資料變動檔       *
* 107/04/12  V1.01.01  Brian   transfer to java       *
...
/*聯名卡卡片個人資料變動檔(B2I005)媒體處理*/
public class IpsF015 extends AccessDAO {
    private String progname = "聯名卡卡片個人資料變動檔(B2I005)媒體處理 109/12/15 V1.01.02";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IpsF015 [notify_date [force_flag]]", "");
            }
    ...
    void fileOpenAck() throws Exception {
        ...
        if(f == -1) {
            swAck = "N";
            /*
             * ECSprintf(stderr,"找不到回覆檔[%s], 請通知相關人員處理(error)\n",temstr1);
             */
            return;
        }
    ...
    void insertIpsFileResp() throws Exception {
        ...
        extendField = "ipfr.";
        setValue(extendField+"send_date", hTnlgFtpSendDate);
        setValue(extendField+"file_code", hTempFileName.substring(0, 6));
        setValue(extendField+"file_name", hTempFileName);
        setValue(extendField+"file_type", hRetmFileType);
        setValue(extendField+"proc_date", hTnlgFtpReceiveDate);
        setValue(extendField+"proc_time", hTnlgFtpReceiveTime);
        setValue(extendField+"ng_err_code", hErrType);
        setValueInt(extendField+"err_tot_cnt", hRecordCnt);
        setValueInt(extendField+"err_ok_cnt", hRecordSucc);
        setValueInt(extendField+"err_err_cnt", hRecordFail);
    ...
    void insertIpsFileErrdata() throws Exception {
        ...
        setValue(extendField+"file_code", hTempFileName.substring(0, 6));
        setValue(extendField+"file_name", hTempFileName);
        setValue(extendField+"card_ic_no", hRetdCardIcNo);
        setValue(extendField+"ips_card_no", hRetdIpsCardNo);
        setValue(extendField+"err_code", hRetdErrCode);
        setValue(extendField+"mod_time", sysDate + sysTime);
        setValue(extendField+"mod_pgm", javaProgram);
    ...
    class Buf3 {
        ...
        String allText() throws UnsupportedEncodingException {
            ...
            rtn += fixLeft(errCode, 2);
            rtn += fixLeft(filler1, 28);
            rtn += fixLeft(fillerEnd, 2);
            return rtn;
        }
