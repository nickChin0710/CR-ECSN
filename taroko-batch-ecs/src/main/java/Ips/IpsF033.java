...
public class IpsF033 extends AccessDAO {
  private final String progname = "鎖退卡暨關閉自動加值檔(I2B003)處理  109/10/30 V1.00.07";
...
//        comc.errExit("Usage : IpsF033 [[notify_date][fo1yy_flag]] [force_flag][seq(nn)]", "");
//      }
...
//      hAdd2SystemDate = getValue("h_add2_system_date");
  ...
  int updateIpsCard() throws Exception {
    ...
    updateSQL +=
        " autoload_clo_date = decode(cast(? as varchar(10)) , '4F', cast(? as varchar(10)) , autoload_clo_date) ,";
    updateSQL += " return_flag  = decode(cast(? as varchar(10)) , '51', 'Y', return_flag) ,";
    updateSQL +=
        " return_date  = decode(cast(? as varchar(10)) , '51', cast(? as varchar(10)) , return_date) ,";
    updateSQL += " lock_flag   = decode(cast(? as varchar(10)) , '91', 'Y', lock_flag) ,";
    updateSQL +=
        " lock_date   = decode(cast(? as varchar(10)) , '91', cast(? as varchar(10)) , lock_date)";
    ...
    setString(6, hI2b3TxnDate);
    setString(7, hI2b3TxnType);
    setString(8, hI2b3TxnType);
    setString(9, hI2b3TxnDate);
    setString(10, hI2b3IpsCardNo);
    updateTable();
    if (notFound.equals("Y")) {
      return 1;
    }
