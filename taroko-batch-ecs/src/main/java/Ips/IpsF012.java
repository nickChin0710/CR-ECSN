...
/*關閉自動加值功\能卡號檔(B2I002)媒體處理*/
public class IpsF012 extends AccessDAO {
    private String progname = "關閉自動加值功\\能卡號檔(B2I002)媒體處理 109/12/15 V1.00.01";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length != 0 && args.length != 1 && args.length != 2) {
                comc.errExit("Usage : IpsF012 [notify_date [force_flag]]", "");
            }
    ...
    void selectIpsNotifyLog() throws Exception {
        ...
        while (fetchTable()) {
            ...
            if (!hTnlgCheckCode.equals("0000")) {
                updateIpsNotifyLog();
                showLogMessage("I", "",
                        String.format("[%s] 整檔處理失敗, 錯誤代碼[%s](error)!", hTnlgFileName, hTnlgCheckCode));
                continue;
            }
            fileOpen1();
    ...
    void fileOpenAck() throws Exception {
        ...
        tempFileName = String.format("%-21.21s.zip.ack", hTnlgFileName);
//        String temstr1 = String.format("%s/media/ips/%s", comc.GetECSHOME());//phopho mod
        String temstr1 = String.format("%s/media/ips/%s", comc.getECSHOME(), tempFileName);
        temstr1 = Normalizer.normalize(temstr1, java.text.Normalizer.Form.NFKD);
        swAck = "Y";
        int f = openInputText(temstr1);
