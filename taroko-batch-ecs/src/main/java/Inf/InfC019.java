...
*  ---------  --------- ----------- ---------------------------------------- *
*  110/08/12  V1.00.00   Castor     program initial                          *
*                                                                            *
*****************************************************************************/
package Inf;
...
public class InfC019 extends AccessDAO {
  private final String progname = "產生送CRDB 19更新委扣帳號資料檔程式 110/08/12 V1.00.00";
  private String prgmId = "InfC019";
  ...
  private final String fileName = "CRU23B1_TYPE_19_";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      List<ChgObj019> chgObjList = findChgObjList(searchDate);
		...
		// open CRU23B1_TYPE_19_yyyymmdd.txt file
   ...
   * @throws Exception
   */
  private List<ChgObj019> findChgObjList(String searchDate) throws Exception {
    System.out.print(searchDate);

	sqlCmd = " SELECT substr(trim(AUTOPAY_ACCT_NO),4,13) AUTOPAY_ACCT_NO, ";
    ...
    List<ChgObj019> chgObjList = new LinkedList<ChgObj019>();
    ChgObj019 chgObj = null;

    for (int i = 0; i < selectCount; i++) {
    	
      chgObj = new ChgObj019();
   ...
   */
  private String getOutputString(List<ChgObj019> chgObjList) throws UnsupportedEncodingException {
	byte[] lineSeparatorArr = {'\r','\n'}; //0D0A
	String nextLine = new String(lineSeparatorArr,"MS950");
	
    StringBuilder sb = new StringBuilder();

    for (ChgObj019 chgObj : chgObjList) {
      ...
      sb.append(comc.fixLeft(chgObj.autopayAcctNo, 13));
