...
*  ---------  --------- ----------- ---------------------------------------- *
*  110/08/03  V1.00.00   Castor     program initial                          *
*  110/08/20  V1.00.02   SunnyTs    將mainProcess private改 public            *
*  110/08/25  V1.00.03   SunnyTs    調整sql寫法，使執行結果能明確印出table Name     *
*  110/08/25  V1.00.04   SunnyTs    調整檢核參數日期及改用getProgDate             *
*  112/04/20  V1.00.05   Ryan     【指定參數日期 or執行日期 (如searchDate)】-1。                                                     *	
*****************************************************************************/
package Inf;
...
public class InfC002 extends AccessDAO {
  private final String progname = "產生送CRDB 02 刪除舊ID下的CAASEG的程式 112/04/20 V1.00.05";
  private String prgmId = "InfC002";
  ...
  private final String fileName = "CRU23B1_TYPE_02_";
...
//      }else
        ...
        List<ChgObj002> chgObjList = findChgObjList(searchDate);
		...
		// open CRU23B1_TYPE_02_yyyymmdd.txt file
   ...
   * @throws Exception
   */
  private List<ChgObj002> findChgObjList(String searchDate) throws Exception {
    System.out.print(searchDate);

// sunny mark 先暫時保留，如取消VD卡時，能調整使用。
//	sqlCmd = " SELECT CARD_NO,OLD_ID_NO,ID_NO ";
//	sqlCmd += " FROM ( ";
//	sqlCmd += " SELECT b.CARD_NO,c.OLD_ID_NO,c.ID_NO,c.MOD_TIME ";
//	sqlCmd += " FROM crd_idno a,crd_card b, crd_chg_id c ";
//	sqlCmd += " WHERE a.ID_P_SEQNO=b.MAJOR_ID_P_SEQNO ";  // 只篩選正卡
//	sqlCmd += " AND a.ID_NO=c.ID_NO ";
//	sqlCmd += " AND c.crt_date= ? ";
//	sqlCmd += " UNION ALL ";
//	sqlCmd += " SELECT b.CARD_NO,c.ID old_id_no,c.AFT_ID ID_NO,c.MOD_TIME ";
//	sqlCmd += " FROM dbc_idno a,dbc_card b, dbc_chg_id c ";
//	sqlCmd += " WHERE a.ID_P_SEQNO=b.MAJOR_ID_P_SEQNO ";  // 只篩選正卡
//	sqlCmd += " AND a.ID_NO=c.AFT_ID ";
//	sqlCmd += " AND c.crt_date= ? ) ";
//	sqlCmd += " ORDER BY ID_NO,MOD_TIME ";
	
	sqlCmd = " 	WITH crd_chg_id1 AS";
	sqlCmd += " (   SELECT b.CARD_NO,c.OLD_ID_NO,c.ID_NO,c.MOD_TIME";
	sqlCmd += " 	FROM crd_idno a,crd_card b, crd_chg_id c";
	sqlCmd += " 	WHERE a.ID_P_SEQNO=b.MAJOR_ID_P_SEQNO";  //只篩選正卡
	sqlCmd += " 	AND a.ID_NO=c.ID_NO";
	...
	sqlCmd += "     AND c.crt_date= ? ";
	sqlCmd += " )";
	sqlCmd += " SELECT * FROM crd_chg_id1";
	sqlCmd += " ORDER BY ID_NO,MOD_TIME";
    ...
    List<ChgObj002> chgObjList = new LinkedList<ChgObj002>();
    ChgObj002 chgObj = null;

    for (int i = 0; i < selectCount; i++) {
    	
      chgObj = new ChgObj002();
      chgObj.cardNo = getValue("CARD_NO", i);
      chgObj.oldIdNo = getValue("OLD_ID_NO", i);
   ...
   */
  private String getOutputString(List<ChgObj002> chgObjList) throws UnsupportedEncodingException {
	byte[] lineSeparatorArr = {'\r','\n'}; //0D0A
	String nextLine = new String(lineSeparatorArr,"MS950");
	
    StringBuilder sb = new StringBuilder();

    for (ChgObj002 chgObj : chgObjList) {
