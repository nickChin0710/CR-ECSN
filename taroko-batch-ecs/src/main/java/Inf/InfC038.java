...
public class InfC038 extends BaseBatch {
private final String progname = "產生送CRDB 38 ORG506額度異動  2023-1206 V1.00.07";
CommCrd comc = new CommCrd();
CommFTP commFTP = null;
CommRoutine comr = null;
String isFileName = "";
private int ilFile38;
String hIdPSeqno = "";
String isProcDate = "";
String hIdNo = "";
String hOppoStatus = "";
...
protected void dataProcess(String[] args) throws Exception {
   ...
   if (liArg > 1) {
      printf("Usage : InfC038 [proc_date]");
      okExit(0);
   }
   ...
   isFileName = "CRU23B1_TYPE_38_"+isProcDate+".txt";
   printf("Process: proc_date[%s], file_name[%s]", isProcDate,isFileName);

   checkOpen();
   selectDataType38();
   closeOutputText(ilFile38);
...
void selectDataType38() throws Exception {
   fetchExtend = "type38.";
   sqlCmd =" select distinct card_no, acno_p_seqno "
       ...
       +" and acct_type ='05' "
   ;
//   sqlCmd += " group by card_no , acno_p_seqno ";

   setString(1, isProcDate);
   openCursor();
   while (fetchTable()) {
      initData();
      hCardNo = colSs("type38.card_no");
      hAcnoPSeqno = colSs("type38.acno_p_seqno");
   ...
   while (fetchTable()) {
      initData();
      hIdNo = colSs("type381.id_no");
      hAftAmt = colInt("type381.tot_amt_month");
      hAcnoPSeqno = colSs("type381.acno_p_seqno");
   ...
   while (fetchTable()) {
      initData();
      hIdPSeqno = colSs("type382.id_p_seqno");
      hAcnoPSeqno = colSs("type382.acno_p_seqno");
      checkAcctType(hAcnoPSeqno);
      if (hContinue)
         continue;
...
//   sqlCmd += " group by card_acct_idx ";
   ...
   while (fetchTable()) {
      initData();
      hCardAcctIdx = colInt("type383.card_acct_idx");
...
void writeTextFile38() throws Exception {
   StringBuffer tempBuf = new StringBuffer();
   String tmpStr = "", newLine = "\r\n";
   tempBuf.append("38"); // --代碼 固定 38
   ...
   this.writeTextFile(ilFile38, tempBuf.toString());
...
void checkOpen() throws Exception {
   ...
   ilFile38 = openOutputText(lsTemp, "MS950");
   if (ilFile38 < 0) {
      printf("CRU23B1-TYPE-38 產檔失敗 ! ; file=[%s]", lsTemp);
      okExit(0);
   }
}
...
void checkOnlyOneCnt(String lsCardNo) throws Exception {
   ...
   sql1 += " and acct_type ='05' and card_no = ? ";
   setString(1, isProcDate);
...
void checkAcnolog(String lsIdPSeqno) throws Exception {
   ...
   sql1 += " where emend_type ='3' and acct_type ='05' and log_date = ? and id_p_seqno = ? ";
