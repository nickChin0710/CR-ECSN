...
public class InfC023 extends BaseBatch {
	private final String progname = "產生送CRDB 23異動英文戶名資料檔程式   112/10/26 V1.00.02";
	...
	protected void dataProcess(String[] args) throws Exception {
		...
		isFileName = "CRU23B1_TYPE_23_" + hChgDate + ".txt";
	...
	void writeText() throws Exception {
	    ...
	    sqlCmd += " ,(CASE WHEN a.debit_flag = 'Y' THEN (select eng_name from dbc_idno where id_p_seqno = a.id_p_seqno) ";
	    sqlCmd += " WHEN a.debit_flag != 'Y' THEN (select eng_name from crd_card where id_p_seqno = a.id_p_seqno ORDER BY ISSUE_DATE DESC FETCH FIRST 1 ROWS ONLY) END) as h_eng_name ";
	    sqlCmd += " from cms_chgcolumn_log as a ";
		sqlCmd += " where a.chg_date = ? ";
		sqlCmd += " and a.chg_table in ('dbc_card','crd_card') ";
		sqlCmd += " and a.chg_column = 'eng_name' ";
		...
		while (fetchTable()) {
			...
			allStr += "23";
			allStr += commString.space(16);
			allStr += commString.rpad(colSs("h_id_no"), 11);
			allStr += comc.fixLeft(commString.left(colSs("h_eng_name"),26),26);
			allStr += commString.space(95);
			allStr += "\r\n";
			totalCnt++;
