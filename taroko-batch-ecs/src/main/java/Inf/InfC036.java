...
*  ---------  --------- ----------- ---------------------------------------- *
*  110/08/24  V1.00.00   Castor     program initial                          *
*                                                                            *
*****************************************************************************/
package Inf;
...
public class InfC036 extends AccessDAO {
  private final String progname = "產生送CRDB 36更新帳單最低應繳資料檔程式 110/08/24 V1.00.00";
  private String prgmId = "InfC036";
  ...
  private final String fileName = "CRU23B1_TYPE_36_";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      List<ChgObj036> chgObjList = findChgObjList(searchDate);
		...
		// open CRU23B1_TYPE_36_yyyymmdd.txt file
   ...
   * @throws Exception
   */
  private List<ChgObj036> findChgObjList(String searchDate) throws Exception {
    System.out.print(searchDate);

	sqlCmd = " SELECT P_SEQNO,  ";
	sqlCmd += " UF_IDNO_ID(ID_P_SEQNO) ID_NO, ";
	sqlCmd += " to_char(round(abs(MIN_PAY),0),'000000000') MIN_PAY ";
	sqlCmd += " FROM  ACT_ACCT ";
	sqlCmd += " WHERE MIN_PAY  > 0 ";  //對帳單最低應繳
    ...
    List<ChgObj036> chgObjList = new LinkedList<ChgObj036>();
    ChgObj036 chgObj = null;
    

    for (int i = 0; i < selectCount; i++) {
    	
      chgObj = new ChgObj036();
      chgObj.cardNo = selectCardNo(getValue("P_SEQNO", i));
      chgObj.idNo = getValue("ID_NO", i);
      chgObj.minPay = getValue("MIN_PAY", i);
   ...
   */
  private String getOutputString(List<ChgObj036> chgObjList) throws UnsupportedEncodingException {
    ...
    for (ChgObj036 chgObj : chgObjList) {
    	
      sb.append("36");
      sb.append(comc.fixLeft(chgObj.cardNo, 16));
      sb.append(comc.fixLeft(chgObj.idNo, 11));
      sb.append(comc.fixLeft(chgObj.minPay, 9));
      sb.append(comc.fixLeft("", 112));
