...
public class InfC013 extends BaseBatch {
private final String progname = "產生送CRDB 13 卡人預現成數異動 2023-1206 V1.00.07";
CommCrd comc = new CommCrd();
CommFTP commFTP = null;
CommRoutine comr = null;

String isFileName = "";
private int ilFile13;
String isProcDate = "";
String hIdPSeqno = "";
String hIdNo = "";
String hOppoStatus = "";
...
protected void dataProcess(String[] args) throws Exception {
   ...
   if (liArg > 1) {
      printf("Usage : InfC013 [business_date]");
      okExit(0);
   }
   ...
   isFileName = "CRU23B1_TYPE_13_"+isProcDate+".txt";
   printf("Process: proc_date[%s], file_name[%s]", isProcDate,isFileName);

   checkOpen();
   selectDataType13();
   closeOutputText(ilFile13);
...
void selectDataType13() throws Exception {
   ...
   while (fetchTable()) {
      initData();
      hIdPSeqno = colSs("type13.id_p_seqno");
      checkOnlyOneType5(hIdPSeqno);
      if (hContinue)
         continue;
      // h_id_no = col_ss("type13.id_no");
      hAcnoPSeqno = colSs("type13.acno_p_seqno");
      hAftAmt = getAcnoAmt(hAcnoPSeqno);
      // h_aft_cash = col_int("type13.aft_loc_cash");
      writeTextFile13();
   }
   closeCursor();
//--------------------------------------------------------
   ...
   sqlCmd = " select acno_p_seqno , id_p_seqno "
       +" from rsk_acnolog "
       +" where log_date = ? "
       +" and emend_type ='1' and son_card_flag <> 'Y' "
       +" group by id_p_seqno , acno_p_seqno ";
   ...
   while (fetchTable()) {
      initData();
      hIdPSeqno = colSs("type131.id_p_seqno");
      hIdNo = colSs("type131.id_no");
      checkOnlyOneType1(hIdPSeqno);
      if (hContinue)
         continue;
      hAcnoPSeqno = colSs("type131.acno_p_seqno");
      // h_cal_bef_amt = col_num("type131.bef_loc_amt");
      // h_cal_bef_cash = col_num("type131.bef_loc_cash");
      hAftAmt = getAcnoAmt(hAcnoPSeqno);
...
void checkOpen() throws Exception {
   ...
   ilFile13 = openOutputText(lsTemp, "MS950");
   if (ilFile13 < 0) {
      printf("CRU23B1-TYPE-13 產檔失敗 ! ; file[%s]", lsTemp);
      okExit(0);
   }
}
...
void writeTextFile13() throws Exception {
   StringBuffer tempBuf = new StringBuffer();
   String tempStr = "", newLine = "\r\n";
   int tempInt = 0;
   tempBuf.append("13"); // --代碼 固定 13
   tempBuf.append(comc.fixLeft("", 16)); // --保留 16
   ...
   this.writeTextFile(ilFile13, tempBuf.toString());
