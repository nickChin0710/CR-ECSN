...
*  ---------  --------- ----------- ---------------------------------------- *
*  110/08/10  V1.00.00   Castor     program initial                          *
*                                                                            *
*****************************************************************************/
package Inf;
...
public class InfC016 extends AccessDAO {
  private final String progname = "產生送CRDB 16更新卡人年利率資料檔程式 110/08/10 V1.00.00";
  private String prgmId = "InfC016";
  ...
  private final String fileName = "CRU23B1_TYPE_16_";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      List<ChgObj016> chgObjList = findChgObjList(searchDate);
		...
		// open CRU23B1_TYPE_16_yyyymmdd.txt file
   ...
   * @throws Exception
   */
  private List<ChgObj016> findChgObjList(String searchDate) throws Exception {
    System.out.print(searchDate);

	sqlCmd = " SELECT P_SEQNO,  ";
	sqlCmd += " UF_IDNO_ID(ACT_ACNO.ID_P_SEQNO) ID_NO, ";
	sqlCmd += " to_char(round(RCRATE_YEAR,2),'00.00') REVOLVE_INT_RATE  ";
	sqlCmd += " FROM ACT_ACNO ";
	sqlCmd += " WHERE  1=1 ";
	sqlCmd += " AND REVOLVE_RATE_S_MONTH || '01'= ? ";
    ...
    List<ChgObj016> chgObjList = new LinkedList<ChgObj016>();
    ChgObj016 chgObj = null;
    String tmpRevolveIntRate="";

    for (int i = 0; i < selectCount; i++) {
    	
      chgObj = new ChgObj016();
      chgObj.cardNo = selectCardNo(getValue("P_SEQNO", i));
      chgObj.idNo = getValue("ID_NO", i);
      tmpRevolveIntRate = getValue("REVOLVE_INT_RATE", i) ;
      chgObj.revolveIntRate = tmpRevolveIntRate.substring(0,2)+tmpRevolveIntRate.substring(3,5) ;
   ...
   */
  private String getOutputString(List<ChgObj016> chgObjList) throws UnsupportedEncodingException {
	byte[] lineSeparatorArr = {'\r','\n'}; //0D0A
	String nextLine = new String(lineSeparatorArr,"MS950");
	
    StringBuilder sb = new StringBuilder();

    for (ChgObj016 chgObj : chgObjList) {
    	
      sb.append("16");
      sb.append(comc.fixLeft(chgObj.cardNo, 16));
      sb.append(comc.fixLeft(chgObj.idNo, 11));
      sb.append(comc.fixLeft(chgObj.revolveIntRate, 4));
      sb.append(comc.fixLeft("", 117));
