...
*  ---------  --------- ----------- ---------------------------------------- *
*  110/08/23  V1.00.00   Castor     program initial                          *
*  113/01/04  V1.00.01   Ryan       findChgObjList  對帳單總應繳款(TTL_AMT)改為大於0    *
*  113/01/05  V1.00.02   Ryan       產檔時移除TTL_AMT_ABS欄位                                                         *
*****************************************************************************/
package Inf;
...
public class InfC035 extends AccessDAO {
  private final String progname = "產生送CRDB 35更新帳單應繳總額資料檔程式 113/01/05 V1.00.02";
  private String prgmId = "InfC035";
  ...
  private final String fileName = "CRU23B1_TYPE_35_";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      List<ChgObj035> chgObjList = findChgObjList(searchDate);
		...
		// open CRU23B1_TYPE_35_yyyymmdd.txt file
   ...
   * @throws Exception
   */
  private List<ChgObj035> findChgObjList(String searchDate) throws Exception {
    System.out.print(searchDate);

	sqlCmd = " SELECT P_SEQNO,  ";
	sqlCmd += " UF_IDNO_ID(ID_P_SEQNO) ID_NO, ";
	sqlCmd += " '' TTL_AMT_ABS, ";
	sqlCmd += " to_char(round(abs(TTL_AMT),0),'000000000') TTL_AMT ";
    ...
    List<ChgObj035> chgObjList = new LinkedList<ChgObj035>();
    ChgObj035 chgObj = null;
    

    for (int i = 0; i < selectCount; i++) {
    	
      chgObj = new ChgObj035();
      chgObj.cardNo = selectCardNo(getValue("P_SEQNO", i));
      chgObj.idNo = getValue("ID_NO", i);
//      chgObj.ttlAmtAbs = getValue("TTL_AMT_ABS", i);
      chgObj.ttlAmt = getValue("TTL_AMT", i);
   ...
   */
  private String getOutputString(List<ChgObj035> chgObjList) throws UnsupportedEncodingException {
    ...
    for (ChgObj035 chgObj : chgObjList) {
    	
      sb.append("35");
      sb.append(comc.fixLeft(chgObj.cardNo, 16));
      sb.append(comc.fixLeft(chgObj.idNo, 11));
//      sb.append(comc.fixLeft(chgObj.ttlAmtAbs, 1));
      sb.append(comc.fixLeft(chgObj.ttlAmt, 9));
      sb.append(comc.fixLeft("", 111));
