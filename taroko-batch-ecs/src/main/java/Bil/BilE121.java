...
public class BilE121 extends AccessDAO {
    private  final String progname = "FISC請款檔分派處理程式-ICFXJQB磁條卡交易 111/09/22 V1.00.06";
	...
	public int mainProcess(String[] args) {
        ...
        try {
			...
			while (fetchTable()) {
				BilE122 bil122 = new BilE122();	
				BilE123 bil123 = new BilE123();
				
				// example: fileName : ICFXJQBD : F00600000.ICFXJQBD.YYMMDDNN.txt
				...
				if(  fileName.substring(1, 18).equals(bankNo + ".ICFXJQBD") ) {
					...
					rtn = bil122.mainProcess(newArgs);
					if (rtn < 0) {
						return rtn;
					}
					rtn = bil123.mainProcess(newArgs);
					if (rtn < 0) {
						return rtn;
					}	
