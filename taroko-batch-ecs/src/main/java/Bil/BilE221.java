...
*  112/08/30  V1.00.03    JeffKung  copy from BilE210                        *
*****************************************************************************/
package Bil;
...
public class BilE221 extends BilE200 {
    private  final String PROGNAME = "公用事業費用請款(重覆資料回沖) 112/08/30 V1.00.03";
    ...
    protected int inputFileProcess() throws Exception {
        ...
        sqlCmd += " and file_name like 'PUCARD_IN_REV.%' ";
...
//    };
	...
	int open_file() throws Exception {
		outputFileName = String.format("PUCARD_OUT_REV.TXT");
...
//	}
	...
	private void getDataD(BilE200InData onusInData, byte[] bytesArr) throws Exception {
		...
		*/
		...
		String[] txt = new String[] {"25", "05"};  //回沖時要顛倒
		...
		if ("25".equals(onusInData.txnCode)) {

			if ("11".equals(payItem) && ("81".equals(itemBId) || "581".equals(itemClass))) {
				onusInData.mchtChiName = "沖正中華話費" + comc.getSubString(userNo, 4, 9) + "***"
