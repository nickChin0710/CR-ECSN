...
*  109/03/12  V1.00.01    JustinWu     delete getDataD method             *
*  109/05/25  V1.00.02    JustinWu     remove .txt String following fileName
*  109/05/26  V1.00.03    JustinWu     change to use the file path from the database and double->BigDecimal
*  109/06/29  V1.00.04    JustinWu     修改取得VISA匯率(Mid Rate)金額的起迄位置
...
*  111/09/22  V1.00.10    JeffKung     substring使用共用function comc.getSubString  *
*****************************************************************************/
package Bil;
...
public class BilE112 extends AccessDAO {
	private  final String progname = "FISC-ICFXGQB磁條卡請款查核處理 111/09/22 V1.00.10";
...
//	        comcr.h_call_batch_seqno = args.length > 0 ? args[args.length - 1] : "";
			...
			// F00600000.ICFXGQBA.XXXXXXXX -> F00600000.ICFXGQBD.XXXXXXXX
			...
			//open  F00600000.ICFXGQBA.XXXXX file=====================
			...
			while(true) {
				...
				if(text.trim().length() != 0 ) {
					...
					//commTxBill.printDebugString("======" + fileNameA + " ======");
			...
			while(true) {
				...
				if(bytes.length != 0 ) {
						...
						// if true表示是通知檔內第一筆美元匯率資訊,取出VISA匯率(Mid Rate)金額
