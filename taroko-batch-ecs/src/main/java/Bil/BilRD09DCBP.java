...
/*國際信用卡清算明細表-VD悠遊卡餘額轉置*/
public class BilRD09DCBP extends AccessDAO {
    private String progname = "列印國際信用卡清算明細表-VD悠遊卡餘額轉置程式  112/11/27  V1.00.02";
    CommFunction   comm  = new CommFunction();
    CommCrd        comc  = new CommCrd();
    CommCrdRoutine comcr = null;

    String prgmId = "BilRD09DCBP";
    String prgmName = "列印國際信用卡清算明細表-VD悠遊卡餘額轉置程式";
    String rptName = "國際信用卡清算明細表-VD悠遊卡餘額轉置";
    String rptId = "D09_DCBP";
    int rptSeq = 0;
    ...
    void printHeader() {
        ...
        buf = comcr.insertStr(buf, "通知日期", 23);  //8
        ...
        buf = comcr.insertStr(buf, "處理結果", 110); //3
        lpar1.add(comcr.putReport(rptId, rptName, sysDate, ++rptSeq, "0", buf));
    ...
    void printFooter() {
        ...
        buf = "";
        buf = comcr.insertStr(buf, "餘額轉置帳款合計:", 1);
        ...
        buf = "";
        buf = comcr.insertStr(buf, "餘額轉置剔退合計:", 1);
    ...
    void printDetail(String orgData) throws Exception {
        ...
        String destinationAmtSign = comc.subMS950String(bytes, 36, 1);
        String destinationAmt = comc.subMS950String(bytes, 37, 12);
        String balanceDate = comc.subMS950String(bytes, 49, 8);
        String respCode = comc.subMS950String(bytes, 57, 4);
        String serviceAmtSign = comc.subMS950String(bytes, 61, 1);
        String serviceAmt = comc.subMS950String(bytes, 62, 12);

        int destAmt = 0;
        if ("-".equals(destinationAmtSign)) {
        	destAmt = comc.str2int(destinationAmt) * -1;
        } else {
        	destAmt = comc.str2int(destinationAmt);
        }
        
        int svcAmt = 0;
        if ("-".equals(serviceAmtSign)) {
        	svcAmt = comc.str2int(serviceAmt) * -1;
        } else {
        	svcAmt = comc.str2int(serviceAmt);
        }

        
        buf = "";
        buf = comcr.insertStr(buf, tscCardNo, 1);  //外顯卡號
        buf = comcr.insertStr(buf, balanceDate, 23); //交易日期
        buf = comcr.insertStr(buf, "", 35); //交易時間
        buf = comcr.insertStr(buf, "餘額轉置回應", 45); //交易摘要
        buf = comcr.insertStr(buf, comcr.commFormat("3$,3$",destAmt), 88); //交易金額
        buf = comcr.insertStr(buf, respCode, 101); //交易代號
        buf = comcr.insertStr(buf, getValue("rpt_resp_code"), 110); //處理結果
       
        totalTWDDestAmtDR = totalTWDDestAmtDR + destAmt;
        totalTWDCntDR = totalTWDCntDR + 1;
        
        //若剔退時加總
        if("0000".equals(getValue("rpt_resp_code"))==false) {
        	totalTWDDestAmtCR = totalTWDDestAmtCR + destAmt;
	...
	void procVouchData() throws Exception {
		...
		tmpstr = String.format("D09DCBP_%s.%s_%s","C02", hVouchCdKind, hPcceCurrCodeGl);  
		comcr.hGsvhModPgm = tmpstr;
		comcr.hGsvhModWs = "BIL_DCBP";
    ...
    public static void main(String[] args) throws Exception {
        BilRD09DCBP proc = new BilRD09DCBP();
