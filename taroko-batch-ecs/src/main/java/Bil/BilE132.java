...
public class BilE132 extends AccessDAO {
    private  final String progname = "FISC-ICEMVQB晶片卡請款查核處理 111/09/22 V1.00.10";
...
//	        comcr.h_call_batch_seqno = args.length > 0 ? args[args.length - 1] : "";
			...
			// F00600000.ICEMVQBD.XXXXXXXX -> F00600000.ICEMVQBD.XXXXXXXX
			...
			while(true) {
				...
				if (bytes.length != 0 ) {
					    ...
					    // if the string converted from the 5th byte equals "2", 
					    // it means this data is record2, so continue.
					   if( CommTxBill.subByteToStr(bytes, 4,5).equals("2")  ) {
						   continue;
					    }
			...
			//commTxBill.printDebugString("cntD+1: " + cntD );
