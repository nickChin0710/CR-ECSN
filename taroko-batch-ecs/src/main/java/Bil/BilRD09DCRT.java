...
/*國際信用卡清算明細表-VD悠遊卡退卡*/
public class BilRD09DCRT extends AccessDAO {
    private String progname = "列印國際信用卡清算明細表-VD悠遊卡退卡資料程式  112/11/27  V1.00.02";
    CommFunction   comm  = new CommFunction();
    CommCrd        comc  = new CommCrd();
    CommCrdRoutine comcr = null;

    String prgmId = "BilRD09DCRT";
    String prgmName = "列印國際信用卡清算明細表-VD悠遊卡退卡程式";
    String rptName = "國際信用卡清算明細表-VD悠遊卡退卡";
    String rptId = "D09_DCRT";
    int rptSeq = 0;
    ...
    void printHeader() {
        ...
        buf = comcr.insertStr(buf, "退卡日期", 23);  //8
        buf = comcr.insertStr(buf, "退卡時間", 34);  //8
        buf = comcr.insertStr(buf, "交易摘要", 45);  //40
        buf = comcr.insertStr(buf, "退費金額", 88);  //8
        buf = comcr.insertStr(buf, "交易代碼", 99);  //4
        buf = comcr.insertStr(buf, "退卡來源", 110); //3
    ...
    void printFooter() {
        ...
        buf = "";
        buf = comcr.insertStr(buf, "退卡帳款合計:", 1);
        ...
        buf = "";
        buf = comcr.insertStr(buf, "退卡帳款剔退合計:", 1);
    ...
    void printDetail(String orgData) throws Exception {
        ...
        String destinationAmtSign = comc.subMS950String(bytes, 37, 1);
        String destinationAmt = comc.subMS950String(bytes, 38, 12);
        String returnSource = comc.subMS950String(bytes, 76, 3);
        String trafficAbbr = comc.subMS950String(bytes, 110, 20);
        String addrAbbr = comc.subMS950String(bytes, 130, 20);
        String tscTxCode = comc.subMS950String(bytes, 150, 4);
        ...
        buf = comcr.insertStr(buf, returnSource, 110); //連線註記
	...
	void procVouchData() throws Exception {
		...
		tmpstr = String.format("D09DCRT_%s.%s_%s","C02", hVouchCdKind, hPcceCurrCodeGl);  
		comcr.hGsvhModPgm = tmpstr;
		comcr.hGsvhModWs = "BIL_DCRT";
    ...
    public static void main(String[] args) throws Exception {
        BilRD09DCRT proc = new BilRD09DCRT();
