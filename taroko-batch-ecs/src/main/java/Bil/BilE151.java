...
public class BilE151 extends AccessDAO {
  private final String progname = "FISC請款檔分派處理程式-ICPTXQQ ONUS交易 111/09/22 V1.00.07";
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      whereStr = " where 1=1 " + " and system_id='FISC_FTP' " + " and source_from='FISC_FCARD' "
          + " and group_id in ('ICPTXQQA', 'ICPTXQQD') " + " and trans_mode='RECV' "
          + " and trans_resp_code = 'Y' " + " and proc_code in ('', '0') ";

      openCursor();
      while (fetchTable()) {
    	  BilE152 bil152 = new BilE152();
          BilE153 bil153 = new BilE153();
        ...
        if ( comc.getSubString(fileName,1, 18).equals(bankNo + ".ICPTXQQD")) {
          ...
          rtn = bil152.mainProcess(newArgs);
          if (rtn < 0) {
            return rtn;
          }
          rtn = bil153.mainProcess(newArgs);
          if (rtn < 0) {
            return rtn;
          }
