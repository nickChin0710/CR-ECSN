...
public class BilE204 extends BilE200 {
    private  final String PROGNAME = "新光火險代扣保費處理程式 112/08/30 V1.00.03";
    ...
    protected int inputFileProcess() throws Exception {
        ...
        sqlCmd += " and file_name like '03458403_3144_C02_%' ";
	...
	int open_file() throws Exception {
		outputFileName = String.format("03458403_3144_C03_%s.txt",hBusinessDate);
	...
	void writeMediaFile(int fo) throws Exception {
		...
		if (getValue("outboe.rec_type").equals("1")) {
			...
			outStr.append(comc.fixLeft(getValue("outboe.row_data"), 91));
			//System.out.println("purchase_date:"+getValueInt("outboe.purchase_date"));
			//outStr.replace(1, 9, fixNum(String.format("%s", getValueInt("outboe.purchase_date")-19110000),8));
			outStr.replace(9, 10, "2");
			outData += comc.getSubString(outStr.substring(0), 0,91);
			...
			outStr = new StringBuffer();
			outStr.append(comc.fixLeft(getValue("outboe.row_data"), 91));
			//outStr.replace(36, 38, rpad(getValue("outboe.txn_code"),2));//交易碼
	...
	private void getDataD(BilE200InData onusInData, byte[] bytesArr) throws Exception {
		if (CommTxBill.subByteToStr(bytesArr, 0, 1).equals("2"))  //明細資料
		{
			...
			onusInData.expireMMYY = CommTxBill.subByteToStr(bytesArr, 32,36);  // 效期(MMYY)
			...
			if (CommTxBill.subByteToStr(bytesArr, 0, 1).equals("1"))  {  //首筆資料
				onusInData.purchaseDate = CommTxBill.subByteToStr(bytesArr, 1,9);
	...
	void procFTP(String isFileName) throws Exception {
		...
		commFTP.hEflgSourceFrom = "BilE204"; /* 區分不同類的 FTP 檔案-細分類 (非必要) */
