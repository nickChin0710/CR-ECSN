...
/*接收農業金庫代收檔處理程式*/
public class ActB105 extends AccessDAO {

  public static final boolean DEBUG_MODE = false;

  private final String PROGNAME = "接收農業金庫代收檔處理程式  112/10/01 "
                                + "V1.00.07";
...
//contentBuf content = new contentBuf();
  ...
  public int mainProcess(String[] args) {
    ...
    try {
      ...
      ***/
      ...
      /*** 抓日期為前一天及當天之農業金庫代收檔 BI006yyymmdd.txt  ***/

      /***
      hChiDate = String.format("%07d", comcr.str2long(hTempPresysdate) - 19110000);
      if (checkDateFile(hChiDate) == 0) {
  	  	procInputFile(hChiDate);
      }
    ...
    //String filePath = String.format("/crdataupload/");
	    ...
	    String getFileDate = "";
	  	for (String file : listOfFiles) {
	  		if (commString.mid(file, 0, 5).equals("BI006") &&
	  		    commString.mid(file, 8, 2).compareTo("01") >= 0 &&
	  		    commString.mid(file, 8, 2).compareTo("12") <= 0 &&
	  		    commString.mid(file, 10, 2).compareTo("01") >= 0 &&
	  		    commString.mid(file, 10, 2).compareTo("31") <= 0 )
	  		{
    			showLogMessage("I", "", "File = [" + file + "]");
				//getFileDate = commString.mid(file, 5, 12);
				  getFileDate = commString.mid(file, 5, 7);
          hFileDate = String.format("%08d", comcr.str2long(getFileDate) + 19110000);
      ...
      if (hFileProcCnt == 0) {
        String likeFile = "BI006yyymmdd.txt";
        exceptExit = 0;
      //comc.errExit(String.format("本日[%s]無農業金庫代收檔[/crdataupload/%s]",
        comc.errExit(String.format("本日[%s]無農業金庫代收檔[%s/media/act/%s]",
  ...
  /***********************************************************************/
    ...
    hPtclMediaName = "BI006";
    hPtclAcctBank = "770";
  //hPtclExternalName = String.format("BI006%s.txt", chiDate);
...
***/
  ...
  /***********************************************************************/
  int getField770() throws Exception {
    String stra = "";
    ...
    hB031TransDate = hFileDate;
    hB031CollStoreNo = str600.substring(57, 57 + 7).trim();
    hB031TransFlag = "+";
    hApdlPayDate = str600.substring(28, 28 + 7).trim();
    hApdlPayDate = String.format("%08d", comcr.str2long(hApdlPayDate) + 19110000);
    hB031ConsumerNo = str600.substring(4, 4 + 16);
    chkPayment();
    
    stra = str600.substring(20, 20 + 8).trim();
    hB031TransAmt = comcr.str2long(stra);

    // if (val_date(h_apdl_pay_date) != 0)
    if (comcr.str2Date(hApdlPayDate) == null) {
        hB031TransDateFlag = "Y";
        hAperErrorReason = "701";
        hAperErrorRemark = String.format("[%s] trans date error", hPaymentType);
    }

    if (hB031TransAmt <= 0) {
        hB031TransAmtFlag = "Y";
        hAperErrorReason = "702";
  ...
  void chkPayment() throws Exception {
    ...
    if (recordCnt <= 0) {
      ...
      if (recordCnt2 <= 0) {
        showLogMessage("I", "", String.format("COU7 繳款代號錯誤[%s]!", hB031ConsumerNo));
        hAperErrorReason = "703";
        hAperErrorRemark = "COU7 consumer_no error";
    ...
    void insertActPayDetail() throws Exception {
      ...
      //setValue("apyd.crt_user", hApbtModUser); // update_user
        setValue("apyd.crt_user", hB031CollStoreNo); //農漁會本會金融機構代號
      //setValue("apyd.crt_date", sysDate); // update_date
        setValue("apyd.crt_date", hB031TransDate); //檔案日期
        ...
        setValue("apyd.collection_store_no", hB031CollStoreNo);//農漁會本會金融機構代號
        insertTable();
        if (dupRecord.equals("Y")) {
...
***/
      ...
      //setValue("apye.crt_user", hApbtModUser);
        setValue("apye.crt_user", hB031CollStoreNo);//農漁會本會金融機構代號
      //setValue("apye.crt_date", sysDate);
        setValue("apye.crt_date", hB031TransDate);//檔案日期
    ...
    void insertActB003r1() throws Exception {
      ...
      //setValue("ab31.allot_account", "");//劃撥帳號
        setValue("ab31.allot_account", hB031CollStoreNo);//農漁會本會金融機構代號
      ...
      //setValue("ab31.telephone_no", "");
        setValue("ab31.telephone_no", hB031TransDate);//檔案日期
  ...
  /***********************************************************************/
	...
	//hType3totamt = commString.ss2Num(comc.subMS950String(bytes, 22, 6));
	  hType3totcnt = commString.ss2int(comc.subMS950String(bytes, 22, 6));
	  hType3totamt = commString.ss2int(comc.subMS950String(bytes, 28, 13));
	}
