 ...
 *  ---------------  ------------ ----------- ----------------------------------------  *
*   2021/09/06     V1.00.01    JeffKung   initial draft                                    *
 *                                                                                                *
 ******************************************************************************/
...
/*消費回饋聯名機構處理程式*/
public class EcsB060 extends AccessDAO {

    private String PROGNAME = "ECS批次程式排程處理程式(Error自動接續Rerun)  V1.00.01 2021/09/06";
    ...
    void selectEcsBatchCtl() throws Exception {
        ...
        sqlCmd += "  where   proc_flag = 'E' ";
        sqlCmd += "  and       shell_id = ? ";
        sqlCmd += "  and       batch_date = ? ";
        sqlCmd += "  and       wait_flag = 'N' ";  //暫停碼="Y"時, 代表暫停執行
        sqlCmd += "  and       ( rerun_proc <> 'Y' "; 
        sqlCmd += "    or          (rerun_proc = 'Y' and rerun_proc_date = '' ) )";  //畫面異動要rerun
        sqlCmd += "  fetch first 1 rows only ";
        
        setString(1, hEcsBatchCtlShellId );
        setString(2, hBusiBusinessDate);
        
        int cnt1 = selectTable();
