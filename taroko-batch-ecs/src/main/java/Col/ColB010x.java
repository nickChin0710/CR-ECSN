...
*  ---------  --------- ----------- ----------------------------------------  *
*  112/03/17  V1.00.00    Sunny       program initial (copy by ColB010)       * 
******************************************************************************/
...
public class ColB010x extends AccessDAO {
    private String progname = "資料轉置(補檔)--轉催外帳主檔處理程式   112/03/17  V1.00.00 ";
   ...
   /********col_bad_debt**********/
    ...
    String debtTransType = "";
    String debtTransDate = "";
    ...
    public int mainProcess(String[] args) {
        ...
        try {
            ...
            if (args.length > 2) {
                comc.errExit("Usage : ColB010x [trans_date][id_no/corp_no]", "1.trans_date : yyyymmdd 2.id(10碼) or corp_no(8碼或11碼)");
            }
    ...
    void selectColBadDebtPerson() throws Exception {
    	sqlCmd = " select id_p_seqno,id_no,stmt_cycle,p_seqno,trans_type,trans_date ";
    	sqlCmd += " from col_bad_debt a where a.trans_type='3' AND a.ACCT_TYPE='01' ";
    	sqlCmd += " AND a.TRANS_DATE <= ? and a.src_amt>0 ";
        ...
        while (fetchTable()) {
        	...
        	debtTransType = getValue("trans_type");
        	debtTransDate = getValue("trans_date");
    ...
    void selectColBadDebtCompanyBase() throws Exception {
    	sqlCmd = " Select DISTINCT corp_p_seqno,stmt_cycle,trans_date from col_bad_debt a ";
    	sqlCmd += " where a.trans_type='3' AND a.ACCT_TYPE='03' and a.src_amt>0 ";
//    	sqlCmd += " and a.src_amt>0 AND a.TRANS_DATE = ? ";
...
//    	sqlCmd += "AND TRANS_TYPE='4' AND A.TRANS_DATE<=B.TRANS_DATE) and a.src_amt>0 ";
    ...
    **/
    ...
    void insertColBadDebtExthst() throws Exception {
    	sqlCmd = " insert into col_bad_debt_exthst select * from col_bad_debt_ext where rowid = ? ";
	...
	int insertColBadDebtExt(String idCorpPSeqno) throws Exception {
		...
		setValue("debt_ext.TRANS_TYPE", debtTransType); //轉檔時帳戶狀態
		setValue("debt_ext.TRANS_DATE", debtTransDate); //轉檔時帳戶狀態日
	...
	int insertColBadJrnlExt(String acctType) throws Exception {
		...
		setValue("jrnl_ext.ADJ_COMMENT", "3".equals(debtTransType)? "歷史轉檔(催)" : "4".equals(debtTransType)? "歷史轉檔(呆)" : "");
