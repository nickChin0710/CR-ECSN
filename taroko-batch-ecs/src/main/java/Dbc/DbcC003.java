...
*  ---------  --------- ----------- ----------------------------------------  *
*  108/12/26  V1.00.00    Pino      program initial                           *
...
*  111/06/16  V1.00.05    Justin    弱點修正                                  *
*  112/02/08  V1.00.06   Wilson     檔案產生路徑調整                                                                                      *
*  112/03/07  V1.00.07   Wilson     新增procFTP                                *
*  112/03/07  V1.00.08   Wilson     檔案的結束符號改成0D0A                          *
*  112/06/30  V1.00.09   Wilson     檔案格式增加hOnlineOpenflag                   *
...
/*產生VD續卡清單檔程式*/
public class DbcC003 extends AccessDAO {
	private String progname = "產生VD續卡清單檔程式  112/10/20 V1.00.11";
	...
	public int mainProcess(String[] args) throws IOException {
		...
		try {
			...
			// =====================================
			if (args.length > 2) {
				comc.errExit("Usage : DbcC003 Y/N  batch_seq", "Usage : Y-->IBM 碼  , N-->非 IBM 碼 ");
			}
			...
			if (!swAsc.toUpperCase(Locale.TAIWAN).equals("Y") && !swAsc.toUpperCase(Locale.TAIWAN).equals("N")) {
				comcr.hCallErrorDesc = " Usage : DbcC003  Y/N !! ";
	...
	void checkOpen() throws Exception {
		checkFileCtl();
		filename = String.format("rqst_vd_t_%8s%02d.txt", hSystemDate, hNn);
	...
	void checkFileCtl() throws Exception {
		String likeFilename = "";
		String hFileName = "";
		likeFilename = String.format("rqst_vd_t_%8s", hSystemDate) + "%";
