...
*  ---------  --------- ----------- ----------------------------------------  *
*  109/03/02  V1.00.00    Pino      program initial                           *
*  109/05/14  V1.00.01    Wilson    bk -> backup                              *
*  109/11/13  V1.00.02  yanghan       修改了變量名稱和方法名稱                                                                               *
*  109/12/24  V1.00.03  yanghan       修改了變量名稱和方法名稱            *
*  112/02/08  V1.00.04    Wilson    讀取檔案路徑調整                                                                                      *
*  112/04/25  V1.00.05    Wilson    hDdtpRtnCode = "000" -> hRejectCode = ""  *
*  112/06/13  V1.00.06    Wilson    無檔案不當掉                                                                                             *
*  112/06/19  V1.00.07    Wilson    hRejectCode chg to hCheckCode             *
*  112/07/03  V1.00.08    Wilson    假日不執行                                                                                                 *
******************************************************************************/
...
/*讀取VD重製清單回覆檔程式*/
public class DbcC006 extends AccessDAO {
	private String progname = "讀取VD重製清單回覆檔程式 112/07/03  V1.00.08";
...
//		final String fileNameTemplate = String.format("%s\\.%s\\..*", "F00600000", "ICCRSQND"); // 檔案正規表達式		
		...
		for (String file : listOfFiles) {
			...
			if(!comc.getSubString(iFileName,0,10).equals("resp_vd_s_"))  
			{
...
//			hCheckCode = "D52";
	...
	/***********************************************************************/
	int updateDbcDebit() throws Exception {
		if (debug == 1)
		...
		if ((hDcepCardNo.length() > 0)) {
			...
			if (hDdtpRtnIbmDate.length() >= 8) {
				stderr = String.format("cardno[%s]此筆資料IBM已回饋,不可重複寫入", hDcepCardNo);
