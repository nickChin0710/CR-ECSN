...
public class DbcD042 extends AccessDAO {
  private final String progname = "接收設一科分行整批註銷作業處理程式 111/06/08 V1.00.07";
  ...
  protected final int[] ed1Ength = {2, 19, 16, 2, 8, 53};
  ...
  public int mainProcess(String[] args) {
    try {
      ...
      // =====================================
      if (args.length > 2) {
        comc.errExit("Usage : DbcD042 [date] [callbatch_seqno]", "");
      }
      ...
      if (args.length > 2) {
        String err1 = "DbcD042 [date] [seq_no]\n";
        comcr.errRtn("DbcD042 [date] [seq_no] ", "", comcr.hCallBatchSeqno);
      }
...
//      comcr.errRtn(String.format("[%s]無檔案可處理!!", filepath), "", hCallBatchSeqno);
    ...
    for (String file : listOfFiles) {
      getFileName = file;
      if (!file.substring(0, 3).equals("VE_")) {
        continue;
      }
      getFileName = file;
      veFile = comc.getECSHOME() + "/media/crd/" + getFileName;
      veFile = Normalizer.normalize(veFile, java.text.Normalizer.Form.NFKD);
  ...
  int readFileData() throws Exception {
    int fi = openInputText(veFile, "MS950");
    if (fi == -1)
      return 1;
    ...
    while (true) {
      ...
      moveData(processDataRecord(getFieldValue(rec, ed1Ength, rec.length()), dt1));
