...
*  ---------  --------- ----------- ----------------------------------------  *
*  109/02/17  V1.00.00    Rou        program initial                          *
*  109/02/26  V1.00.00    Rou        二次修改                                                                                           *
*  109/05/06  V1.00.00    Rou        三次修改                                                                                           *
...
public class DbcD041 extends AccessDAO {
  private final String PROGNAME = "接收設一科未啟用註銷作業處理程式  111/06/08  V1.00.07";
  ...
  public int mainProcess(String[] args) {
    try {
      ...
      // =====================================
      if (args.length > 2) {
        comc.errExit("Usage : DbcD041 [date] [callbatch_seqno]", "");
      }
      ...
      if (args.length > 2) {
        String err1 = "DbcD041 [date] [seq_no]\n";
        comcr.errRtn("DbcD041 [date] [seq_no] ", "", comcr.hCallBatchSeqno);
      }
...
//      comcr.errRtn(String.format("[%s]無檔案可處理!!", filepath), "", hCallBatchSeqno);
    ...
    for (String file : listOfFiles) {
      getFileName = file;
      if (!file.substring(0, 3).equals("C4_")) {
        continue;
      }
      getFileName = file;
      c4File = comc.getECSHOME() + "/media/crd/" + getFileName;
      c4File = Normalizer.normalize(c4File, java.text.Normalizer.Form.NFKD);
  ...
  int readFileData() throws Exception {
    int fi = openInputText(c4File, "MS950");
    if (fi == -1)
      return 1;
  ...
  int selectCrdCard() throws Exception {
    sqlCmd = "select ";
    sqlCmd += "card_no,acct_type,acno_p_seqno,id_p_seqno,lost_fee_code ";
    ...
    if (recordCnt > 0) {
      if (debug == 1)
        showLogMessage("I", "", "Read card=[" + hCardNo + "]");
      showLogMessage("I", "", "card = [" + hCardNo + "] is combo card.");
      ibDebit = false;
  ...
  void selectDbcCard() throws Exception {
    ...
    if (recordCnt > 0) {
      if (debug == 1)
        showLogMessage("I", "", "Read card=[" + hCardNo + "]");
      showLogMessage("I", "", "card = [" + hCardNo + "] is vd card.");
      ibDebit = true;
      hAcctType = getValue("acct_type");
  ...
  void selectDbcEmboss() throws Exception {
    ...
    if (recordCnt > 0) {
      showLogMessage("I", "", "Read card=[" + hCardNo + "]");
      showLogMessage("I", "", "card = [" + hCardNo + "] is vd perfab card.");
      updateDbcEmboss();
    }
  ...
  void updateCrdCard() throws Exception {
    ...
    if (notFound.equals("Y")) {
      comcr.errRtn("update_crd_card not found!", hCardNo, comcr.hCallBatchSeqno);
      return;
    }
    insertOnbat2ecs();
  }
