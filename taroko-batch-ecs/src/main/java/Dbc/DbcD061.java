...
*  ---------  --------- ----------- --------------------------------------- *
*  108/12/24  V1.01.01  Rou         Initial                                 *
*  									Update create_nccc_new_file()			*
*  109/04/21  V1.01.02  Rou         二次修改                                                                                                *
...
/*產生VISA DEBIT預製卡製卡檔程式*/
public class DbcD061 extends AccessDAO {
    private String progname = "產生VISA DEBIT預製卡製卡檔程式  112/12/11  V1.00.18";
	...
	boolean getVdDebit() throws Exception {    	
        ...
        for (String file : listOfFiles) {
            ...
            if (getFileName.length() != 29)    
            	continue;
            if (!getFileName.substring(0, 15).equals("vd_debit_mkc_p_"))
                continue;           
...
//            	continue;           
    ...
    int openTextFile() throws Exception {
        ...
        String likeFilename = "";
    	String hFileName = "";
    	likeFilename = String.format("vd_%02d_makecard_pf_%s.txt", tmpVendor, hSysDate) + "%";
        ...
        hNcccFilename = String.format("vd_%02d_makecard_pf_%s%02d.txt", tmpVendor, hSysDate, hNn);
