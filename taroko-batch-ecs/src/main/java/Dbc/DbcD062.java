...
*  ---------  --------- ----------- --------------------------------------- *
*  109/05/15   V1.01.01     Rou         Initial program                     *
*  109/11/16  V1.01.02  yanghan       修改了變量名稱和方法名稱                                                            *
*  109/11/25  V1.01.03  Wilson        outPutTextFile調整                                                           *  
...
*  112/07/04  V1.00.17  Wilson      處理悠遊卡自動加值旗標                                                                     *
*  112/08/26  V1.00.18  Wilson      修正弱掃問題                                                                                       *
*  112/12/11  V1.00.19  Wilson      crd_item_unit不判斷卡種                                                        *
****************************************************************************/
...
/*產生VISA DEBIT新製/重製製卡檔程式*/
public class DbcD062 extends AccessDAO {
    private String progname = "產生VISA DEBIT新製/重製製卡檔程式  112/12/11  V1.00.19";
	...
	boolean getVdDebit() throws Exception {    	
        ...
        for (String file : listOfFiles) {
            ...
            if (getFileName.length() != 30)    
            	continue;
            if (!getFileName.substring(0, 16).equals("vd_debit_mkc_rs_"))
                continue;           
//            if (!getFileName.substring(16, 24).equals(fileDate))
//            	continue;
//          if (checkFileCtl() != 0)
//            	continue;           
            fileCount ++;
    ...
    int openTextFile() throws Exception {
        ...
        String likeFilename = "";
    	String hFileName = "";
    	likeFilename = String.format("vd_%02d_makecard_ns_%s.txt", tmpVendor, hSysDate) + "%";
        ...
        hNcccFilename = String.format("vd_%02d_makecard_ns_%s%02d.txt", tmpVendor, hSysDate, hNn);
...
//        sqlCmd += "  and d.card_type     = a.card_type ";
        sqlCmd += "  and a.apply_source in ('R', 'S') ";
...
if(debug == 1) 
   showLogMessage("I", "", "   pvv="+hDcesPvv+","+hDcesCvv+","+hDcesCvv2+","+hDcesPvki);
    ...
    void createNcccNewFile() throws Exception {
        ...
        else if(hDcesElectronicCode.equals("01")){
        	if(hDcesTscAutoloadFlag.length() > 0) {
        		ncccData.autoloadDefYn = hDcesTscAutoloadFlag;
        	}
        	else {
...
//            pYOfflnLmt      = getValueInt("u_offln_lmt");
